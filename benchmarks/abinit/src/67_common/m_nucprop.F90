...
module m_nucprop
  ...
  use m_linalg_interfaces, only: AB_DSYEV
...
!!  rhor(nfft,nspden)=electron density on grid (strictly $\tiAB_LDE{n}+\hat{n}$)
...
!!
!! CHILDREN
!!      AB_DSYEV,free_my_atmtab,get_my_atmtab,make_efg_el,make_efg_ion
...
!End of the abilint section
    ...
    LDA=3; LWORK=8;N=3 ! these parameters are needed for the LAPACK AB_DSYEV routine
    do iatom = 1, natom
       matr(:,:) = efg(:,:,iatom)
       call AB_DSYEV('V','U',N,matr,LDA,eigval,work,LWORK,INFO) ! get eigenvalues and eigenvectors
...
!! nsym=number of symmetries in space group
!! rhor(nfft,nspden), valence electron density, here $\tiAB_LDE{n} + \hat{n}$
...
!! routine computes the second derivatives of the potential generated by $\tiAB_LDE{n}$ (see Kresse and Joubert
...
!! \left(\frac{\tiAB_LDE{n}(G)}{\pi G^2}\right)e^{2\pi i G\cdot R}
!! \end{displaymath}
!!
!!
!! PARENTS
