...
*  ===========
*
*       SUBROUTINE AB_SGEQR( M, N, A, LDA, T, TSIZE, WORK, LWORK,
*                         INFO )
*
*       .. Scalar Arguments ..
...
*>
*> \verbatim
*> AB_SGEQR computes a QR factorization of an M-by-N matrix A.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*>                           AB_SLATSQR or AB_SGEQRT
*>
*>  Depending on the matrix dimensions M and N, and row and column
*>  block sizes MB and NB returned by AB_ILAENV, AB_SGEQR will use either
*>  AB_SLATSQR (if the matrix is tall-and-skinny) or AB_SGEQRT to compute
...
*     .. External Subroutines ..
      EXTERNAL           AB_SLATSQR, AB_SGEQRT, AB_XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( MIN( M, N ).GT.0 ) THEN
        MB = AB_ILAENV( 1, 'AB_SGEQR ', ' ', M, N, 1, -1 )
        NB = AB_ILAENV( 1, 'AB_SGEQR ', ' ', M, N, 2, -1 )
      ELSE
        MB = M
        NB = 1
      END IF
...
*
      IF( ( M.LE.N ) .OR. ( MB.LE.N ) .OR. ( MB.GE.M ) ) THEN
        CALL AB_SGEQRT( M, N, NB, A, LDA, T( 6 ), NB, WORK, INFO )
      ELSE
        CALL AB_SLATSQR( M, N, MB, NB, A, LDA, T( 6 ), NB, WORK,
     $                LWORK, INFO )
      END IF
*
      WORK( 1 ) = MAX( 1, NB*N )
