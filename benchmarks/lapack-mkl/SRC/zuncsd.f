...
*
*> \htmlonly
*> Download AB_zuncsd + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_zuncsd.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_zuncsd.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_zuncsd.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       RECURSIVE SUBROUTINE AB_zuncsd( JOBU1, JOBU2, JOBV1T, JOBV2T, TRANS,
...
*> \verbatim
*>
*> AB_zuncsd computes the CS decomposition of an M-by-M partitioned
*> unitary matrix X:
*>
*>                                 [  I  0  0 |  0  0  0 ]
*>                                 [  0  C  0 |  0 -S  0 ]
...
*>          > 0:  AB_ZBBCSD did not converge. See the description of RWORK
*>                above for details.
*> \endverbatim
*
*> \par References:
...
*
*  =====================================================================
      RECURSIVE SUBROUTINE AB_zuncsd( JOBU1, JOBU2, JOBV1T, JOBV2T, TRAN
     $S,
     $                             SIGNS, M, P, Q, X11, LDX11, X12,
     $                             LDX12, X21, LDX21, X22, LDX22, THETA,
     $                             U1, LDU1, U2, LDU2, V1T, LDV1T, V2T,
...
*     .. External Subroutines ..
      EXTERNAL           AB_XERBLA, AB_ZBBCSD, AB_ZLACPY, AB_ZLAPMR, AB_
     $ZLAPMT,
     $                   AB_ZUNBDB, AB_ZUNGLQ, AB_ZUNGQR
*     ..
*     .. External Functions ..
      LOGICAL            AB_LSAME
      EXTERNAL           AB_LSAME
...
*
      IF( INFO .EQ. 0 .AND. MIN( P, M-P ) .LT. MIN( Q, M-Q ) ) THEN
         ...
         ELSE
            SIGNST = 'D'
         END IF
         CALL AB_zuncsd( JOBV1T, JOBV2T, JOBU1, JOBU2, TRANST, SIGNST, M
     $,
...
*
      IF( INFO .EQ. 0 .AND. M-Q .LT. Q ) THEN
         ...
         ELSE
            SIGNST = 'D'
         END IF
         CALL AB_zuncsd( JOBU2, JOBU1, JOBV2T, JOBV1T, TRANS, SIGNST, M,
...
*
         ...
         CALL AB_ZBBCSD( JOBU1, JOBU2, JOBV1T, JOBV2T, TRANS, M, P, Q,
...
*
         ...
         CALL AB_ZUNGQR( M-Q, M-Q, M-Q, U1, MAX(1,M-Q), U1, WORK, -1,
     $                CHILDINFO )
         LORGQRWORKOPT = INT( WORK(1) )
         LORGQRWORKMIN = MAX( 1, M - Q )
         IORGLQ = ITAUQ2 + MAX( 1, M - Q )
         CALL AB_ZUNGLQ( M-Q, M-Q, M-Q, U1, MAX(1,M-Q), U1, WORK, -1,
     $                CHILDINFO )
         LORGLQWORKOPT = INT( WORK(1) )
         LORGLQWORKMIN = MAX( 1, M - Q )
         IORBDB = ITAUQ2 + MAX( 1, M - Q )
         CALL AB_ZUNBDB( TRANS, SIGNS, M, P, Q, X11, LDX11, X12, LDX12,
...
*
      CALL AB_ZUNBDB( TRANS, SIGNS, M, P, Q, X11, LDX11, X12, LDX12, X21
     $,
...
*
     ...
     $                   WORK(IORGQR), LORGQRWORK, INFO )
         ...
         IF( WANTV1T .AND. Q .GT. 0 ) THEN
            CALL AB_ZLACPY( 'U', Q-1, Q-1, X11(1,2), LDX11, V1T(2,2),
     $                   LDV1T )
            V1T(1, 1) = ONE
            DO J = 2, Q
               V1T(1,J) = ZERO
     ...
     $                   WORK(IORGLQ), LORGLQWORK, INFO )
         ...
         IF( WANTV2T .AND. M-Q .GT. 0 ) THEN
            CALL AB_ZLACPY( 'U', P, M-Q, X12, LDX12, V2T, LDV2T )
            IF( M-P .GT. Q) THEN
               CALL AB_ZLACPY( 'U', M-P-Q, M-P-Q, X22(Q+1,P+1), LDX22,
     $                      V2T(P+1,P+1), LDV2T )
            END IF
            IF( M .GT. Q ) THEN
               CALL AB_ZUNGLQ( M-Q, M-Q, M-Q, V2T, LDV2T, WORK(ITAUQ2),
     $                      WORK(IORGLQ), LORGLQWORK, INFO )
            END IF
         END IF
      ELSE
         IF( WANTU1 .AND. P .GT. 0 ) THEN
            CALL AB_ZLACPY( 'U', Q, P, X11, LDX11, U1, LDU1 )
            CALL AB_ZUNGLQ( P, P, Q, U1, LDU1, WORK(ITAUP1), WORK(IORGLQ
     $),
     $                   LORGLQWORK, INFO)
         END IF
         IF( WANTU2 .AND. M-P .GT. 0 ) THEN
            CALL AB_ZLACPY( 'U', Q, M-P, X21, LDX21, U2, LDU2 )
            CALL AB_ZUNGLQ( M-P, M-P, Q, U2, LDU2, WORK(ITAUP2),
     $                   WORK(IORGLQ), LORGLQWORK, INFO )
         END IF
         IF( WANTV1T .AND. Q .GT. 0 ) THEN
            CALL AB_ZLACPY( 'L', Q-1, Q-1, X11(2,1), LDX11, V1T(2,2),
     $                   LDV1T )
            V1T(1, 1) = ONE
            DO J = 2, Q
               V1T(1,J) = ZERO
               V1T(J,1) = ZERO
            END DO
            CALL AB_ZUNGQR( Q-1, Q-1, Q-1, V1T(2,2), LDV1T, WORK(ITAUQ1)
     $,
     $                   WORK(IORGQR), LORGQRWORK, INFO )
         END IF
         IF( WANTV2T .AND. M-Q .GT. 0 ) THEN
            P1 = MIN( P+1, M )
            Q1 = MIN( Q+1, M )
            CALL AB_ZLACPY( 'L', M-Q, P, X12, LDX12, V2T, LDV2T )
            IF( M .GT. P+Q ) THEN
               CALL AB_ZLACPY( 'L', M-P-Q, M-P-Q, X22(P1,Q1), LDX22,
     $                      V2T(P+1,P+1), LDV2T )
            END IF
            CALL AB_ZUNGQR( M-Q, M-Q, M-Q, V2T, LDV2T, WORK(ITAUQ2),
     $                   WORK(IORGQR), LORGQRWORK, INFO )
         END IF
      END IF
*
...
*
      CALL AB_ZBBCSD( JOBU1, JOBU2, JOBV1T, JOBV2T, TRANS, M, P, Q, THET
     $A,
...
*
      IF( Q .GT. 0 .AND. WANTU2 ) THEN
         ...
         ELSE
            CALL AB_ZLAPMR( .FALSE., M-P, M-P, U2, LDU2, IWORK )
         END IF
      END IF
      IF( M .GT. 0 .AND. WANTV2T ) THEN
         ...
         IF( .NOT. COLMAJOR ) THEN
            CALL AB_ZLAPMT( .FALSE., M-Q, M-Q, V2T, LDV2T, IWORK )
         ELSE
            CALL AB_ZLAPMR( .FALSE., M-Q, M-Q, V2T, LDV2T, IWORK )
         END IF
      END IF
*
      RETURN
