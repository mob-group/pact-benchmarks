...
*     .. External Subroutines ..
      EXTERNAL           AB_CGEQRF, AB_CLAQP2, AB_CLAQPS, AB_CSWAP, AB_C
     $UNMQR, AB_XERBLA
*     ..
*     .. External Functions ..
      INTEGER            AB_ILAENV
...
*
      IF( INFO.EQ.0 ) THEN
         ...
         ELSE
            IWS = N + 1
            NB = AB_ILAENV( INB, 'AB_CGEQRF', ' ', M, N, -1, -1 )
            LWKOPT = ( N + 1 )*NB
         END IF
         WORK( 1 ) = CMPLX( LWKOPT )
*
...
*
      IF( NFXD.GT.0 ) THEN
         NA = MIN( M, NFXD )
*CC      CALL AB_CGEQR2( M, NA, A, LDA, TAU, WORK, INFO )
         CALL AB_CGEQRF( M, NA, A, LDA, TAU, WORK, LWORK, INFO )
...
*
         NB = AB_ILAENV( INB, 'AB_CGEQRF', ' ', SM, SN, -1, -1 )
         NBMIN = 2
         NX = 0
*
...
*
            NX = MAX( 0, AB_ILAENV( IXOVER, 'AB_CGEQRF', ' ', SM, SN, -1
     $,
     $           -1 ) )
*
*
...
*
                  NB = LWORK / ( SN+1 )
                  NBMIN = MAX( 2, AB_ILAENV( INBMIN, 'AB_CGEQRF', ' ', S
     $M, SN,
     $                    -1, -1 ) )
*
*
