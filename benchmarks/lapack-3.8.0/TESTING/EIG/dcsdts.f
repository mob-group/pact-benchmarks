...
*  ===========
*
*       SUBROUTINE AB_DCSDTS( M, P, Q, X, XF, LDX, U1, LDU1, U2, LDU2, V1T,
...
*> \verbatim
*>
*> AB_DCSDTS tests DORCSD, which, given an M-by-M partitioned orthogonal
*> matrix X,
*>              Q  M-Q
*>       X = [ X11 X12 ] P   ,
*>           [ X21 X22 ] M-P
...
*     .. External Subroutines ..
      EXTERNAL           AB_DGEMM, AB_DLACPY, AB_DLASET, DORCSD, AB_DORC
     $SD2BY1,
     $                   AB_AB_DSYRK
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL AB_DLASET( 'Full', M, M, ZERO, ONE, WORK, LDX )
      CALL AB_AB_DSYRK( 'Upper', 'Conjugate transpose', M, M, -ONE, X, L
     $DX,
     $            ONE, WORK, LDX )
      IF (M.GT.0) THEN
         EPS2 = MAX( ULP,
     $               AB_DLANGE( '1', M, M, WORK, LDX, RWORK ) / DBLE( M 
     $) )
      ELSE
         EPS2 = ULP
      END IF
...
*
      CALL AB_DLACPY( 'Full', M, M, X, LDX, XF, LDX )
*
*     Compute the CSD
*
...
*
      CALL AB_DLACPY( 'Full', M, M, X, LDX, XF, LDX )
*
      CALL AB_DGEMM( 'No transpose', 'Conjugate transpose', P, Q, Q, ONE
     $,
     $            XF, LDX, V1T, LDV1T, ZERO, WORK, LDX )
*
      CALL AB_DGEMM( 'Conjugate transpose', 'No transpose', P, Q, P, ONE
     $,
...
*
     ...
     $           XF(MIN(P,Q)-R+I,MIN(P,Q)-R+I) - COS(THETA(I))
      END DO
*
      CALL AB_DGEMM( 'No transpose', 'Conjugate transpose', P, M-Q, M-Q,
     $            ONE, XF(1,Q+1), LDX, V2T, LDV2T, ZERO, WORK, LDX )
*
      CALL AB_DGEMM( 'Conjugate transpose', 'No transpose', P, M-Q, P,
...
*
     ...
     $      SIN(THETA(R-I+1))
      END DO
*
      CALL AB_DGEMM( 'No transpose', 'Conjugate transpose', M-P, Q, Q, O
     $NE,
     $            XF(P+1,1), LDX, V1T, LDV1T, ZERO, WORK, LDX )
*
      CALL AB_DGEMM( 'Conjugate transpose', 'No transpose', M-P, Q, M-P,
...
*
     ...
     $             SIN(THETA(R-I+1))
      END DO
*
      CALL AB_DGEMM( 'No transpose', 'Conjugate transpose', M-P, M-Q, M-
     $Q,
     $            ONE, XF(P+1,Q+1), LDX, V2T, LDV2T, ZERO, WORK, LDX )
*
      CALL AB_DGEMM( 'Conjugate transpose', 'No transpose', M-P, M-Q, M-
     $P,
...
*
      RESID = AB_DLANGE( '1', P, Q, XF, LDX, RWORK )
      RESULT( 1 ) = ( RESID / DBLE(MAX(1,P,Q)) ) / EPS2
*
*     Compute norm( U1'*X12*V2 - D12 ) / ( MAX(1,P,M-Q)*EPS2 ) .
*
      RESID = AB_DLANGE( '1', P, M-Q, XF(1,Q+1), LDX, RWORK )
      RESULT( 2 ) = ( RESID / DBLE(MAX(1,P,M-Q)) ) / EPS2
*
*     Compute norm( U2'*X21*V1 - D21 ) / ( MAX(1,M-P,Q)*EPS2 ) .
*
      RESID = AB_DLANGE( '1', M-P, Q, XF(P+1,1), LDX, RWORK )
      RESULT( 3 ) = ( RESID / DBLE(MAX(1,M-P,Q)) ) / EPS2
*
*     Compute norm( U2'*X22*V2 - D22 ) / ( MAX(1,M-P,M-Q)*EPS2 ) .
*
      RESID = AB_DLANGE( '1', M-P, M-Q, XF(P+1,Q+1), LDX, RWORK )
      RESULT( 4 ) = ( RESID / DBLE(MAX(1,M-P,M-Q)) ) / EPS2
*
*     Compute I - U1'*U1
*
      CALL AB_DLASET( 'Full', P, P, ZERO, ONE, WORK, LDU1 )
      CALL AB_AB_DSYRK( 'Upper', 'Conjugate transpose', P, P, -ONE, U1, 
     $LDU1,
     $            ONE, WORK, LDU1 )
*
*     Compute norm( I - U'*U ) / ( MAX(1,P) * ULP ) .
*
      RESID = AB_DLANSY( '1', 'Upper', P, WORK, LDU1, RWORK )
      RESULT( 5 ) = ( RESID / DBLE(MAX(1,P)) ) / ULP
*
*     Compute I - U2'*U2
*
      CALL AB_DLASET( 'Full', M-P, M-P, ZERO, ONE, WORK, LDU2 )
      CALL AB_AB_DSYRK( 'Upper', 'Conjugate transpose', M-P, M-P, -ONE, 
     $U2,
     $            LDU2, ONE, WORK, LDU2 )
*
*     Compute norm( I - U2'*U2 ) / ( MAX(1,M-P) * ULP ) .
*
      RESID = AB_DLANSY( '1', 'Upper', M-P, WORK, LDU2, RWORK )
      RESULT( 6 ) = ( RESID / DBLE(MAX(1,M-P)) ) / ULP
*
*     Compute I - V1T*V1T'
*
      CALL AB_DLASET( 'Full', Q, Q, ZERO, ONE, WORK, LDV1T )
      CALL AB_AB_DSYRK( 'Upper', 'No transpose', Q, Q, -ONE, V1T, LDV1T,
     $ ONE,
     $            WORK, LDV1T )
*
*     Compute norm( I - V1T*V1T' ) / ( MAX(1,Q) * ULP ) .
*
      RESID = AB_DLANSY( '1', 'Upper', Q, WORK, LDV1T, RWORK )
      RESULT( 7 ) = ( RESID / DBLE(MAX(1,Q)) ) / ULP
*
*     Compute I - V2T*V2T'
*
      CALL AB_DLASET( 'Full', M-Q, M-Q, ZERO, ONE, WORK, LDV2T )
      CALL AB_AB_DSYRK( 'Upper', 'No transpose', M-Q, M-Q, -ONE, V2T, LD
     $V2T,
     $            ONE, WORK, LDV2T )
*
*     Compute norm( I - V2T*V2T' ) / ( MAX(1,M-Q) * ULP ) .
*
      RESID = AB_DLANSY( '1', 'Upper', M-Q, WORK, LDV2T, RWORK )
...
*
      CALL AB_DLASET( 'Full', Q, Q, ZERO, ONE, WORK, LDX )
      CALL AB_AB_DSYRK( 'Upper', 'Conjugate transpose', Q, M, -ONE, X, L
     $DX,
     $            ONE, WORK, LDX )
      IF( M.GT.0 ) THEN
         EPS2 = MAX( ULP,
     $               AB_DLANGE( '1', Q, Q, WORK, LDX, RWORK ) / DBLE( M 
     $) )
      ELSE
         EPS2 = ULP
      END IF
...
*
      CALL AB_DLACPY( 'Full', M, Q, X, LDX, XF, LDX )
*
*     Compute the CSD
*
...
*     Compute norm( U1'*X11*V1 - D11 ) / ( MAX(1,P,Q)*EPS2 ) .
*
      RESID = AB_DLANGE( '1', P, Q, X, LDX, RWORK )
      RESULT( 10 ) = ( RESID / DBLE(MAX(1,P,Q)) ) / EPS2
*
*     Compute norm( U2'*X21*V1 - D21 ) / ( MAX(1,M-P,Q)*EPS2 ) .
*
      RESID = AB_DLANGE( '1', M-P, Q, X(P+1,1), LDX, RWORK )
