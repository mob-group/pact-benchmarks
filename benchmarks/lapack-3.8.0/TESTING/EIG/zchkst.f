...
*  ===========
*
*       SUBROUTINE AB_ZCHKST( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*> AB_ZCHKST  checks the Hermitian eigenvalue problem routines.
*>
...
*>    of A; AB_ZCHKST checks both cases.
...
*>
*> When AB_ZCHKST is called, a number of matrix "sizes" ("n's") and a
...
*>          AB_ZCHKST does nothing.  It must be at least zero.
*> \endverbatim
*>
*> \param[in] NN
*> \verbatim
...
*> \verbatim
*>          NTYPES is INTEGER
*>          The number of elements in DOTYPE.   If it is zero, AB_ZCHKST
...
*>          next call to AB_ZCHKST to continue the same random number
*>          sequence.
*> \endverbatim
*>
*> \param[in] THRESH
...
*
            NTEST = 3
            CALL AB_ZHETRD( 'L', N, V, LDU, SD, SE, TAU, WORK, LWORK,
     $                   IINFO )
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'AB_ZHETRD(L)', IINFO, N, JTYP
     $E,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
            ...
            END IF
*
            CALL AB_ZLACPY( 'L', N, N, V, LDU, U, LDU )
*
            NTEST = 4
            CALL AB_ZUNGTR( 'L', N, U, LDU, TAU, WORK, LWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'AB_ZUNGTR(L)', IINFO, N, JTYP
     $E,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
            ...
            END IF
*
            CALL AB_ZHET21( 2, 'Lower', N, 1, A, LDA, SD, SE, U, LDU, V,
     $                   LDU, TAU, WORK, RWORK, RESULT( 3 ) )
            CALL AB_ZHET21( 3, 'Lower', N, 1, A, LDA, SD, SE, U, LDU, V,
...
*
      ...
      RETURN
*
 9999 FORMAT( ' AB_ZCHKST: ', A, ' returned INFO=', I6, '.', / 9X, 'N=',
     $      I6, ', JTYPE=', I6, ', ISEED=(', 3( I5, ',' ), I5, ')' )
*
 9998 FORMAT( / 1X, A3, ' -- Complex Hermitian eigenvalue problem' )
 9997 FORMAT( ' Matrix types (see AB_ZCHKST for details): ' )
*
...
*
 9987 FORMAT( / 'Test performed:  see AB_ZCHKST for details.', / )
*     End of AB_ZCHKST
*
      END
