...
*  ===========
*
*       SUBROUTINE AB_SDRGEV( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*> AB_SDRGEV checks the nonsymmetric generalized eigenvalue problem driver
*> routine AB_SGGEV.
*>
*> AB_SGGEV computes for a pair of n-by-n nonsymmetric matrices (A,B) the
...
*>          The (left) eigenvectors matrix computed by AB_SGGEV.
*> \endverbatim
*>
*> \param[in] LDQ
*> \verbatim
...
*> \verbatim
*>          The generalized eigenvalues of (A,B) computed by AB_SGGEV.
...
*>          eigenvalues of A and B, but those computed when AB_SGGEV only
...
*     .. External Subroutines ..
      EXTERNAL           AB_ALASVM, AB_SGET52, AB_SGGEV, AB_SLABAD, AB_S
     $LACPY, AB_AB_SLARFG,
...
*
  ...
  110       CONTINUE
*
*           Call AB_SGGEV to compute eigenvalues and eigenvectors.
*
            CALL AB_SLACPY( ' ', N, N, A, LDA, S, LDA )
            CALL AB_SLACPY( ' ', N, N, B, LDA, T, LDA )
            CALL AB_SGGEV( 'V', 'V', N, S, LDA, T, LDA, ALPHAR, ALPHAI,
     $                  BETA, Q, LDQ, Z, LDQ, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'AB_SGGEV1', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
     ...
     $                   ALPHAI, BETA, WORK, RESULT( 1 ) )
            IF( RESULT( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'AB_SGGEV1',
...
*
     ...
     $                   ALPHAI, BETA, WORK, RESULT( 3 ) )
            IF( RESULT( 4 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'AB_SGGEV1',
...
*
            CALL AB_SLACPY( ' ', N, N, A, LDA, S, LDA )
            CALL AB_SLACPY( ' ', N, N, B, LDA, T, LDA )
            CALL AB_SGGEV( 'N', 'N', N, S, LDA, T, LDA, ALPHR1, ALPHI1,
     $                  BETA1, Q, LDQ, Z, LDQ, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'AB_SGGEV2', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
            CALL AB_SLACPY( ' ', N, N, A, LDA, S, LDA )
            CALL AB_SLACPY( ' ', N, N, B, LDA, T, LDA )
            CALL AB_SGGEV( 'V', 'N', N, S, LDA, T, LDA, ALPHR1, ALPHI1,
     $                  BETA1, QE, LDQE, Z, LDQ, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'AB_AB_SGGEV3', IERR, N, JTYPE
     $,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
...
*
            CALL AB_SLACPY( ' ', N, N, A, LDA, S, LDA )
            CALL AB_SLACPY( ' ', N, N, B, LDA, T, LDA )
            CALL AB_SGGEV( 'N', 'V', N, S, LDA, T, LDA, ALPHR1, ALPHI1,
     $                  BETA1, Q, LDQ, QE, LDQE, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'AB_SGGEV4', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
      RETURN
*
 9999 FORMAT( ' AB_SDRGEV: ', A, ' returned INFO=', I6, '.', / 3X, 'N=',
     $      I6, ', JTYPE=', I6, ', ISEED=(', 4( I4, ',' ), I5, ')' )
*
 9998 FORMAT( ' AB_SDRGEV: ', A, ' Eigenvectors from ', A, ' incorrectly
     $ ',
...
*
 9997 FORMAT( / 1X, A3, ' -- Real Generalized eigenvalue problem driver'
     $       )
*
 9996 FORMAT( ' Matrix types (see AB_SDRGEV for details): ' )
*
