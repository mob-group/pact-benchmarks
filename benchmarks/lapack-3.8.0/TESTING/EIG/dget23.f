...
*  ===========
*
*       SUBROUTINE AB_DGET23( COMP, BALANC, JTYPE, THRESH, ISEED, NOUNIT, N,
...
*> \verbatim
*>
*>    AB_DGET23  checks the nonsymmetric eigenvalue problem driver AB_AB_SGEEVX.
...
*>      computed by AB_AB_DGEEVX and RCDVIN (the precomputed true value)
...
*>      computed by AB_AB_DGEEVX and RCDEIN (the precomputed true value)
...
*>          Another copy of the test matrix A, modified by AB_AB_DGEEVX.
*> \endverbatim
*>
*> \param[out] WR
*> \verbatim
...
*>          but those computed when AB_AB_DGEEVX only computes a partial
...
*>          If >0, AB_AB_DGEEVX returned an error code, the absolute
...
*
*  =====================================================================
      SUBROUTINE AB_DGET23( COMP, BALANC, JTYPE, THRESH, ISEED, NOUNIT, 
     $N,
     $                   A, LDA, H, WR, WI, WR1, WI1, VL, LDVL, VR,
     $                   LDVR, LRE, LDLRE, RCONDV, RCNDV1, RCDVIN,
     $                   RCONDE, RCNDE1, RCDEIN, SCALE, SCALE1, RESULT,
...
*     .. External Functions ..
      ...
      EXTERNAL           AB_LSAME, AB_DLAMCH, AB_DLAPY2, AB_DNRM2
*     ..
*     .. External Subroutines ..
      EXTERNAL           AB_AB_DGEEVX, AB_DGET22, AB_DLACPY, AB_XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      CALL AB_AB_DGEEVX( BALANC, 'V', 'V', SENSE, N, H, LDA, WR, WI, VL,
     $ LDVL,
     ...
     $             WORK, LWORK, IWORK, IINFO )
      IF( IINFO.NE.0 ) THEN
         RESULT( 1 ) = ULPINV
         IF( JTYPE.NE.22 ) THEN
            WRITE( NOUNIT, FMT = 9998 )'AB_AB_DGEEVX1', IINFO, N, JTYPE,
     $         BALANC, ISEED
         ELSE
            WRITE( NOUNIT, FMT = 9999 )'AB_AB_DGEEVX1', IINFO, N, ISEED(
     $ 1 )
         END IF
         INFO = ABS( IINFO )
         RETURN
...
*     Do Test (1)
*
      CALL AB_DGET22( 'N', 'N', 'N', N, A, LDA, VR, LDVR, WR, WI, WORK,
     $             RES )
      RESULT( 1 ) = RES( 1 )
*
*     Do Test (2)
*
      CALL AB_DGET22( 'T', 'N', 'T', N, A, LDA, VL, LDVL, WR, WI, WORK,
     $             RES )
      RESULT( 2 ) = RES( 1 )
*
*     Do Test (3)
*
      DO 30 J = 1, N
         ...
         IF( WI( J ).EQ.ZERO ) THEN
            TNRM = AB_DNRM2( N, VR( 1, J ), 1 )
         ELSE IF( WI( J ).GT.ZERO ) THEN
            TNRM = AB_DLAPY2( AB_DNRM2( N, VR( 1, J ), 1 ),
     $             AB_DNRM2( N, VR( 1, J+1 ), 1 ) )
         END IF
     ...
     $                 MIN( ULPINV, ABS( TNRM-ONE ) / ULP ) )
         IF( WI( J ).GT.ZERO ) THEN
            ...
            VRMX = ZERO
            DO 20 JJ = 1, N
               VTST = AB_DLAPY2( VR( JJ, J ), VR( JJ, J+1 ) )
               IF( VTST.GT.VMX )
     $            VMX = VTST
...
*
      DO 50 J = 1, N
         TNRM = ONE
         IF( WI( J ).EQ.ZERO ) THEN
            TNRM = AB_DNRM2( N, VL( 1, J ), 1 )
         ELSE IF( WI( J ).GT.ZERO ) THEN
            TNRM = AB_DLAPY2( AB_DNRM2( N, VL( 1, J ), 1 ),
     $             AB_DNRM2( N, VL( 1, J+1 ), 1 ) )
         END IF
     ...
     $                 MIN( ULPINV, ABS( TNRM-ONE ) / ULP ) )
         IF( WI( J ).GT.ZERO ) THEN
            ...
            VRMX = ZERO
            DO 40 JJ = 1, N
               VTST = AB_DLAPY2( VL( JJ, J ), VL( JJ, J+1 ) )
               IF( VTST.GT.VMX )
     $            VMX = VTST
...
*
         CALL AB_DLACPY( 'F', N, N, A, LDA, H, LDA )
         CALL AB_AB_DGEEVX( BALANC, 'N', 'N', SENSE, N, H, LDA, WR1, WI1
     $, DUM,
     ...
     $                RCNDV1, WORK, LWORK, IWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 1 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'AB_AB_DGEEVX2', IINFO, N, JTY
     $PE,
     $            BALANC, ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'AB_AB_DGEEVX2', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 190
...
*
         CALL AB_DLACPY( 'F', N, N, A, LDA, H, LDA )
         CALL AB_AB_DGEEVX( BALANC, 'N', 'V', SENSE, N, H, LDA, WR1, WI1
     $, DUM,
     ...
     $                RCNDV1, WORK, LWORK, IWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 1 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'AB_AB_DGEEVX3', IINFO, N, JTY
     $PE,
     $            BALANC, ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'AB_AB_DGEEVX3', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 190
...
*
         CALL AB_DLACPY( 'F', N, N, A, LDA, H, LDA )
         CALL AB_AB_DGEEVX( BALANC, 'V', 'N', SENSE, N, H, LDA, WR1, WI1
     $, LRE,
     ...
     $                RCNDV1, WORK, LWORK, IWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 1 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'AB_AB_DGEEVX4', IINFO, N, JTY
     $PE,
     $            BALANC, ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'AB_AB_DGEEVX4', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 190
...
*
      IF( COMP ) THEN
         CALL AB_DLACPY( 'F', N, N, A, LDA, H, LDA )
         CALL AB_AB_DGEEVX( 'N', 'V', 'V', 'B', N, H, LDA, WR, WI, VL, L
     $DVL,
     $                VR, LDVR, ILO, IHI, SCALE, ABNRM, RCONDE, RCONDV,
     $                WORK, LWORK, IWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 1 ) = ULPINV
            WRITE( NOUNIT, FMT = 9999 )'AB_AB_DGEEVX5', IINFO, N, ISEED(
     $ 1 )
            INFO = ABS( IINFO )
            GO TO 250
         END IF
...
*
      END IF
*
 9999 FORMAT( ' AB_DGET23: ', A, ' returned INFO=', I6, '.', / 9X, 'N=',
     $      I6, ', INPUT EXAMPLE NUMBER = ', I4 )
 9998 FORMAT( ' AB_DGET23: ', A, ' returned INFO=', I6, '.', / 9X, 'N=',
