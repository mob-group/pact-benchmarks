...
*  ===========
*
*       SUBROUTINE AB_AB_SDRGES3( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*> AB_AB_SDRGES3 checks the nonsymmetric generalized eigenvalue (Schur form)
*> problem driver AB_AB_SGGES3.
*>
*> AB_AB_SGGES3 factors A and B as Q S Z'  and Q T Z' , where ' means
...
*> Schur forms.
*>
*> When AB_AB_SDRGES3 is called, a number of matrix "sizes" ("N's") and a
...
*>          AB_AB_SDRGES3 does nothing.  NSIZES >= 0.
*> \endverbatim
*>
*> \param[in] NN
*> \verbatim
...
*> \verbatim
*>          NTYPES is INTEGER
*>          The number of elements in DOTYPE.   If it is zero, AB_AB_SDRGES3
...
*>          next call to AB_AB_SDRGES3 to continue the same random number
*>          sequence.
*> \endverbatim
*>
*> \param[in] THRESH
...
*>          The Schur form matrix computed from A by AB_AB_SGGES3.  On exit, S
...
*>          The upper triangular matrix computed from B by AB_AB_SGGES3.
*> \endverbatim
*>
*> \param[out] Q
*> \verbatim
*>          Q is REAL array, dimension (LDQ, max(NN))
*>          The (left) orthogonal matrix computed by AB_AB_SGGES3.
*> \endverbatim
*>
*> \param[in] LDQ
*> \verbatim
...
*>          The (right) orthogonal matrix computed by AB_AB_SGGES3.
*> \endverbatim
*>
*> \param[out] ALPHAR
*> \verbatim
...
*>
*>          The generalized eigenvalues of (A,B) computed by AB_AB_SGGES3.
...
*
*  =====================================================================
      SUBROUTINE AB_AB_SDRGES3( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRES
     $H,
     $                    NOUNIT, A, LDA, B, S, T, Q, LDQ, Z, ALPHAR,
     $                    ALPHAI, BETA, WORK, LWORK, RESULT, BWORK,
     $                    INFO )
...
*     .. External Functions ..
      ...
      EXTERNAL           AB_SLCTES, AB_ILAENV, AB_SLAMCH, AB_SLARND
*     ..
*     .. External Subroutines ..
      EXTERNAL           AB_ALASVM, AB_SGET51, AB_SGET53, AB_SGET54, AB_
     $AB_SGGES3, AB_SLABAD,
     $                   AB_SLACPY, AB_AB_SLARFG, AB_SLASET, AB_SLATM4, 
     $AB_SORM2R, AB_XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      IF( INFO.EQ.0 .AND. LWORK.GE.1 ) THEN
         ...
         NB = MAX( 1, AB_ILAENV( 1, 'AB_AB_SGEQRF', ' ', NMAX, NMAX, -1,
     $ -1 ),
     $        AB_ILAENV( 1, 'AB_SORMQR', 'LT', NMAX, NMAX, NMAX, -1 ),
     $        AB_ILAENV( 1, 'AB_SORGQR', ' ', NMAX, NMAX, NMAX, -1 ) )
...
*
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_SDRGES3', -INFO )
         RETURN
      END IF
*
...
*
                  DO 50 JC = 1, N - 1
                     DO 40 JR = JC, N
                        Q( JR, JC ) = AB_SLARND( 3, ISEED )
                        Z( JR, JC ) = AB_SLARND( 3, ISEED )
   40                CONTINUE
                     CALL AB_AB_SLARFG( N+1-JC, Q( JC, JC ), Q( JC+1, JC
     $ ), 1,
     $                            WORK( JC ) )
                     WORK( 2*N+JC ) = SIGN( ONE, Q( JC, JC ) )
                     Q( JC, JC ) = ONE
                     CALL AB_AB_SLARFG( N+1-JC, Z( JC, JC ), Z( JC+1, JC
     $ ), 1,
     $                            WORK( N+JC ) )
...
*
   ...
   60                CONTINUE
   70             CONTINUE
                  CALL AB_SORM2R( 'L', 'N', N, N, N-1, Q, LDQ, WORK, A,
     $                         LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                  CALL AB_SORM2R( 'R', 'T', N, N, N-1, Z, LDQ, WORK( N+1
     $ ),
     $                         A, LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                  CALL AB_SORM2R( 'L', 'N', N, N, N-1, Q, LDQ, WORK, B,
     $                         LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                  CALL AB_SORM2R( 'R', 'T', N, N, N-1, Z, LDQ, WORK( N+1
...
*
            DO 150 ISORT = 0, 1
               ...
               END IF
*
*              Call AB_AB_SGGES3 to compute H, T, Q, Z, alpha, and beta.
*
               CALL AB_SLACPY( 'Full', N, N, A, LDA, S, LDA )
               CALL AB_SLACPY( 'Full', N, N, B, LDA, T, LDA )
               NTEST = 1 + RSUB + ISORT
               RESULT( 1+RSUB+ISORT ) = ULPINV
               CALL AB_AB_SGGES3( 'V', 'V', SORT, AB_SLCTES, N, S, LDA, 
     $T, LDA,
     $                      SDIM, ALPHAR, ALPHAI, BETA, Q, LDQ, Z, LDQ,
     $                      WORK, LWORK, BWORK, IINFO )
               IF( IINFO.NE.0 .AND. IINFO.NE.N+2 ) THEN
                  RESULT( 1+RSUB+ISORT ) = ULPINV
                  WRITE( NOUNIT, FMT = 9999 )'AB_AB_SGGES3', IINFO, N, J
     $TYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 160
...
*
               IF( ISORT.EQ.0 ) THEN
                  CALL AB_SGET51( 1, N, A, LDA, S, LDA, Q, LDQ, Z, LDQ,
     $                         WORK, RESULT( 1 ) )
                  CALL AB_SGET51( 1, N, B, LDA, T, LDA, Q, LDQ, Z, LDQ,
     $                         WORK, RESULT( 2 ) )
               ELSE
                  CALL AB_SGET54( N, A, LDA, B, LDA, S, LDA, T, LDA, Q,
     $                         LDQ, Z, LDQ, WORK, RESULT( 7 ) )
               END IF
               CALL AB_SGET51( 3, N, A, LDA, T, LDA, Q, LDQ, Q, LDQ, WOR
     $K,
     $                      RESULT( 3+RSUB ) )
               CALL AB_SGET51( 3, N, B, LDA, T, LDA, Z, LDQ, Z, LDQ, WOR
     $K,
     $                      RESULT( 4+RSUB ) )
*
...
*
                  ELSE
                     ...
                     END IF
                     IF( .NOT.ILABAD ) THEN
                        CALL AB_SGET53( S( I1, I1 ), LDA, T( I1, I1 ), L
     $DA,
     $                               BETA( J ), ALPHAR( J ),
     $                               ALPHAI( J ), TEMP2, IERR )
                        IF( IERR.GE.3 ) THEN
...
*
                  ...
                  KNTEIG = 0
                  DO 140 I = 1, N
                     IF( AB_SLCTES( ALPHAR( I ), ALPHAI( I ),
     $                   BETA( I ) ) .OR. AB_SLCTES( ALPHAR( I ),
     $                   -ALPHAI( I ), BETA( I ) ) ) THEN
                        KNTEIG = KNTEIG + 1
                     END IF
                     IF( I.LT.N ) THEN
                        IF( ( AB_SLCTES( ALPHAR( I+1 ), ALPHAI( I+1 ),
     $                      BETA( I+1 ) ) .OR. AB_SLCTES( ALPHAR( I+1 ),
     $                      -ALPHAI( I+1 ), BETA( I+1 ) ) ) .AND.
     $                      ( .NOT.( AB_SLCTES( ALPHAR( I ), ALPHAI( I )
     $,
     $                      BETA( I ) ) .OR. AB_SLCTES( ALPHAR( I ),
     $                      -ALPHAI( I ), BETA( I ) ) ) ) .AND.
     $                      IINFO.NE.N+2 ) THEN
...
*     Summary
*
      CALL AB_ALASVM( 'SGS', NOUNIT, NERRS, NTESTT, 0 )
*
      WORK( 1 ) = MAXWRK
*
      RETURN
*
 9999 FORMAT( ' AB_AB_SDRGES3: ', A, ' returned INFO=', I6, '.', / 9X, '
     $N=',
     $      I6, ', JTYPE=', I6, ', ISEED=(', 4( I4, ',' ), I5, ')' )
*
 9998 FORMAT( ' AB_AB_SDRGES3: AB_SGET53 returned INFO=', I1, ' for eige
     $nvalue ',
     $      I6, '.', / 9X, 'N=', I6, ', JTYPE=', I6, ', ISEED=(',
     $      4( I4, ',' ), I5, ')' )
*
 9997 FORMAT( ' AB_AB_SDRGES3: S not in Schur form at eigenvalue ', I6, 
     $'.',
...
*
 9995 FORMAT( ' Matrix types (see AB_AB_SDRGES3 for details): ' )
*
