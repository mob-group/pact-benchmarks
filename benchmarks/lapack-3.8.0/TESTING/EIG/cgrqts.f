...
*  ===========
*
*       SUBROUTINE AB_CGRQTS( M, P, N, A, AF, Q, R, LDA, TAUA, B, BF, Z, T,
...
*> \verbatim
*>
*> AB_CGRQTS tests AB_CGGRQF, which computes the GRQ factorization of an
...
*>          by AB_CGGRQF, see AB_CGGRQF for further details.
*> \endverbatim
*>
*> \param[out] Q
*> \verbatim
...
*     .. External Subroutines ..
      EXTERNAL           AB_CGEMM, AB_CGGRQF, AB_CLACPY, AB_CLASET, AB_C
     $UNGQR,
...
*
      CALL AB_CGGRQF( M, P, N, AF, LDA, TAUA, BF, LDB, TAUB, WORK,
     $             LWORK, INFO )
*
...
*
     ...
     $                   Q( 2, 1 ), LDA )
      END IF
      CALL AB_CUNGRQ( N, N, MIN( M, N ), Q, LDA, TAUA, WORK, LWORK, INFO
     $ )
*
...
*
      ...
      IF( P.GT.1 )
     $   CALL AB_CLACPY( 'Lower', P-1, N, BF( 2,1 ), LDB, Z( 2,1 ), LDB 
     $)
      CALL AB_CUNGQR( P, P, MIN( P,N ), Z, LDB, TAUB, WORK, LWORK, INFO 
     $)
*
*     Copy R
*
      CALL AB_CLASET( 'Full', M, N, CZERO, CZERO, R, LDA )
      IF( M.LE.N )THEN
         CALL AB_CLACPY( 'Upper', M, M, AF( 1, N-M+1 ), LDA, R( 1, N-M+1
     $ ),
     $                LDA )
      ELSE
         CALL AB_CLACPY( 'Full', M-N, N, AF, LDA, R, LDA )
         CALL AB_CLACPY( 'Upper', N, N, AF( M-N+1, 1 ), LDA, R( M-N+1, 1
     $ ),
     $                LDA )
      END IF
*
*     Copy T
*
      CALL AB_CLASET( 'Full', P, N, CZERO, CZERO, T, LDB )
      CALL AB_CLACPY( 'Upper', P, N, BF, LDB, T, LDB )
*
*     Compute R - A*Q'
*
      CALL AB_CGEMM( 'No transpose', 'Conjugate transpose', M, N, N, -CO
     $NE,
...
*
     ...
     $           Z, LDB, B, LDB, CZERO, BWK, LDB )
      CALL AB_CGEMM( 'No transpose', 'No transpose', P, N, N, CONE, T, L
     $DB,
     $            Q, LDA, -CONE, BWK, LDB )
*
*     Compute norm( T*Q - Z'*B ) / ( MAX(P,N)*norm(A)*ULP ) .
*
      RESID = AB_CLANGE( '1', P, N, BWK, LDB, RWORK )
