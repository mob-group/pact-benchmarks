...
*  ===========
*
*       SUBROUTINE AB_AB_DDRGEV3( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*> AB_AB_DDRGEV3 checks the nonsymmetric generalized eigenvalue problem driver
*> routine AB_AB_DGGEV3.
*>
*> AB_AB_DGGEV3 computes for a pair of n-by-n nonsymmetric matrices (A,B) the
...
*>          AB_AB_DDRGEV3 does nothing.  NSIZES >= 0.
*> \endverbatim
*>
*> \param[in] NN
*> \verbatim
...
*> \verbatim
*>          NTYPES is INTEGER
*>          The number of elements in DOTYPE.   If it is zero, AB_AB_DDRGEV3
...
*>          next call to AB_AB_DDRGEV3 to continue the same random number
*>          sequence.
*> \endverbatim
*>
*> \param[in] THRESH
...
*>                                 dimension (LDA, max(NN))
*>          The Schur form matrix computed from A by AB_AB_DGGEV3.  On exit, S
...
*>                                 dimension (LDA, max(NN))
*>          The upper triangular matrix computed from B by AB_AB_DGGEV3.
*> \endverbatim
*>
*> \param[out] Q
*> \verbatim
...
*>          The (left) eigenvectors matrix computed by AB_AB_DGGEV3.
*> \endverbatim
*>
*> \param[in] LDQ
*> \verbatim
...
*>          The (right) orthogonal matrix computed by AB_AB_DGGEV3.
*> \endverbatim
*>
*> \param[out] QE
*> \verbatim
...
*>
*>          The generalized eigenvalues of (A,B) computed by AB_AB_DGGEV3.
...
*>          eigenvalues of A and B, but those computed when AB_AB_DGGEV3 only
...
*
*  =====================================================================
      SUBROUTINE AB_AB_DDRGEV3( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRES
     $H,
     $                   NOUNIT, A, LDA, B, S, T, Q, LDQ, Z, QE, LDQE,
     $                   ALPHAR, ALPHAI, BETA, ALPHR1, ALPHI1, BETA1,
     $                   WORK, LWORK, RESULT, INFO )
...
*     .. External Subroutines ..
      EXTERNAL           AB_ALASVM, AB_DGET52, AB_AB_DGGEV3, AB_DLABAD, 
     $AB_DLACPY, AB_AB_DLARFG,
...
*
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_DDRGEV3', -INFO )
         RETURN
      END IF
*
...
*
  ...
  110       CONTINUE
*
*           Call AB_AB_DGGEV3 to compute eigenvalues and eigenvectors.
*
            CALL AB_DLACPY( ' ', N, N, A, LDA, S, LDA )
            CALL AB_DLACPY( ' ', N, N, B, LDA, T, LDA )
            CALL AB_AB_DGGEV3( 'V', 'V', N, S, LDA, T, LDA, ALPHAR, ALPH
     $AI,
     $                  BETA, Q, LDQ, Z, LDQ, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'AB_AB_DGGEV31', IERR, N, JTYP
     $E,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
...
*
     ...
     $                   ALPHAI, BETA, WORK, RESULT( 1 ) )
            IF( RESULT( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'AB_AB_DGGEV31',
...
*
     ...
     $                   ALPHAI, BETA, WORK, RESULT( 3 ) )
            IF( RESULT( 4 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'AB_AB_DGGEV31',
...
*
            CALL AB_DLACPY( ' ', N, N, A, LDA, S, LDA )
            CALL AB_DLACPY( ' ', N, N, B, LDA, T, LDA )
            CALL AB_AB_DGGEV3( 'N', 'N', N, S, LDA, T, LDA, ALPHR1, ALPH
     $I1,
     $                  BETA1, Q, LDQ, Z, LDQ, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'AB_AB_DGGEV32', IERR, N, JTYP
     $E,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
...
*
            CALL AB_DLACPY( ' ', N, N, A, LDA, S, LDA )
            CALL AB_DLACPY( ' ', N, N, B, LDA, T, LDA )
            CALL AB_AB_DGGEV3( 'V', 'N', N, S, LDA, T, LDA, ALPHR1, ALPH
     $I1,
     $                  BETA1, QE, LDQE, Z, LDQ, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'AB_AB_DGGEV33', IERR, N, JTYP
     $E,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
...
*
            CALL AB_DLACPY( ' ', N, N, A, LDA, S, LDA )
            CALL AB_DLACPY( ' ', N, N, B, LDA, T, LDA )
            CALL AB_AB_DGGEV3( 'N', 'V', N, S, LDA, T, LDA, ALPHR1, ALPH
     $I1,
     $                  BETA1, Q, LDQ, QE, LDQE, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'AB_AB_DGGEV34', IERR, N, JTYP
     $E,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
...
*
      RETURN
*
 9999 FORMAT( ' AB_AB_DDRGEV3: ', A, ' returned INFO=', I6, '.', / 3X, '
     $N=',
     $      I6, ', JTYPE=', I6, ', ISEED=(', 4( I4, ',' ), I5, ')' )
*
 9998 FORMAT( ' AB_AB_DDRGEV3: ', A, ' Eigenvectors from ', A,
...
*
 9997 FORMAT( / 1X, A3, ' -- Real Generalized eigenvalue problem driver'
     $       )
*
 9996 FORMAT( ' Matrix types (see AB_AB_DDRGEV3 for details): ' )
*
