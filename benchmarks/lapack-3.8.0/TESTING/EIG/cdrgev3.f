...
*  ===========
*
*       SUBROUTINE AB_AB_CDRGEV3( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*> AB_AB_CDRGEV3 checks the nonsymmetric generalized eigenvalue problem driver
*> routine AB_AB_CGGEV3.
*>
*> AB_AB_CGGEV3 computes for a pair of n-by-n nonsymmetric matrices (A,B) the
...
*>          AB_AB_CDRGEV3 does nothing.  NSIZES >= 0.
*> \endverbatim
*>
*> \param[in] NN
*> \verbatim
...
*> \verbatim
*>          NTYPES is INTEGER
*>          The number of elements in DOTYPE.   If it is zero, AB_AB_CDRGEV3
...
*>          next call to AB_AB_CDRGEV3 to continue the same random number
*>          sequence.
*> \endverbatim
*>
*> \param[in] THRESH
...
*>          The Schur form matrix computed from A by AB_AB_CGGEV3.  On exit, S
...
*>          The upper triangular matrix computed from B by AB_AB_CGGEV3.
*> \endverbatim
*>
*> \param[out] Q
*> \verbatim
*>          Q is COMPLEX array, dimension (LDQ, max(NN))
*>          The (left) eigenvectors matrix computed by AB_AB_CGGEV3.
*> \endverbatim
*>
*> \param[in] LDQ
*> \verbatim
...
*>          The (right) orthogonal matrix computed by AB_AB_CGGEV3.
*> \endverbatim
*>
*> \param[out] QE
*> \verbatim
...
*>
*>          The generalized eigenvalues of (A,B) computed by AB_AB_CGGEV3.
...
*>          eigenvalues of A and B, but those computed when AB_AB_CGGEV3 only
...
*
*  =====================================================================
      SUBROUTINE AB_AB_CDRGEV3( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRES
     $H,
     $                    NOUNIT, A, LDA, B, S, T, Q, LDQ, Z, QE, LDQE,
...
*     .. External Functions ..
      ...
      REAL               AB_SLAMCH
      COMPLEX            AB_CLARND
      EXTERNAL           AB_ILAENV, AB_SLAMCH, AB_CLARND
*     ..
*     .. External Subroutines ..
      EXTERNAL           AB_ALASVM, AB_CGET52, AB_AB_CGGEV3, AB_CLACPY, 
     $AB_AB_CLARFG, AB_CLASET,
     $                   AB_CLATM4, AB_CUNM2R, AB_SLABAD, AB_XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      IF( INFO.EQ.0 .AND. LWORK.GE.1 ) THEN
         MINWRK = NMAX*( NMAX+1 )
         NB = MAX( 1, AB_ILAENV( 1, 'AB_AB_CGEQRF', ' ', NMAX, NMAX, -1,
     $ -1 ),
     $        AB_ILAENV( 1, 'AB_CUNMQR', 'LC', NMAX, NMAX, NMAX, -1 ),
     $        AB_ILAENV( 1, 'AB_CUNGQR', ' ', NMAX, NMAX, NMAX, -1 ) )
...
*
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_CDRGEV3', -INFO )
         RETURN
      END IF
*
...
*
                  DO 40 JC = 1, N - 1
                     DO 30 JR = JC, N
                        Q( JR, JC ) = AB_CLARND( 3, ISEED )
                        Z( JR, JC ) = AB_CLARND( 3, ISEED )
   30                CONTINUE
                     CALL AB_AB_CLARFG( N+1-JC, Q( JC, JC ), Q( JC+1, JC
     $ ), 1,
     $                            WORK( JC ) )
                     WORK( 2*N+JC ) = SIGN( ONE, REAL( Q( JC, JC ) ) )
                     Q( JC, JC ) = CONE
                     CALL AB_AB_CLARFG( N+1-JC, Z( JC, JC ), Z( JC+1, JC
     $ ), 1,
     $                            WORK( N+JC ) )
...
*
   ...
   50                CONTINUE
   60             CONTINUE
                  CALL AB_CUNM2R( 'L', 'N', N, N, N-1, Q, LDQ, WORK, A,
     $                         LDA, WORK( 2*N+1 ), IERR )
                  IF( IERR.NE.0 )
     $               GO TO 90
                  CALL AB_CUNM2R( 'R', 'C', N, N, N-1, Z, LDQ, WORK( N+1
     $ ),
     $                         A, LDA, WORK( 2*N+1 ), IERR )
                  IF( IERR.NE.0 )
     $               GO TO 90
                  CALL AB_CUNM2R( 'L', 'N', N, N, N-1, Q, LDQ, WORK, B,
     $                         LDA, WORK( 2*N+1 ), IERR )
                  IF( IERR.NE.0 )
     $               GO TO 90
                  CALL AB_CUNM2R( 'R', 'C', N, N, N-1, Z, LDQ, WORK( N+1
...
*
  ...
  110       CONTINUE
*
*           Call AB_AB_CGGEV3 to compute eigenvalues and eigenvectors.
*
            CALL AB_CLACPY( ' ', N, N, A, LDA, S, LDA )
            CALL AB_CLACPY( ' ', N, N, B, LDA, T, LDA )
            CALL AB_AB_CGGEV3( 'V', 'V', N, S, LDA, T, LDA, ALPHA, BETA,
     $ Q,
     $                   LDQ, Z, LDQ, WORK, LWORK, RWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'AB_AB_CGGEV31', IERR, N, JTYP
     $E,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
...
*
            CALL AB_CGET52( .TRUE., N, A, LDA, B, LDA, Q, LDQ, ALPHA, BE
     $TA,
     $                   WORK, RWORK, RESULT( 1 ) )
            IF( RESULT( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'AB_AB_CGGEV31',
     $            RESULT( 2 ), N, JTYPE, IOLDSD
            END IF
*
*           Do the tests (3) and (4)
*
            CALL AB_CGET52( .FALSE., N, A, LDA, B, LDA, Z, LDQ, ALPHA
     $,
     $                   BETA, WORK, RWORK, RESULT( 3 ) )
            IF( RESULT( 4 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'AB_AB_CGGEV31',
...
*
            CALL AB_CLACPY( ' ', N, N, A, LDA, S, LDA )
            CALL AB_CLACPY( ' ', N, N, B, LDA, T, LDA )
            CALL AB_AB_CGGEV3( 'N', 'N', N, S, LDA, T, LDA, ALPHA1, BETA
     $1, Q,
     $                   LDQ, Z, LDQ, WORK, LWORK, RWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'AB_AB_CGGEV32', IERR, N, JTYP
     $E,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
...
*
            CALL AB_CLACPY( ' ', N, N, A, LDA, S, LDA )
            CALL AB_CLACPY( ' ', N, N, B, LDA, T, LDA )
            CALL AB_AB_CGGEV3( 'V', 'N', N, S, LDA, T, LDA, ALPHA1, BETA
     $1, QE,
     $                   LDQE, Z, LDQ, WORK, LWORK, RWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'AB_AB_CGGEV33', IERR, N, JTYP
     $E,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
...
*
            CALL AB_CLACPY( ' ', N, N, A, LDA, S, LDA )
            CALL AB_CLACPY( ' ', N, N, B, LDA, T, LDA )
            CALL AB_AB_CGGEV3( 'N', 'V', N, S, LDA, T, LDA, ALPHA1, BETA
     $1, Q,
     $                  LDQ, QE, LDQE, WORK, LWORK, RWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'AB_AB_CGGEV34', IERR, N, JTYP
     $E,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
...
*     Summary
*
      CALL AB_ALASVM( 'CGV3', NOUNIT, NERRS, NTESTT, 0 )
*
      WORK( 1 ) = MAXWRK
*
      RETURN
*
 9999 FORMAT( ' AB_AB_CDRGEV3: ', A, ' returned INFO=', I6, '.', / 3X, '
     $N=',
     $      I6, ', JTYPE=', I6, ', ISEED=(', 3( I5, ',' ), I5, ')' )
*
 9998 FORMAT( ' AB_AB_CDRGEV3: ', A, ' Eigenvectors from ', A,
...
*
 9997 FORMAT( / 1X, A3, ' -- Complex Generalized eigenvalue problem ',
     $      'driver' )
*
 9996 FORMAT( ' Matrix types (see AB_AB_CDRGEV3 for details): ' )
*
