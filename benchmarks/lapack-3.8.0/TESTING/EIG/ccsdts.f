...
*  ===========
*
*       SUBROUTINE AB_CCSDTS( M, P, Q, X, XF, LDX, U1, LDU1, U2, LDU2, V1T,
...
*> \verbatim
*>
*> AB_CCSDTS tests CUNCSD, which, given an M-by-M partitioned unitary
*> matrix X,
*>              Q  M-Q
*>       X = [ X11 X12 ] P   ,
*>           [ X21 X22 ] M-P
...
*
*     .. Parameters ..
      REAL               PIOVER2, REALONE, REAAB_LZERO
...
*     .. External Subroutines ..
      EXTERNAL           AB_CGEMM, AB_AB_CHERK, AB_CLACPY, AB_CLASET, CU
     $NCSD,
     $                   AB_CUNCSD2BY1
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL AB_CLASET( 'Full', M, M, ZERO, ONE, WORK, LDX )
      CALL AB_AB_CHERK( 'Upper', 'Conjugate transpose', M, M, -REALONE,
     $            X, LDX, REALONE, WORK, LDX )
      IF (M.GT.0) THEN
         EPS2 = MAX( ULP,
     $               AB_CLANGE( '1', M, M, WORK, LDX, RWORK ) / REAL( M 
     $) )
      ELSE
         EPS2 = ULP
      END IF
...
*
      CALL AB_CLACPY( 'Full', M, M, X, LDX, XF, LDX )
*
*     Compute the CSD
*
...
*
      CALL AB_CLACPY( 'Full', M, M, X, LDX, XF, LDX )
*
      CALL AB_CGEMM( 'No transpose', 'Conjugate transpose', P, Q, Q, ONE
     $,
     $            XF, LDX, V1T, LDV1T, ZERO, WORK, LDX )
*
      CALL AB_CGEMM( 'Conjugate transpose', 'No transpose', P, Q, P, ONE
     $,
...
*
     ...
     $              0.0E0 )
      END DO
*
      CALL AB_CGEMM( 'No transpose', 'Conjugate transpose', P, M-Q, M-Q,
     $            ONE, XF(1,Q+1), LDX, V2T, LDV2T, ZERO, WORK, LDX )
*
      CALL AB_CGEMM( 'Conjugate transpose', 'No transpose', P, M-Q, P,
...
*
     ...
     $      CMPLX( SIN(THETA(R-I+1)), 0.0E0 )
      END DO
*
      CALL AB_CGEMM( 'No transpose', 'Conjugate transpose', M-P, Q, Q, O
     $NE,
     $            XF(P+1,1), LDX, V1T, LDV1T, ZERO, WORK, LDX )
*
      CALL AB_CGEMM( 'Conjugate transpose', 'No transpose', M-P, Q, M-P,
...
*
     ...
     $             CMPLX( SIN(THETA(R-I+1)), 0.0E0 )
      END DO
*
      CALL AB_CGEMM( 'No transpose', 'Conjugate transpose', M-P, M-Q, M-
     $Q,
     $            ONE, XF(P+1,Q+1), LDX, V2T, LDV2T, ZERO, WORK, LDX )
*
      CALL AB_CGEMM( 'Conjugate transpose', 'No transpose', M-P, M-Q, M-
     $P,
...
*
      RESID = AB_CLANGE( '1', P, Q, XF, LDX, RWORK )
      RESULT( 1 ) = ( RESID / REAL(MAX(1,P,Q)) ) / EPS2
*
*     Compute norm( U1'*X12*V2 - D12 ) / ( MAX(1,P,M-Q)*EPS2 ) .
*
      RESID = AB_CLANGE( '1', P, M-Q, XF(1,Q+1), LDX, RWORK )
      RESULT( 2 ) = ( RESID / REAL(MAX(1,P,M-Q)) ) / EPS2
*
*     Compute norm( U2'*X21*V1 - D21 ) / ( MAX(1,M-P,Q)*EPS2 ) .
*
      RESID = AB_CLANGE( '1', M-P, Q, XF(P+1,1), LDX, RWORK )
      RESULT( 3 ) = ( RESID / REAL(MAX(1,M-P,Q)) ) / EPS2
*
*     Compute norm( U2'*X22*V2 - D22 ) / ( MAX(1,M-P,M-Q)*EPS2 ) .
*
      RESID = AB_CLANGE( '1', M-P, M-Q, XF(P+1,Q+1), LDX, RWORK )
      RESULT( 4 ) = ( RESID / REAL(MAX(1,M-P,M-Q)) ) / EPS2
*
*     Compute I - U1'*U1
*
      CALL AB_CLASET( 'Full', P, P, ZERO, ONE, WORK, LDU1 )
      CALL AB_AB_CHERK( 'Upper', 'Conjugate transpose', P, P, -REALONE,
     $            U1, LDU1, REALONE, WORK, LDU1 )
*
*     Compute norm( I - U'*U ) / ( MAX(1,P) * ULP ) .
*
      RESID = AB_CLANHE( '1', 'Upper', P, WORK, LDU1, RWORK )
      RESULT( 5 ) = ( RESID / REAL(MAX(1,P)) ) / ULP
*
*     Compute I - U2'*U2
*
      CALL AB_CLASET( 'Full', M-P, M-P, ZERO, ONE, WORK, LDU2 )
      CALL AB_AB_CHERK( 'Upper', 'Conjugate transpose', M-P, M-P, -REALO
     $NE,
     $            U2, LDU2, REALONE, WORK, LDU2 )
*
*     Compute norm( I - U2'*U2 ) / ( MAX(1,M-P) * ULP ) .
*
      RESID = AB_CLANHE( '1', 'Upper', M-P, WORK, LDU2, RWORK )
      RESULT( 6 ) = ( RESID / REAL(MAX(1,M-P)) ) / ULP
*
*     Compute I - V1T*V1T'
*
      CALL AB_CLASET( 'Full', Q, Q, ZERO, ONE, WORK, LDV1T )
      CALL AB_AB_CHERK( 'Upper', 'No transpose', Q, Q, -REALONE,
     $            V1T, LDV1T, REALONE, WORK, LDV1T )
*
*     Compute norm( I - V1T*V1T' ) / ( MAX(1,Q) * ULP ) .
*
      RESID = AB_CLANHE( '1', 'Upper', Q, WORK, LDV1T, RWORK )
      RESULT( 7 ) = ( RESID / REAL(MAX(1,Q)) ) / ULP
*
*     Compute I - V2T*V2T'
*
      CALL AB_CLASET( 'Full', M-Q, M-Q, ZERO, ONE, WORK, LDV2T )
      CALL AB_AB_CHERK( 'Upper', 'No transpose', M-Q, M-Q, -REALONE,
     $            V2T, LDV2T, REALONE, WORK, LDV2T )
*
*     Compute norm( I - V2T*V2T' ) / ( MAX(1,M-Q) * ULP ) .
*
      RESID = AB_CLANHE( '1', 'Upper', M-Q, WORK, LDV2T, RWORK )
...
*
      CALL AB_CLASET( 'Full', Q, Q, ZERO, ONE, WORK, LDX )
      CALL AB_AB_CHERK( 'Upper', 'Conjugate transpose', Q, M, -REALONE,
     $            X, LDX, REALONE, WORK, LDX )
      IF (M.GT.0) THEN
         EPS2 = MAX( ULP,
     $               AB_CLANGE( '1', Q, Q, WORK, LDX, RWORK ) / REAL( M 
     $) )
      ELSE
         EPS2 = ULP
      END IF
...
*     Copy the matrix X to the array XF.
*
      CALL AB_CLACPY( 'Full', M, Q, X, LDX, XF, LDX )
*
*     Compute the CSD
*
      CALL AB_CUNCSD2BY1( 'Y', 'Y', 'Y', M, P, Q, XF(1,1), LDX, XF(P+1,1
     $),
...
*     Compute norm( U1'*X11*V1 - D11 ) / ( MAX(1,P,Q)*EPS2 ) .
*
      RESID = AB_CLANGE( '1', P, Q, X, LDX, RWORK )
      RESULT( 10 ) = ( RESID / REAL(MAX(1,P,Q)) ) / EPS2
*
*     Compute norm( U2'*X21*V1 - D21 ) / ( MAX(1,M-P,Q)*EPS2 ) .
*
      RESID = AB_CLANGE( '1', M-P, Q, X(P+1,1), LDX, RWORK )
