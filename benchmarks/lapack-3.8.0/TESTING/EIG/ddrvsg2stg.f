...
*  ===========
*
*       SUBROUTINE AB_AB_DDRVSG2STG( NSIZES, NN, NTYPES, DOTYPE, ISEED, THRESH,
...
*> \verbatim
*>
*>      AB_AB_DDRVSG2STG checks the real symmetric generalized eigenproblem
*>      drivers.
*>
...
*>              generalized eigenproblem.
*>
*>      When AB_AB_DDRVSG2STG is called, a number of matrix "sizes" ("n's") and a
...
*>                                         AB_DSYGV and  D2 is computed by
*>                                         AB_AB_DSYGV_2STAGE. This test is
...
*>          AB_AB_DDRVSG2STG does nothing.  It must be at least zero.
*>          Not modified.
*>
...
*>
*>  NTYPES  INTEGER
*>          The number of elements in DOTYPE.   If it is zero, AB_AB_DDRVSG2STG
...
*>          next call to AB_AB_DDRVSG2STG to continue the same random number
*>          sequence.
*>          Modified.
*>
...
*
*  =====================================================================
      SUBROUTINE AB_AB_DDRVSG2STG( NSIZES, NN, NTYPES, DOTYPE, ISEED, TH
     $RESH,
     $                       NOUNIT, A, LDA, B, LDB, D, D2, Z, LDZ, AB,
     $                       BB, AP, BP, WORK, NWORK, IWORK, LIWORK, 
     $                       RESULT, INFO )
...
*
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_DDRVSG2STG', -INFO )
         RETURN
      END IF
*
...
*
                  ...
                  CALL AB_DLACPY( UPLO, N, N, B, LDB, BB, LDB )
*
                  CALL AB_AB_DSYGV_2STAGE( IBTYPE, 'N', UPLO, N, Z, LDZ,
     $                               BB, LDB, D2, WORK, NWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )
     $                  'AB_AB_DSYGV_2STAGE(V,' // UPLO //
     $                  ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*                 Do Test
*
C                  CALL AB_DSGT01( IBTYPE, UPLO, N, N, A, LDA, B, LDB, Z
     $,
...
*
      RETURN
*
*     End of AB_AB_DDRVSG2STG
*
 9999 FORMAT( ' AB_AB_DDRVSG2STG: ', A, ' returned INFO=', I6, '.', / 9X
     $,
