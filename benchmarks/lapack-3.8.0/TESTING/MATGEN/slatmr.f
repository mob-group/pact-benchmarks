...
*  ===========
*
*       SUBROUTINE AB_SLATMR( M, N, DIST, ISEED, SYM, D, MODE, COND, DMAX,
...
*> \verbatim
*>
*>    AB_SLATMR generates random matrices of various types for testing
*>    LAPACK programs.
*>
*>    AB_SLATMR operates by applying the following sequence of
*>    operations:
*>
...
*>
*>    Note: If two calls to AB_SLATMR differ only in the PACK parameter,
*>          they will generate mathematically equivalent matrices.
*>
*>          If two calls to AB_SLATMR both have full bandwidth (KL = M-1
...
*>           exit, and can be used in the next call to AB_SLATMR
...
*>           are permuted.   If, say, row pivoting is selected, AB_SLATMR
...
*>
*>           If two calls to AB_SLATMR differ only in the PACK parameter,
*>           they will generate mathematically equivalent matrices.
*>           Not modified.
*> \endverbatim
*>
...
*>             3 => Error return from AB_SLATM1 (computing DL)
*>             4 => Error return from AB_SLATM1 (computing DR)
...
*     .. External Functions ..
      LOGICAL            AB_LSAME
      REAL               AB_SLANGB, AB_SLANGE, AB_SLANSB, AB_SLANSP, AB_
     $SLANSY, AB_SLATM2,
     $                   AB_SLATM3
      EXTERNAL           AB_LSAME, AB_SLANGB, AB_SLANGE, AB_SLANSB, AB_S
     $LANSP, AB_SLANSY,
     $                   AB_SLATM2, AB_SLATM3
*     ..
*     .. External Subroutines ..
      EXTERNAL           AB_SLATM1, AB_SSCAL, AB_XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL AB_SLATM1( MODE, COND, IRSIGN, IDIST, ISEED, D, MNMIN, INFO )
      IF( INFO.NE.0 ) THEN
         INFO = 1
         RETURN
      END IF
...
*
     ...
     $    5 ) THEN
         CALL AB_SLATM1( MODEL, CONDL, 0, IDIST, ISEED, DL, M, INFO )
         IF( INFO.NE.0 ) THEN
            INFO = 3
            RETURN
         END IF
...
*
      IF( IGRADE.EQ.2 .OR. IGRADE.EQ.3 ) THEN
         CALL AB_SLATM1( MODER, CONDR, 0, IDIST, ISEED, DR, N, INFO )
         IF( INFO.NE.0 ) THEN
            INFO = 4
            RETURN
         END IF
...
*
      IF( FULBND ) THEN
*
*        Use AB_SLATM3 so matrices generated with differing PIVOTing only
...
*
         IF( IPACK.EQ.0 ) THEN
            IF( ISYM.EQ.0 ) THEN
               DO 100 J = 1, N
                  DO 90 I = 1, J
                     TEMP = AB_SLATM3( M, N, I, J, ISUB, JSUB, KL, KU,
...
*
            DO 140 J = 1, N
               DO 130 I = 1, J
                  TEMP = AB_SLATM3( M, N, I, J, ISUB, JSUB, KL, KU, IDIS
     $T,
...
*
         IF( IPACK.EQ.0 ) THEN
            IF( ISYM.EQ.0 ) THEN
               DO 300 J = 1, N
                  DO 290 I = 1, J
                     A( I, J ) = AB_SLATM2( M, N, I, J, KL, KU, IDIST,
...
*
            DO 340 J = 1, N
               DO 330 I = 1, J
                  A( I, J ) = AB_SLATM2( M, N, I, J, KL, KU, IDIST, ISEE
     $D,
...
*
            DO 360 J = 1, N
               DO 350 I = 1, J
                  A( J, I ) = AB_SLATM2( M, N, I, J, KL, KU, IDIST, ISEE
     $D,
     $                        D, IGRADE, DL, DR, IPVTNG, IWORK, SPARSE )
                  IF( I.NE.J )
     $               A( I, J ) = ZERO
            ...
            DO 380 J = 1, N
               DO 370 I = 1, J
                  ...
                  IF( ISUB.GT.LDA ) THEN
                     ...
                     JSUB = JSUB + 1
                  END IF
                  A( ISUB, JSUB ) = AB_SLATM2( M, N, I, J, KL, KU, IDIST
     $,
     $                              ISEED, D, IGRADE, DL, DR, IPVTNG,
     $                              IWORK, SPARSE )
  370          CONTINUE
...
*
                     ...
                     ISUB = K - LDA*( JSUB-1 )
*
                     A( ISUB, JSUB ) = AB_SLATM2( M, N, I, J, KL, KU,
            ...
            DO 440 J = 1, N
               DO 430 I = J - KUU, J
                  IF( I.LT.1 ) THEN
                     A( J-I+1, I+N ) = ZERO
                  ELSE
                     A( J-I+1, I ) = AB_SLATM2( M, N, I, J, KL, KU, IDIS
     $T,
     $                               ISEED, D, IGRADE, DL, DR, IPVTNG,
     $                               IWORK, SPARSE )
                  END IF
...
*
            DO 460 J = 1, N
               DO 450 I = J - KUU, J
                  A( I-J+KUU+1, J ) = AB_SLATM2( M, N, I, J, KL, KU, IDI
     $ST,
     $                                ISEED, D, IGRADE, DL, DR, IPVTNG,
     $                                IWORK, SPARSE )
  450          CONTINUE
...
*
            IF( ISYM.EQ.0 ) THEN
               DO 480 J = 1, N
                  DO 470 I = J - KUU, J
                     A( I-J+KUU+1, J ) = AB_SLATM2( M, N, I, J, KL, KU,
...
*
      IF( IPACK.EQ.0 ) THEN
         ONORM = AB_SLANGE( 'M', M, N, A, LDA, TEMPA )
      ELSE IF( IPACK.EQ.1 ) THEN
         ONORM = AB_SLANSY( 'M', 'U', N, A, LDA, TEMPA )
      ELSE IF( IPACK.EQ.2 ) THEN
         ONORM = AB_SLANSY( 'M', 'L', N, A, LDA, TEMPA )
      ELSE IF( IPACK.EQ.3 ) THEN
         ONORM = AB_SLANSP( 'M', 'U', N, A, TEMPA )
      ELSE IF( IPACK.EQ.4 ) THEN
         ONORM = AB_SLANSP( 'M', 'L', N, A, TEMPA )
      ELSE IF( IPACK.EQ.5 ) THEN
         ONORM = AB_SLANSB( 'M', 'L', N, KLL, A, LDA, TEMPA )
      ELSE IF( IPACK.EQ.6 ) THEN
         ONORM = AB_SLANSB( 'M', 'U', N, KUU, A, LDA, TEMPA )
      ELSE IF( IPACK.EQ.7 ) THEN
         ONORM = AB_SLANGB( 'M', N, KLL, KUU, A, LDA, TEMPA )
      END IF
*
...
*
            IF( IPACK.LE.2 ) THEN
               DO 510 J = 1, N
                  CALL AB_SSCAL( M, ONE / ONORM, A( 1, J ), 1 )
                  CALL AB_SSCAL( M, ANORM, A( 1, J ), 1 )
  510          CONTINUE
*
            ELSE IF( IPACK.EQ.3 .OR. IPACK.EQ.4 ) THEN
*
               CALL AB_SSCAL( N*( N+1 ) / 2, ONE / ONORM, A, 1 )
               CALL AB_SSCAL( N*( N+1 ) / 2, ANORM, A, 1 )
*
            ELSE IF( IPACK.GE.5 ) THEN
*
               DO 520 J = 1, N
                  CALL AB_SSCAL( KLL+KUU+1, ONE / ONORM, A( 1, J ), 1 )
                  CALL AB_SSCAL( KLL+KUU+1, ANORM, A( 1, J ), 1 )
  520          CONTINUE
*
            END IF
*
...
*
            IF( IPACK.LE.2 ) THEN
               DO 530 J = 1, N
                  CALL AB_SSCAL( M, ANORM / ONORM, A( 1, J ), 1 )
  530          CONTINUE
*
            ELSE IF( IPACK.EQ.3 .OR. IPACK.EQ.4 ) THEN
*
               CALL AB_SSCAL( N*( N+1 ) / 2, ANORM / ONORM, A, 1 )
*
            ELSE IF( IPACK.GE.5 ) THEN
*
               DO 540 J = 1, N
                  CALL AB_SSCAL( KLL+KUU+1, ANORM / ONORM, A( 1, J ), 1 
     $)
  540          CONTINUE
            END IF
*
