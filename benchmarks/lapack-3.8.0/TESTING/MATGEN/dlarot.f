...
*  ===========
*
*       SUBROUTINE AB_DLAROT( LROWS, LLEFT, LRIGHT, NL, C, S, A, LDA, XLEFT,
*                          XRIGHT )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*>    AB_DLAROT applies a (Givens) rotation to two adjacent rows or
...
*>    AB_DLAROT would rotate columns j and j+1, using p, and restore
...
*>
*>              CALL AB_DLAROT(.TRUE.,.FALSE.,.FALSE., N, C,S,
...
*>              j = MAX(1, i-KL )
*>              NL = MIN( N, i+KU+1 ) + 1-j
*>              CALL AB_DLAROT( .TRUE., i-KL.GE.1, i+KU.LT.N, NL, C,S,
...
*>              j = MAX(1, i-K )
*>              NL = MIN( K+1, i ) + 1
*>              CALL AB_DLAROT( .TRUE., i-K.GE.1, .TRUE., NL, C,S,
*>                      A(i,j), LDA, XLEFT, XRIGHT )
*>
*>      Same, but upper triangle only:
*>
*>              NL = MIN( K+1, N-i ) + 1
*>              CALL AB_DLAROT( .TRUE., .TRUE., i+K.LT.N, NL, C,S,
...
*>              j = MAX(1, i-KU )
*>              NL = MIN( N, i+KL+1 ) + 1-j
*>              CALL AB_DLAROT( .TRUE., i-KU.GE.1, i+KL.LT.N, NL, C,S,
...
*> \verbatim
*>  LROWS  - LOGICAL
*>           If .TRUE., then AB_DLAROT will rotate two rows.  If .FALSE.,
...
*>           A were dimensioned A(LDA,*) in AB_DLAROT, then A(1,j) would
...
*
*  =====================================================================
      SUBROUTINE AB_DLAROT( LROWS, LLEFT, LRIGHT, NL, C, S, A, LDA, XLEF
     $T,
     $                   XRIGHT )
*
...
*
      IF( NL.LT.NT ) THEN
         CALL AB_XERBLA( 'AB_DLAROT', 4 )
         RETURN
      END IF
      IF( LDA.LE.0 .OR. ( .NOT.LROWS .AND. LDA.LT.NL-NT ) ) THEN
         CALL AB_XERBLA( 'AB_DLAROT', 8 )
         RETURN
      END IF
*
*     Rotate
*
      CALL AB_DROT( NL-NT, A( IX ), IINC, A( IY ), IINC, C, S )
      CALL AB_DROT( NT, XT, 1, YT, 1, C, S )
*
