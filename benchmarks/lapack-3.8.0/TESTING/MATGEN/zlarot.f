...
*  ===========
*
*       SUBROUTINE AB_ZLAROT( LROWS, LLEFT, LRIGHT, NL, C, S, A, LDA, XLEFT,
*                          XRIGHT )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*>    AB_ZLAROT applies a (Givens) rotation to two adjacent rows or
...
*>
*>    Those columns which have two '*'s can be handled by AB_DROT.
...
*>    AB_ZLAROT would rotate columns j and j+1, using p, and restore
...
*>
*>              CALL AB_ZLAROT(.TRUE.,.FALSE.,.FALSE., N, C,S,
...
*>              j = MAX(1, i-KL )
*>              NL = MIN( N, i+KU+1 ) + 1-j
*>              CALL AB_ZLAROT( .TRUE., i-KL.GE.1, i+KU.LT.N, NL, C,S,
...
*>              j = MAX(1, i-K )
*>              NL = MIN( K+1, i ) + 1
*>              CALL AB_ZLAROT( .TRUE., i-K.GE.1, .TRUE., NL, C,S,
*>                      A(i,j), LDA, XLEFT, XRIGHT )
*>
*>      Same, but upper triangle only:
*>
*>              NL = MIN( K+1, N-i ) + 1
*>              CALL AB_ZLAROT( .TRUE., .TRUE., i+K.LT.N, NL, C,S,
...
*>              j = MAX(1, i-KU )
*>              NL = MIN( N, i+KL+1 ) + 1-j
*>              CALL AB_ZLAROT( .TRUE., i-KU.GE.1, i+KL.LT.N, NL, C,S,
...
*> \verbatim
*>  LROWS  - LOGICAL
*>           If .TRUE., then AB_ZLAROT will rotate two rows.  If .FALSE.,
...
*>           output of AB_AB_ZROTG or to most versions of AB_ZROT, both C and S
...
*>           dimensioned A(LDA,*) in AB_ZLAROT, then A(1,j) would be the
...
*
*  =====================================================================
      SUBROUTINE AB_ZLAROT( LROWS, LLEFT, LRIGHT, NL, C, S, A, LDA, XLEF
     $T,
     $                   XRIGHT )
*
...
*
      IF( NL.LT.NT ) THEN
         CALL AB_XERBLA( 'AB_ZLAROT', 4 )
         RETURN
      END IF
      IF( LDA.LE.0 .OR. ( .NOT.LROWS .AND. LDA.LT.NL-NT ) ) THEN
         CALL AB_XERBLA( 'AB_ZLAROT', 8 )
         RETURN
      END IF
*
*     Rotate
*
*     AB_ZROT( NL-NT, A(IX),IINC, A(IY),IINC, C, S ) with complex C, S
*
      DO 10 J = 0, NL - NT - 1
   ...
   10 CONTINUE
*
*     AB_ZROT( NT, XT,1, YT,1, C, S ) with complex C, S
*
      DO 20 J = 1, NT
         TEMPX = C*XT( J ) + S*YT( J )
