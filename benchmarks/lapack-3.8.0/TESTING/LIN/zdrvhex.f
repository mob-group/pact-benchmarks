...
*  ===========
*
*       SUBROUTINE AB_ZDRVHE( DOTYPE, NN, NVAL, NRHS, THRESH, TSTERR, NMAX,
...
*> \verbatim
*>
*> AB_ZDRVHE tests the driver routines AB_ZHESV, -SVX, and -SVXX.
*>
*> Note that this file is used only when the XBLAS are available,
*> otherwise AB_ZDRVHE.f defines this subroutine.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*     .. External Subroutines ..
     ...
     $ZERRVX, AB_ZGET04,
     $                   AB_ZHESV, AB_AB_ZHESVX, AB_ZHET01, AB_ZHETRF, A
     $B_AB_ZHETRI2, AB_ZLACPY,
     $                   AB_ZLAIPD, AB_ZLARHS, AB_ZLASET, AB_ZLATB4, AB_
     $ZLATMS, AB_ZPOT02,
     $                   AB_ZPOT05, AB_AB_AB_ZHESVXX
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
      CHARACTER*32       SRNAMT
...
*                 the value returned by AB_AB_ZHESVX.
*
                  IF( ZEROT ) THEN
                     IF( IFACT.EQ.1 )
     $                  GO TO 150
...
*
                     ...
                     CALL AB_ZHETRF( UPLO, N, AFAC, LDA, IWORK, WORK,
     $                            LWORK, INFO )
*
...
*
                     ...
                     LWORK = (N+NB+1)*(NB+3)
                     CALL AB_AB_ZHETRI2( UPLO, N, AINV, LDA, IWORK, WORK
     $,
     $                            LWORK, INFO )
                     AINVNM = AB_ZLANHE( '1', UPLO, N, AINV, LDA, RWORK 
     $)
*
...
*
*                    Factor the matrix and solve the system using AB_ZHESV.
*
                     SRNAMT = 'AB_ZHESV '
                     CALL AB_ZHESV( UPLO, N, NRHS, AFAC, LDA, IWORK, X,
     $                           LDA, WORK, LWORK, INFO )
*
...
*
  ...
  100                   CONTINUE
                     ...
                     END IF
*
*                    Check error code from AB_ZHESV .
*
                     IF( INFO.NE.K ) THEN
                        CALL AB_ALAERH( PATH, 'AB_ZHESV ', INFO, K, UPLO
     $, N,
...
*                    residual.
*
                     CALL AB_ZHET01( UPLO, N, A, LDA, AFAC, LDA, IWORK,
...
*
     ...
     $                        CALL AB_ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'AB_ZHESV ', UPLO, N
     $,
     $                        IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
...
*                 error bounds using AB_AB_ZHESVX.
*
                  SRNAMT = 'AB_AB_ZHESVX'
                  CALL AB_AB_ZHESVX( FACT, UPLO, N, NRHS, A, LDA, AFAC, 
     $LDA,
...
*
  ...
  130                CONTINUE
                  ...
                  END IF
*
*                 Check the error code from AB_AB_ZHESVX.
*
                  IF( INFO.NE.K ) THEN
                     CALL AB_ALAERH( PATH, 'AB_AB_ZHESVX', INFO, K, FACT
     $ // UPLO,
...
*                       residual.
*
                        CALL AB_ZHET01( UPLO, N, A, LDA, AFAC, LDA, IWOR
     $K,
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from AB_AB_ZHESVX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL AB_ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'AB_AB_ZHESVX', FACT, U
     $PLO,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  140             CONTINUE
                  NRUN = NRUN + 7 - K1
*
*                 --- Test AB_AB_AB_ZHESVXX ---
*
*                 Restore the matrices A and B.
*
                  IF( IFACT.EQ.2 )
     $               CALL AB_ZLASET( UPLO, N, N, CMPLX( ZERO ),
     $                 CMPLX( ZERO ), AFAC, LDA )
                  CALL AB_ZLASET( 'Full', N, NRHS, CMPLX( ZERO ),
     $                 CMPLX( ZERO ), X, LDA )
*
*                 Solve the system and compute the condition number
*                 and error bounds using AB_AB_AB_ZHESVXX.
*
                  SRNAMT = 'AB_AB_AB_ZHESVXX'
                  N_ERR_BNDS = 3
                  EQUED = 'N'
                  CALL AB_AB_AB_ZHESVXX( FACT, UPLO, N, NRHS, A, LDA, AF
     $AC,
...
*
 ...
 135                 CONTINUE
                  ...
                  END IF
*
*                 Check the error code from AB_AB_AB_ZHESVXX.
*
                  IF( INFO.NE.K .AND. INFO.LE.N) THEN
                     CALL AB_ALAERH( PATH, 'AB_AB_AB_ZHESVXX', INFO, K,
...
*
     ...
     $                    RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from AB_AB_AB_ZHESVXX with the computed value
*                 in RCONDC.
*
                  RESULT( 6 ) = AB_DGET06( RCOND, RCONDC )
*
...
*
                  DO 85 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                       CALL AB_ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'AB_AB_AB_ZHESVXX',
     $                       FACT, UPLO, N, IMAT, K,
     $                       RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
...
*     Test Error Bounds from AB_AB_AB_ZHESVXX
