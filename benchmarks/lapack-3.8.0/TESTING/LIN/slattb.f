...
*  ===========
*
*       SUBROUTINE AB_SLATTB( IMAT, UPLO, TRANS, DIAG, ISEED, N, KD, AB,
*                          LDAB, B, WORK, INFO )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> AB_SLATTB generates a triangular test matrix in 2-dimensional storage.
...
*
         IF( KD.EQ.1 ) THEN
            IF( UPPER ) THEN
               AB( 1, 2 ) = SIGN( TNORM, AB_SLARND( 2, ISEED ) )
               LENJ = ( N-3 ) / 2
               CALL AB_SLARNV( 2, ISEED, LENJ, WORK )
               DO 90 J = 1, LENJ
                  AB( 1, 2*( J+1 ) ) = TNORM*WORK( J )
   90          CONTINUE
            ELSE
               AB( 2, 1 ) = SIGN( TNORM, AB_SLARND( 2, ISEED ) )
               LENJ = ( N-3 ) / 2
               CALL AB_SLARNV( 2, ISEED, LENJ, WORK )
               DO 100 J = 1, LENJ
...
*
            STAR1 = SIGN( TNORM, AB_SLARND( 2, ISEED ) )
            SFAC = SQRT( TNORM )
            PLUS1 = SIGN( SFAC, AB_SLARND( 2, ISEED ) )
            DO 110 J = 1, N, 2
               PLUS2 = STAR1 / PLUS1
               WORK( J ) = PLUS1
               WORK( N+J ) = STAR1
...
*
            IF( UPPER ) THEN
               CALL AB_SCOPY( N-1, WORK, 1, AB( KD, 2 ), LDAB )
               CALL AB_SCOPY( N-2, WORK( N+1 ), 1, AB( KD-1, 3 ), LDAB )
            ELSE
               CALL AB_SCOPY( N-1, WORK, 1, AB( 2, 1 ), LDAB )
               CALL AB_SCOPY( N-2, WORK( N+1 ), 1, AB( 3, 1 ), LDAB )
            END IF
         END IF
*
...
*
         IF( UPPER ) THEN
            DO 120 J = 1, N
               LENJ = MIN( J, KD+1 )
               CALL AB_SLARNV( 2, ISEED, LENJ, AB( KD+2-LENJ, J ) )
  ...
  120       CONTINUE
         ELSE
            DO 130 J = 1, N
               LENJ = MIN( N-J+1, KD+1 )
               IF( LENJ.GT.0 )
     $            CALL AB_SLARNV( 2, ISEED, LENJ, AB( 1, J ) )
...
*
         ...
         IF( UPPER ) THEN
            DO 140 J = 1, N
               LENJ = MIN( J, KD+1 )
               CALL AB_SLARNV( 2, ISEED, LENJ, AB( KD+2-LENJ, J ) )
               CALL AB_SSCAL( LENJ-1, TSCAL, AB( KD+2-LENJ, J ), 1 )
  ...
  140       CONTINUE
         ...
         ELSE
            DO 150 J = 1, N
               ...
               CALL AB_SLARNV( 2, ISEED, LENJ, AB( 1, J ) )
               IF( LENJ.GT.1 )
     $            CALL AB_SSCAL( LENJ-1, TSCAL, AB( 2, J ), 1 )
...
*
         IF( UPPER ) THEN
            DO 370 J = 1, N
               LENJ = MIN( J-1, KD )
               CALL AB_SLARNV( 2, ISEED, LENJ, AB( KD+1-LENJ, J ) )
               AB( KD+1, J ) = REAL( J )
  370       CONTINUE
         ELSE
            DO 380 J = 1, N
               LENJ = MIN( N-J, KD )
               IF( LENJ.GT.0 )
     $            CALL AB_SLARNV( 2, ISEED, LENJ, AB( 2, J ) )
               AB( 1, J ) = REAL( J )
  380       CONTINUE
         END IF
*
      ...
      IF( .NOT.AB_LSAME( TRANS, 'N' ) ) THEN
         IF( UPPER ) THEN
            DO 430 J = 1, N / 2
               ...
               CALL AB_SSWAP( LENJ, AB( KD+1, J ), LDAB-1,
     $                     AB( KD+2-LENJ, N-J+1 ), -1 )
  430       CONTINUE
         ELSE
            DO 440 J = 1, N / 2
               LENJ = MIN( N-2*J+1, KD+1 )
               CALL AB_SSWAP( LENJ, AB( 1, J ), 1, AB( LENJ, N-J+2-LENJ 
     $),
     $                     -LDAB+1 )
  440       CONTINUE
         END IF
