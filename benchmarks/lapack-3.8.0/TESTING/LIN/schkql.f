...
*  ===========
*
*       SUBROUTINE AB_SCHKQL( DOTYPE, NM, MVAL, NN, NVAL, NNB, NBVAL, NXVAL,
...
*> \verbatim
*>
*> AB_SCHKQL tests AB_SGEQLF, AB_SORGQL and AB_SORMQL.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*
*  =====================================================================
      SUBROUTINE AB_SCHKQL( DOTYPE, NM, MVAL, NN, NVAL, NNB, NBVAL, NXVA
     $L,
     $                   NRHS, THRESH, TSTERR, NMAX, A, AF, AQ, AL, AC,
     $                   B, X, XACT, TAU, WORK, RWORK, NOUT )
*
...
*     .. External Subroutines ..
      EXTERNAL           AB_ALAERH, AB_ALAHD, AB_ALASUM, AB_SERRQL, AB_S
     $GEQLS, AB_SGET02,
...
*              corresponding to the call of AB_SQLT01; other values are
*              used in the calls of AB_SQLT02, and must not exceed MINMN.
*
               KVAL( 1 ) = MINMN
               KVAL( 2 ) = 0
               KVAL( 3 ) = 1
...
*                       Test AB_SGEQLF
*
                        CALL AB_SQLT01( M, N, A, AF, AQ, AL, LDA, TAU,
     $                               WORK, LWORK, RWORK, RESULT( 1 ) )
                     ELSE IF( M.GE.N ) THEN
*
*                       Test AB_SORGQL, using factorization
*                       returned by AB_SQLT01
*
                        CALL AB_SQLT02( M, N, K, A, AF, AQ, AL, LDA, TAU
     $,
     $                               WORK, LWORK, RWORK, RESULT( 1 ) )
                     END IF
                     IF( M.GE.K ) THEN
*
*                       Test AB_SORMQL, using factorization returned
*                       by AB_SQLT01
*
                        CALL AB_SQLT03( M, N, K, AF, AC, AL, AQ, LDA, TA
     $U,
     $                               WORK, LWORK, RWORK, RESULT( 3 ) )
                        NT = NT + 4
*
*                       If M>=N and K=N, call AB_SGEQLS to solve a system
...
*
     ...
     $                                  LDA )
                           SRNAMT = 'AB_SGEQLS'
                           CALL AB_SGEQLS( M, N, NRHS, AF, LDA, TAU, X,
     $                                  LDA, WORK, LWORK, INFO )
*
*                          Check error code from AB_SGEQLS.
*
                           IF( INFO.NE.0 )
     $                        CALL AB_ALAERH( PATH, 'AB_SGEQLS', INFO, 0
     $, ' ',
     $                                     M, N, NRHS, -1, NB, IMAT,
     $                                     NFAIL, NERRS, NOUT )
*
