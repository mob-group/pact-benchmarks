...
*> \verbatim
*>
*> AB_SDRVGB tests the driver routines AB_SGBSV, -SVX, and -SVXX.
*>
*> Note that this file is used only when the XBLAS are available,
*> otherwise AB_SDRVGB.f defines this subroutine.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*     .. External Subroutines ..
     ...
     $SLASET, AB_SLATB4,
     $                   AB_SLATMS, AB_XLAENV, AB_AB_AB_SGBSVXX
*     ..
*     .. Intrinsic Functions ..
...
*
                           END IF
*
*                    --- Test AB_AB_AB_SGBSVXX ---
*
*                    Restore the matrices A and B.
*
                     CALL AB_SLACPY( 'Full', KL+KU+1, N, ASAV, LDA, A,
     $                          LDA )
                     CALL AB_SLACPY( 'Full', N, NRHS, BSAV, LDB, B, LDB 
     $)

                     IF( .NOT.PREFAC )
     $                  CALL AB_SLASET( 'Full', 2*KL+KU+1, N, ZERO, ZERO
     $,
     $                    AFB, LDAFB )
                     CALL AB_SLASET( 'Full', N, NRHS, ZERO, ZERO, X, LDB
     $ )
                     IF( IEQUED.GT.1 .AND. N.GT.0 ) THEN
*
*                       Equilibrate the matrix if FACT = 'F' and
...
*                    and error bounds using AB_AB_AB_SGBSVXX.
*
                     SRNAMT = 'AB_AB_AB_SGBSVXX'
                     N_ERR_BNDS = 3
                     CALL AB_AB_AB_SGBSVXX( FACT, TRANS, N, KL, KU, NRHS
     $, A, LDA,
...
*                    Check the error code from AB_AB_AB_SGBSVXX.
*
                     IF( INFO.EQ.N+1 ) GOTO 90
                     IF( INFO.NE.IZERO ) THEN
                        CALL AB_ALAERH( PATH, 'AB_AB_AB_SGBSVXX', INFO, 
     $IZERO,
     ...
     $                               IMAT, NFAIL, NERRS, NOUT )
                     ...
                     END IF
*
*                    Compare rpvgrw_svxx from AB_AB_AB_SGBSVXX with the computed
*                    reciprocal pivot growth factor RPVGRW
*

                     IF ( INFO .GT. 0 .AND. INFO .LT. N+1 ) THEN
                        RPVGRW = AB_SLA_GBRPVGRW(N, KL, KU, INFO, A, LDA
     $,
     $                       AFB, LDAFB )
                     ELSE
                        RPVGRW = AB_SLA_GBRPVGRW(N, KL, KU, N, A, LDA,
     $                       AFB, LDAFB )
                     ENDIF
...
*                       residual.
*
                        CALL AB_SGBT01( N, N, KL, KU, A, LDA, AFB, LDAFB
     $,
     $                               IWORK, WORK,
     $                               RESULT( 1 ) )
                        K1 = 1
...
*                       Compute residual of the computed solution.
*
                        CALL AB_SLACPY( 'Full', N, NRHS, BSAV, LDB, WORK
     $,
     $                               LDB )
                        CALL AB_SGBT02( TRANS, N, N, KL, KU, NRHS, ASAV,
     $                               LDA, X, LDB, WORK, LDB,
     $                               RESULT( 2 ) )
*
...
*
     ...
     $                                  ROLDC, RESULT( 3 ) )
                     ...
                     END IF
*
*                    Compare RCOND from AB_AB_AB_SGBSVXX with the computed value
*                    in RCONDC.
*
                     RESULT( 6 ) = AB_SGET06( RCOND, RCONDC )
*
...
*
     ...
     $                           CALL AB_ALADHD( NOUT, PATH )
                              IF( PREFAC ) THEN
                                 WRITE( NOUT, FMT = 9995 )'AB_AB_AB_SGBS
     $VXX',
     $                                FACT, TRANS, N, KL, KU, EQUED,
     $                                IMAT, K, RESULT( K )
                              ELSE
                                 WRITE( NOUT, FMT = 9996 )'AB_AB_AB_SGBS
     $VXX',
 ...
 45                     CONTINUE
     ...
     $                        CALL AB_ALADHD( NOUT, PATH )
                           IF( PREFAC ) THEN
                              WRITE( NOUT, FMT = 9995 )'AB_AB_AB_SGBSVXX
     $', FACT,
     $                             TRANS, N, KL, KU, EQUED, IMAT, 1,
     $                             RESULT( 1 )
                           ELSE
                              WRITE( NOUT, FMT = 9996 )'AB_AB_AB_SGBSVXX
     $', FACT,
...
*     Test Error Bounds from AB_AB_AB_SGBSVXX
