...
*> \verbatim
*>
*> AB_ZDRVPO tests the driver routines AB_ZPOSV, -SVX, and -SVXX.
*>
*> Note that this file is used only when the XBLAS are available,
*> otherwise AB_ZDRVPO.f defines this subroutine.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*     .. External Subroutines ..
     ...
     $B_ZPOT01, AB_ZPOT02,
     $                   AB_ZPOT05, AB_ZPOTRF, AB_ZPOTRI, AB_AB_AB_ZPOSV
     $XX
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
*                    Solve the system and compute the condition number
*                    and error bounds using AB_AB_AB_ZPOSVXX.
*
                     SRNAMT = 'AB_AB_AB_ZPOSVXX'
                     N_ERR_BNDS = 3
                     CALL AB_AB_AB_ZPOSVXX( FACT, UPLO, N, NRHS, A, LDA,
     $ AFAC,
     $                    LDA, EQUED, S, B, LDA, X,
     ...
     $                    RWORK( 2*NRHS+1 ), INFO )
*
*                    Check the error code from AB_AB_AB_ZPOSVXX.
*
                     IF( INFO.EQ.N+1 ) GOTO 90
                     IF( INFO.NE.IZERO ) THEN
                        CALL AB_ALAERH( PATH, 'AB_AB_AB_ZPOSVXX', INFO, 
     $IZERO,
     $                               FACT // UPLO, N, N, -1, -1, NRHS,
     $                               IMAT, NFAIL, NERRS, NOUT )
                        GO TO 90
...
*
     ...
     $                               RWORK( NRHS+1 ), RESULT( 4 ) )
                     ...
                     END IF
*
*                    Compare RCOND from AB_AB_AB_ZPOSVXX with the computed value
*                    in RCONDC.
*
                     RESULT( 6 ) = AB_DGET06( RCOND, RCONDC )
*
...
*
     ...
     $                        CALL AB_ALADHD( NOUT, PATH )
                           IF( PREFAC ) THEN
                              WRITE( NOUT, FMT = 9997 )'AB_AB_AB_ZPOSVXX
     $', FACT,
     $                           UPLO, N, EQUED, IMAT, K, RESULT( K )
                           ELSE
                              WRITE( NOUT, FMT = 9998 )'AB_AB_AB_ZPOSVXX
     $', FACT,
     $                           UPLO, N, IMAT, K, RESULT( K )
                           END IF
                           NFAIL = NFAIL + 1
