...
*  ===========
*
*       SUBROUTINE AB_SLAVSP( UPLO, TRANS, DIAG, N, NRHS, A, IPIV, B, LDB,
*                          INFO )
*
*       .. Scalar Arguments ..
...
*> \verbatim
*>
*> AB_SLAVSP  performs one of the matrix-vector operations
*>    x := A*x  or  x := A'*x,
*> where x is an N element vector and  A is one of the factors
*> from the block U*D*U' or L*D*L' factorization computed by AB_SSPTRF.
*>
...
*>          matrix as computed by AB_SSPTRF.
*> \endverbatim
*>
*> \param[in] IPIV
*> \verbatim
...
*
               IF( NOUNIT )
     $            CALL AB_SSCAL( NRHS, A( KC+K-1 ), B( K, 1 ), LDB )
*
...
*
                  CALL AB_SGER( K-1, NRHS, ONE, A( KC ), 1, B( K, 1 ), L
     $DB,
     $                       B( 1, 1 ), LDB )
*
...
*
     ...
     $DB,
     $                       B( 1, 1 ), LDB )
                  CALL AB_SGER( K-1, NRHS, ONE, A( KCNEXT ), 1,
...
*
               IF( NOUNIT )
     $            CALL AB_SSCAL( NRHS, A( KC ), B( K, 1 ), LDB )
*
...
*                 Apply the transformation.
*
                  CALL AB_SGER( N-K, NRHS, ONE, A( KC+1 ), 1, B( K, 1 ),
     $                       LDB, B( K+1, 1 ), LDB )
*
...
*
                  CALL AB_SGER( N-K, NRHS, ONE, A( KC+1 ), 1, B( K, 1 ),
     $                       LDB, B( K+1, 1 ), LDB )
                  CALL AB_SGER( N-K, NRHS, ONE, A( KCNEXT+2 ), 1,
