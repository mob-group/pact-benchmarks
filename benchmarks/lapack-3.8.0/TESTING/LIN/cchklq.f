...
*  ===========
*
*       SUBROUTINE AB_CCHKLQ( DOTYPE, NM, MVAL, NN, NVAL, NNB, NBVAL, NXVAL,
...
*> \verbatim
*>
*> AB_CCHKLQ tests AB_AB_CGELQF, AB_CUNGLQ and AB_CUNMLQ.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*
*  =====================================================================
      SUBROUTINE AB_CCHKLQ( DOTYPE, NM, MVAL, NN, NVAL, NNB, NBVAL, NXVA
     $L,
     $                   NRHS, THRESH, TSTERR, NMAX, A, AF, AQ, AL, AC,
     $                   B, X, XACT, TAU, WORK, RWORK, NOUT )
*
...
*     .. External Subroutines ..
      EXTERNAL           AB_ALAERH, AB_ALAHD, AB_ALASUM, AB_CERRLQ, AB_A
     $B_CGELQS, AB_CGET02,
...
*              corresponding to the call of AB_CLQT01; other values are
*              used in the calls of AB_CLQT02, and must not exceed MINMN.
*
               KVAL( 1 ) = MINMN
               KVAL( 2 ) = 0
               KVAL( 3 ) = 1
...
*                       Test AB_AB_CGELQF
*
                        CALL AB_CLQT01( M, N, A, AF, AQ, AL, LDA, TAU,
     $                               WORK, LWORK, RWORK, RESULT( 1 ) )
                     ELSE IF( M.LE.N ) THEN
*
*                       Test AB_CUNGLQ, using factorization
*                       returned by AB_CLQT01
*
                        CALL AB_CLQT02( M, N, K, A, AF, AQ, AL, LDA, TAU
     $,
     $                               WORK, LWORK, RWORK, RESULT( 1 ) )
                     END IF
                     IF( M.GE.K ) THEN
*
*                       Test AB_CUNMLQ, using factorization returned
*                       by AB_CLQT01
*
                        CALL AB_CLQT03( M, N, K, AF, AC, AL, AQ, LDA, TA
     $U,
     $                               WORK, LWORK, RWORK, RESULT( 3 ) )
                        NT = NT + 4
*
*                       If M>=N and K=N, call AB_AB_CGELQS to solve a system
...
*
     ...
     $                                  LDA )
                           SRNAMT = 'AB_AB_CGELQS'
                           CALL AB_AB_CGELQS( M, N, NRHS, AF, LDA, TAU, 
     $X,
     $                                  LDA, WORK, LWORK, INFO )
*
*                          Check error code from AB_AB_CGELQS.
*
                           IF( INFO.NE.0 )
     $                        CALL AB_ALAERH( PATH, 'AB_AB_CGELQS', INFO
     $, 0, ' ',
     $                                     M, N, NRHS, -1, NB, IMAT,
     $                                     NFAIL, NERRS, NOUT )
*
