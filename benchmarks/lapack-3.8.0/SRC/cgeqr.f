...
*  ===========
*
*       SUBROUTINE AB_CGEQR( M, N, A, LDA, T, TSIZE, WORK, LWORK,
*                         INFO )
*
*       .. Scalar Arguments ..
...
*>
*> \verbatim
*> AB_CGEQR computes a QR factorization of an M-by-N matrix A.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*     .. External Subroutines ..
      EXTERNAL           AB_CLATSQR, AB_AB_CGEQRT, AB_XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( MIN( M, N ).GT.0 ) THEN
        MB = AB_ILAENV( 1, 'AB_CGEQR ', ' ', M, N, 1, -1 )
        NB = AB_ILAENV( 1, 'AB_CGEQR ', ' ', M, N, 2, -1 )
      ELSE
        MB = M
        NB = 1
      END IF
...
*
      IF( ( M.LE.N ) .OR. ( MB.LE.N ) .OR. ( MB.GE.M ) ) THEN
        CALL AB_AB_CGEQRT( M, N, NB, A, LDA, T( 6 ), NB, WORK, INFO )
      ELSE
        CALL AB_CLATSQR( M, N, MB, NB, A, LDA, T( 6 ), NB, WORK,
     $                LWORK, INFO )
      END IF
*
      WORK( 1 ) = MAX( 1, NB*N )
