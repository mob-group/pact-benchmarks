*> \brief <b> AB_AB_ZGELSD computes the minimum-norm solution to a linear least squares problem for GE matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download AB_AB_ZGELSD + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_AB_ZGELSD.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_AB_ZGELSD.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_AB_ZGELSD.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE AB_AB_ZGELSD( M, N, NRHS, A, LDA, B, LDB, S, RCOND, RANK,
...
*> \verbatim
*>
*> AB_AB_ZGELSD computes the minimum-norm solution to a real linear least
*> squares problem:
...
*
*  =====================================================================
      SUBROUTINE AB_AB_ZGELSD( M, N, NRHS, A, LDA, B, LDB, S, RCOND, RAN
     $K,
     $                   WORK, LWORK, RWORK, IWORK, INFO )
*
...
*     .. External Subroutines ..
     ...
     $ZGEBRD, AB_AB_ZGELQF,
     $                   AB_AB_ZGEQRF, AB_ZLACPY, AB_ZLALSD, AB_ZLASCL, 
     $AB_ZLASET, AB_ZUNMBR,
     $                   AB_ZUNMLQ, AB_ZUNMQR
*     ..
*     .. External Functions ..
...
*
      IF( INFO.EQ.0 ) THEN
         ...
         LRWORK = 1
         IF( MINMN.GT.0 ) THEN
            SMLSIZ = AB_ILAENV( 9, 'AB_AB_ZGELSD', ' ', 0, 0, 0, 0 )
            MNTHR = AB_ILAENV( 6, 'AB_AB_ZGELSD', ' ', M, N, NRHS, -1 )
...
*
               MM = N
               MAXWRK = MAX( MAXWRK, N*AB_ILAENV( 1, 'AB_AB_ZGEQRF', ' '
     $, M, N,
     $                       -1, -1 ) )
               MAXWRK = MAX( MAXWRK, NRHS*AB_ILAENV( 1, 'AB_ZUNMQR', 'LC
     $', M,
     $                       NRHS, N, -1 ) )
            END IF
            IF( M.GE.N ) THEN
...
*
     ...
     $                       'AB_ZGEBRD', ' ', MM, N, -1, -1 ) )
               MAXWRK = MAX( MAXWRK, 2*N + NRHS*AB_ILAENV( 1, 'AB_ZUNMBR
     $',
     $                       'QLC', MM, NRHS, N, -1 ) )
               MAXWRK = MAX( MAXWRK, 2*N + ( N - 1 )*AB_ILAENV( 1,
     $                       'AB_ZUNMBR', 'PLN', N, NRHS, N, -1 ) )
...
*
                  MAXWRK = 2*M + ( N + M )*AB_ILAENV( 1, 'AB_ZGEBRD', ' 
     $', M,
     $                     N, -1, -1 )
                  MAXWRK = MAX( MAXWRK, 2*M + NRHS*AB_ILAENV( 1, 'AB_ZUN
     $MBR',
     $                          'QLC', M, NRHS, M, -1 ) )
                  MAXWRK = MAX( MAXWRK, 2*M + M*AB_ILAENV( 1, 'AB_ZUNMBR
     $',
     $                          'PLN', N, NRHS, M, -1 ) )
...
*
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_ZGELSD', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
            CALL AB_AB_ZGEQRF( M, N, A, LDA, WORK( ITAU ), WORK( NWORK )
     $,
     $                   LWORK-NWORK+1, INFO )
*
...
*
         CALL AB_ZLALSD( 'U', SMLSIZ, N, NRHS, S, RWORK( IE ), B, LDB,
...
*
         CALL AB_ZUNMBR( 'P', 'L', 'N', N, NRHS, N, A, LDA, WORK( ITAUP 
     $),
     $                B, LDB, WORK( NWORK ), LWORK-NWORK+1, INFO )
*
...
*
         CALL AB_AB_ZGELQF( M, N, A, LDA, WORK( ITAU ), WORK( NWORK ),
     $                LWORK-NWORK+1, INFO )
         IL = NWORK
*
...
*        Solve the bidiagonal least squares problem.
*
         CALL AB_ZLALSD( 'U', SMLSIZ, M, NRHS, S, RWORK( IE ), B, LDB,
     $                RCOND, RANK, WORK( NWORK ), RWORK( NRWORK ),
     $                IWORK, INFO )
         IF( INFO.NE.0 ) THEN
            GO TO 10
...
*
         CALL AB_ZUNMBR( 'P', 'L', 'N', M, NRHS, M, WORK( IL ), LDWORK,
...
*        Solve the bidiagonal least squares problem.
*
         CALL AB_ZLALSD( 'L', SMLSIZ, M, NRHS, S, RWORK( IE ), B, LDB,
     $                RCOND, RANK, WORK( NWORK ), RWORK( NRWORK ),
     $                IWORK, INFO )
         IF( INFO.NE.0 ) THEN
            GO TO 10
...
*
         CALL AB_ZUNMBR( 'P', 'L', 'N', N, NRHS, M, A, LDA, WORK( ITAUP 
     $),
     $                B, LDB, WORK( NWORK ), LWORK-NWORK+1, INFO )
*
      END IF
