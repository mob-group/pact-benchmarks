...
*
*> \htmlonly
*> Download AB_ZHBGST + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_ZHBGST.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_ZHBGST.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_ZHBGST.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE AB_ZHBGST( VECT, UPLO, N, KA, KB, AB, LDAB, BB, LDBB, X,
*                          LDX, WORK, RWORK, INFO )
*
*       .. Scalar Arguments ..
*       CHARACTER          UPLO, VECT
...
*> \verbatim
*>
*> AB_ZHBGST reduces a complex Hermitian-definite banded generalized
*> eigenproblem  A*x = lambda*B*x  to standard form  C*y = lambda*y,
*> such that C has the same bandwidth as A.
*>
*> B must have been previously factorized as S**H*S by AB_ZPBSTF, using a
...
*>          B, as returned by AB_ZPBSTF, stored in the first kb+1 rows of
*>          the array.
*> \endverbatim
*>
*> \param[in] LDBB
...
*     .. External Subroutines ..
      EXTERNAL           AB_XERBLA, ZAB_DSCAL, AB_ZGERC, AB_ZGERU, AB_ZL
     $ACGV, AB_ZLAR2V,
     $                   AB_ZLARGV, AB_ZLARTG, AB_ZLARTV, AB_ZLASET, AB_
     $ZROT
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( WANTX )
     $   CALL AB_ZLASET( 'Full', N, N, CZERO, CONE, X, LDX )
*
*     Set M to the splitting point m. It must be the same value as is
*     used in AB_ZPBSTF. The chosen value allows the arrays WORK and RWORK
...
*
               CALL ZAB_DSCAL( N-M, ONE / BII, X( M+1, I ), 1 )
               IF( KBT.GT.0 )
     $            CALL AB_ZGERC( N-M, KBT, -CONE, X( M+1, I ), 1,
...
*
                  CALL AB_ZLARTG( AB( K+1, I-K+KA ), RA1,
...
*
            IF( NRT.GT.0 )
     $         CALL AB_ZLARGV( NRT, AB( 1, J2T ), INCA, WORK( J2T-M ), K
     $A1,
     $                      RWORK( J2T-M ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 100 L = 1, KA - 1
                  CALL AB_ZLARTV( NR, AB( KA1-L, J2 ), INCA,
...
*              blocks
*
               CALL AB_ZLAR2V( NR, AB( KA1, J2 ), AB( KA1, J2+1 ),
     $                      AB( KA, J2+1 ), INCA, RWORK( J2-M ),
     $                      WORK( J2-M ), KA1 )
*
               CALL AB_ZLACGV( NR, WORK( J2-M ), KA1 )
            END IF
*
...
*
            DO 110 L = KA - 1, KB - K + 1, -1
               NRT = ( N-J2+L ) / KA1
               IF( NRT.GT.0 )
     $            CALL AB_ZLARTV( NRT, AB( L, J2+KA1-L ), INCA,
...
*
               DO 120 J = J2, J1, KA1
                  CALL AB_ZROT( N-M, X( M+1, J ), 1, X( M+1, J+1 ), 1,
...
*
            DO 140 L = KB - K, 1, -1
               NRT = ( N-J2+KA+L ) / KA1
               IF( NRT.GT.0 )
     $            CALL AB_ZLARTV( NRT, AB( L, J2-L+1 ), INCA,
...
*
               CALL AB_ZLARGV( NR, AB( 1, J2 ), INCA, WORK( J2 ), KA1,
     $                      RWORK( J2 ), KA1 )
*
...
*
               CALL ZAB_DSCAL( N-M, ONE / BII, X( M+1, I ), 1 )
               IF( KBT.GT.0 )
     $            CALL AB_ZGERU( N-M, KBT, -CONE, X( M+1, I ), 1,
...
*
                  CALL AB_ZLARTG( AB( KA1-K, I ), RA1, RWORK( I-K+KA-M )
     $,
     $                         WORK( I-K+KA-M ), RA )
*
...
*
            IF( NRT.GT.0 )
     $         CALL AB_ZLARGV( NRT, AB( KA1, J2T-KA ), INCA, WORK( J2T-M
     $ ),
     $                      KA1, RWORK( J2T-M ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 330 L = 1, KA - 1
                  CALL AB_ZLARTV( NR, AB( L+1, J2-L ), INCA,
...
*              blocks
*
               CALL AB_ZLAR2V( NR, AB( 1, J2 ), AB( 1, J2+1 ), AB( 2, J2
     $ ),
...
*
            DO 340 L = KA - 1, KB - K + 1, -1
               NRT = ( N-J2+L ) / KA1
               IF( NRT.GT.0 )
     $            CALL AB_ZLARTV( NRT, AB( KA1-L+1, J2 ), INCA,
...
*
            DO 370 L = KB - K, 1, -1
               NRT = ( N-J2+KA+L ) / KA1
               IF( NRT.GT.0 )
     $            CALL AB_ZLARTV( NRT, AB( KA1-L+1, J2-KA ), INCA,
...
*              which have been created outside the band
*
               CALL AB_ZLARGV( NR, AB( KA1, J2-KA ), INCA, WORK( J2 ), K
     $A1,
     $                      RWORK( J2 ), KA1 )
*
...
*              post-multiply X by inv(S(i))
*
               CALL ZAB_DSCAL( NX, ONE / BII, X( 1, I ), 1 )
               IF( KBT.GT.0 )
     $            CALL AB_ZGERU( NX, KBT, -CONE, X( 1, I ), 1,
...
*                 generate rotation to annihilate a(i+k-ka-1,i)
*
                  CALL AB_ZLARTG( AB( K+1, I ), RA1, RWORK( I+K-KA ),
     $                         WORK( I+K-KA ), RA )
*
*                 create nonzero element a(i+k-ka-1,i+k) outside the
...
*
            IF( NRT.GT.0 )
     $         CALL AB_ZLARGV( NRT, AB( 1, J1+KA ), INCA, WORK( J1 ), KA
     $1,
     $                      RWORK( J1 ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 580 L = 1, KA - 1
                  CALL AB_ZLARTV( NR, AB( KA1-L, J1+L ), INCA,
...
*              blocks
*
               CALL AB_ZLAR2V( NR, AB( KA1, J1 ), AB( KA1, J1-1 ),
     $                      AB( KA, J1 ), INCA, RWORK( J1 ), WORK( J1 ),
     $                      KA1 )
*
               CALL AB_ZLACGV( NR, WORK( J1 ), KA1 )
            END IF
*
*           start applying rotations in 1st set from the right
*
            DO 590 L = KA - 1, KB - K + 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )
     $            CALL AB_ZLARTV( NRT, AB( L, J1T ), INCA,
...
*
               DO 600 J = J1, J2, KA1
                  CALL AB_ZROT( NX, X( 1, J ), 1, X( 1, J-1 ), 1,
     $                       RWORK( J ), WORK( J ) )
  600          CONTINUE
            END IF
  610    CONTINUE
...
*
            DO 620 L = KB - K, 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )
     $            CALL AB_ZLARTV( NRT, AB( L, J1T+KA ), INCA,
...
*              which have been created outside the band
*
               CALL AB_ZLARGV( NR, AB( 1, J1+KA ), INCA, WORK( M-KB+J1 )
     $,
...
*
     ...
     $                      WORK( M-KB+J1 ), KA1 )
*
               CALL AB_ZLACGV( NR, WORK( M-KB+J1 ), KA1 )
            END IF
*
*           start applying rotations in 2nd set from the right
*
...
*
               CALL ZAB_DSCAL( NX, ONE / BII, X( 1, I ), 1 )
               IF( KBT.GT.0 )
     $            CALL AB_ZGERC( NX, KBT, -CONE, X( 1, I ), 1, BB( 2, I 
     $),
     $                        1, X( 1, I+1 ), LDX )
            END IF
*
...
*                 generate rotation to annihilate a(i,i+k-ka-1)
*
                  CALL AB_ZLARTG( AB( KA1-K, I+K-KA ), RA1,
...
*
            IF( NRT.GT.0 )
     $         CALL AB_ZLARGV( NRT, AB( KA1, J1 ), INCA, WORK( J1 ), KA1
     $,
     $                      RWORK( J1 ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 810 L = 1, KA - 1
                  CALL AB_ZLARTV( NR, AB( L+1, J1 ), INCA, AB( L+2, J1-1
     $ ),
...
*              blocks
*
               CALL AB_ZLAR2V( NR, AB( 1, J1 ), AB( 1, J1-1 ),
...
*
            DO 820 L = KA - 1, KB - K + 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )
     $            CALL AB_ZLARTV( NRT, AB( KA1-L+1, J1T-KA1+L ), INCA,
...
*
            DO 850 L = KB - K, 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )
     $            CALL AB_ZLARTV( NRT, AB( KA1-L+1, J1T+L-1 ), INCA,
...
*              which have been created outside the band
*
               CALL AB_ZLARGV( NR, AB( KA1, J1 ), INCA, WORK( M-KB+J1 ),
     $                      KA1, RWORK( M-KB+J1 ), KA1 )
*
*              apply rotations in 2nd set from the right
*
