*> \brief <b> AB_ZGEEV computes the eigenvalues and, optionally, the left and/or right eigenvectors for GE matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download AB_ZGEEV + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_ZGEEV.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_ZGEEV.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_ZGEEV.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
...
*> \verbatim
*>
*> AB_ZGEEV computes for an N-by-N complex nonsymmetric matrix A, the
...
*
*  =====================================================================
      SUBROUTINE AB_ZGEEV( JOBVL, JOBVR, N, A, LDA, W, VL, LDVL, VR, LDV
     $R,
     $                  WORK, LWORK, RWORK, INFO )
      implicit none
*
...
*     .. External Subroutines ..
      EXTERNAL           AB_DLABAD, AB_XERBLA, ZAB_DSCAL, AB_ZGEBAK, AB_
     $ZGEBAL, AB_ZGEHRD,
     $                   AB_ZHSEQR, AB_ZLACPY, AB_ZLASCL, AB_ZSCAL, AB_A
     $B_ZTREVC3, AB_ZUNGHR
*     ..
*     .. External Functions ..
      LOGICAL            AB_LSAME
...
*
     ...
     $GHR',
     $                       ' ', N, 1, N, -1 ) )
               CALL AB_AB_ZTREVC3( 'L', 'B', SELECT, N, A, LDA,
     $                       VL, LDVL, VR, LDVR,
     ...
     $GHR',
     $                       ' ', N, 1, N, -1 ) )
               CALL AB_AB_ZTREVC3( 'R', 'B', SELECT, N, A, LDA,
     $                       VL, LDVL, VR, LDVR,
     $                       N, NOUT, WORK, -1, RWORK, -1, IERR )
               LWORK_TREVC = INT( WORK(1) )
               MAXWRK = MAX( MAXWRK, N + LWORK_TREVC )
...
*
      IBAL = 1
      CALL AB_ZGEBAL( 'B', N, A, LDA, ILO, IHI, RWORK( IBAL ), IERR )
*
...
*
         IWRK = ITAU
         CALL AB_ZHSEQR( 'E', 'N', N, ILO, IHI, A, LDA, W, VR, LDVR,
     $                WORK( IWRK ), LWORK-IWRK+1, INFO )
      END IF
*
...
*
         IRWORK = IBAL + N
         CALL AB_AB_ZTREVC3( SIDE, 'B', SELECT, N, A, LDA, VL, LDVL, VR,
     $ LDVR,
...
*        (RWorkspace: need N)
*
         CALL AB_ZGEBAK( 'B', 'L', N, ILO, IHI, RWORK( IBAL ), N, VL, LD
     $VL,
     $                IERR )
*
...
*
         DO 20 I = 1, N
            ...
            CALL ZAB_DSCAL( N, SCL, VL( 1, I ), 1 )
            DO 10 K = 1, N
               RWORK( IRWORK+K-1 ) = DBLE( VL( K, I ) )**2 +
     $                               AIMAG( VL( K, I ) )**2
   10       CONTINUE
            K = AB_IDAMAX( N, RWORK( IRWORK ), 1 )
...
*        (RWorkspace: need N)
*
         CALL AB_ZGEBAK( 'B', 'R', N, ILO, IHI, RWORK( IBAL ), N, VR, LD
     $VR,
     $                IERR )
*
