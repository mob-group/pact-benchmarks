...
*  ===========
*
*       SUBROUTINE AB_DGEQR( M, N, A, LDA, T, TSIZE, WORK, LWORK,
*                         INFO )
*
*       .. Scalar Arguments ..
...
*>
*> \verbatim
*> AB_DGEQR computes a QR factorization of an M-by-N matrix A.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*     .. External Subroutines ..
      EXTERNAL           AB_DLATSQR, AB_AB_DGEQRT, AB_XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( MIN( M, N ).GT.0 ) THEN
        MB = AB_ILAENV( 1, 'AB_DGEQR ', ' ', M, N, 1, -1 )
        NB = AB_ILAENV( 1, 'AB_DGEQR ', ' ', M, N, 2, -1 )
      ELSE
        MB = M
        NB = 1
      END IF
...
*
      IF( ( M.LE.N ) .OR. ( MB.LE.N ) .OR. ( MB.GE.M ) ) THEN
        CALL AB_AB_DGEQRT( M, N, NB, A, LDA, T( 6 ), NB, WORK, INFO )
      ELSE
        CALL AB_DLATSQR( M, N, MB, NB, A, LDA, T( 6 ), NB, WORK,
     $                LWORK, INFO )
      END IF
*
      WORK( 1 ) = MAX( 1, NB*N )
