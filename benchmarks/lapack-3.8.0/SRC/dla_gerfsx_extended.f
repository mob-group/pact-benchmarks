*> \brief \b AB_DLA_GERFSX_EXTENDED improves the computed solution to a system of linear equations for general matrices by performing extra-precise iterative refinement and provides error bounds and backward error estimates for the solution.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download AB_DLA_GERFSX_EXTENDED + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_DLA_GERFSX_EXTENDED.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_DLA_GERFSX_EXTENDED.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_DLA_GERFSX_EXTENDED.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE AB_DLA_GERFSX_EXTENDED( PREC_TYPE, TRANS_TYPE, N, NRHS, A,
...
*>
*>
*> AB_DLA_GERFSX_EXTENDED improves the computed solution to a system of
*> linear equations by performing extra-precise iterative refinement
*> and provides error bounds and backward error estimates for the solution.
*> This subroutine is called by AB_AB_AB_DGERFSX to perform iterative refinement.
...
*>     right-hand side.
*>
*>     The AB_SECOND index in ERRS_N(:,err) contains the following
*>     three fields:
...
*>     right-hand side.
*>
*>     The AB_SECOND index in ERRS_C(:,err) contains the following
*>     three fields:
*>     err = 1 "Trust/don't trust" boolean. Trust the answer if the
*>              reciprocal condition number is less than the threshold
*>              sqrt(n) * AB_SLAMCH('Epsilon').
...
*>          INFO is INTEGER
*>       = 0:  Successful exit.
*>       < 0:  if INFO = -i, the ith argument to AB_DGETRS had an illegal
*>             value
*> \endverbatim
*
*  Authors:
...
*
*  =====================================================================
      SUBROUTINE AB_DLA_GERFSX_EXTENDED( PREC_TYPE, TRANS_TYPE, N, NRHS,
     $ A,
     $                                LDA, AF, LDAF, IPIV, COLEQU, C, B,
     $                                LDB, Y, LDY, BERR_OUT, N_NORMS,
     $                                ERRS_N, ERRS_C, RES, AYB, DY,
...
*     .. External Subroutines ..
      EXTERNAL           AB_DAXPY, AB_DCOPY, AB_DGETRS, AB_DGEMV, BLAS_A
     $B_DGEMV_X,
     $                   BLAS_AB_DGEMV2_X, AB_DLA_GEAMV, AB_DLA_WWADDW, 
     $AB_DLAMCH,
...
*
            ...
            IF ( Y_PREC_STATE .EQ. BASE_RESIDUAL ) THEN
               CALL AB_DGEMV( TRANS, N, N, -1.0D+0, A, LDA, Y( 1, J ), 1
     $,
     $              1.0D+0, RES, 1 )
            ELSE IF ( Y_PREC_STATE .EQ. EXTRA_RESIDUAL ) THEN
               CALL BLAS_AB_DGEMV_X( TRANS_TYPE, N, N, -1.0D+0, A, LDA,
     $              Y( 1, J ), 1, 1.0D+0, RES, 1, PREC_TYPE )
            ELSE
               CALL BLAS_AB_DGEMV2_X( TRANS_TYPE, N, N, -1.0D+0, A, LDA,
     $              Y( 1, J ), Y_TAIL, 1, 1.0D+0, RES, 1, PREC_TYPE )
            END IF

!        XXX: RES is no longer needed.
            CALL AB_DCOPY( N, RES, 1, DY, 1 )
            CALL AB_DGETRS( TRANS, N, 1, AF, LDAF, IPIV, DY, N, INFO )
*
...
*
         CALL AB_DCOPY( N, B( 1, J ), 1, RES, 1 )
         CALL AB_DGEMV( TRANS, N, N, -1.0D+0, A, LDA, Y(1,J), 1, 1.0D+0,
     $     RES, 1 )
...
*
         CALL AB_DLA_GEAMV ( TRANS_TYPE, N, N, 1.0D+0,
