...
*     .. External Subroutines ..
      EXTERNAL           AB_CLASR, AB_CSCAL, AB_CSWAP, AB_AB_SLARTGP, AB
...
*
         IF( B11D(IMIN)**2+B11BULGE**2 .GT. THRESH**2 ) THEN
            CALL AB_AB_SLARTGP( B11BULGE, B11D(IMIN), RWORK(IU1SN+IMIN-1
     $),
     ...
     $                    RWORK(IU1CS+IMIN-1), RWORK(IU1SN+IMIN-1) )
         ...
         IF( B21D(IMIN)**2+B21BULGE**2 .GT. THRESH**2 ) THEN
            CALL AB_AB_SLARTGP( B21BULGE, B21D(IMIN), RWORK(IU2SN+IMIN-1
     $),
...
*
            IF( .NOT. RESTART11 .AND. .NOT. RESTART21 ) THEN
               CALL AB_AB_SLARTGP( X2, X1, RWORK(IV1TSN+I-1),
     $                       RWORK(IV1TCS+I-1), R )
            ELSE IF( .NOT. RESTART11 .AND. RESTART21 ) THEN
               CALL AB_AB_SLARTGP( B11BULGE, B11E(I-1), RWORK(IV1TSN+I-1
     $),
     $                       RWORK(IV1TCS+I-1), R )
            ELSE IF( RESTART11 .AND. .NOT. RESTART21 ) THEN
               CALL AB_AB_SLARTGP( B21BULGE, B21E(I-1), RWORK(IV1TSN+I-1
     $),
     $                       RWORK(IV1TCS+I-1), R )
            ELSE IF( MU .LE. NU ) THEN
               CALL AB_AB_SLARTGS( B11D(I), B11E(I), MU, RWORK(IV1TCS+I-
     $1),
     $                       RWORK(IV1TSN+I-1) )
            ELSE
               CALL AB_AB_SLARTGS( B21D(I), B21E(I), NU, RWORK(IV1TCS+I-
     $1),
     $                       RWORK(IV1TSN+I-1) )
            END IF
            ...
            IF( .NOT. RESTART12 .AND. .NOT. RESTART22 ) THEN
               CALL AB_AB_SLARTGP( Y2, Y1, RWORK(IV2TSN+I-1-1),
     $                       RWORK(IV2TCS+I-1-1), R )
            ELSE IF( .NOT. RESTART12 .AND. RESTART22 ) THEN
               CALL AB_AB_SLARTGP( B12BULGE, B12D(I-1), RWORK(IV2TSN+I-1
     $-1),
     $                       RWORK(IV2TCS+I-1-1), R )
            ELSE IF( RESTART12 .AND. .NOT. RESTART22 ) THEN
               CALL AB_AB_SLARTGP( B22BULGE, B22D(I-1), RWORK(IV2TSN+I-1
     $-1),
     $                       RWORK(IV2TCS+I-1-1), R )
            ELSE IF( NU .LT. MU ) THEN
               CALL AB_AB_SLARTGS( B12E(I-1), B12D(I), NU,
     $                       RWORK(IV2TCS+I-1-1), RWORK(IV2TSN+I-1-1) )
            ELSE
               CALL AB_AB_SLARTGS( B22E(I-1), B22D(I), MU,
     $                       RWORK(IV2TCS+I-1-1), RWORK(IV2TSN+I-1-1) )
            END IF
*
...
*
            IF( .NOT. RESTART11 .AND. .NOT. RESTART12 ) THEN
               CALL AB_AB_SLARTGP( X2, X1, RWORK(IU1SN+I-1), RWORK(IU1CS
     $+I-1),
     $                       R )
            ELSE IF( .NOT. RESTART11 .AND. RESTART12 ) THEN
               CALL AB_AB_SLARTGP( B11BULGE, B11D(I), RWORK(IU1SN+I-1),
     $                       RWORK(IU1CS+I-1), R )
            ELSE IF( RESTART11 .AND. .NOT. RESTART12 ) THEN
               CALL AB_AB_SLARTGP( B12BULGE, B12E(I-1), RWORK(IU1SN+I-1)
     $,
     $                       RWORK(IU1CS+I-1), R )
            ELSE IF( MU .LE. NU ) THEN
               CALL AB_AB_SLARTGS( B11E(I), B11D(I+1), MU, RWORK(IU1CS+I
     $-1),
     $                       RWORK(IU1SN+I-1) )
            ELSE
               CALL AB_AB_SLARTGS( B12D(I), B12E(I), NU, RWORK(IU1CS+I-1
     $),
     $                       RWORK(IU1SN+I-1) )
            END IF
            IF( .NOT. RESTART21 .AND. .NOT. RESTART22 ) THEN
               CALL AB_AB_SLARTGP( Y2, Y1, RWORK(IU2SN+I-1), RWORK(IU2CS
     $+I-1),
     $                       R )
            ELSE IF( .NOT. RESTART21 .AND. RESTART22 ) THEN
               CALL AB_AB_SLARTGP( B21BULGE, B21D(I), RWORK(IU2SN+I-1),
     $                       RWORK(IU2CS+I-1), R )
            ELSE IF( RESTART21 .AND. .NOT. RESTART22 ) THEN
               CALL AB_AB_SLARTGP( B22BULGE, B22E(I-1), RWORK(IU2SN+I-1)
     $,
     $                       RWORK(IU2CS+I-1), R )
            ELSE IF( NU .LT. MU ) THEN
               CALL AB_AB_SLARTGS( B21E(I), B21E(I+1), NU, RWORK(IU2CS+I
     $-1),
     $                       RWORK(IU2SN+I-1) )
            ELSE
               CALL AB_AB_SLARTGS( B22D(I), B22E(I), MU, RWORK(IU2CS+I-1
     $),
     $                       RWORK(IU2SN+I-1) )
            END IF
...
*
         IF( .NOT. RESTART12 .AND. .NOT. RESTART22 ) THEN
            CALL AB_AB_SLARTGP( Y2, Y1, RWORK(IV2TSN+IMAX-1-1),
     $                    RWORK(IV2TCS+IMAX-1-1), R )
         ELSE IF( .NOT. RESTART12 .AND. RESTART22 ) THEN
            CALL AB_AB_SLARTGP( B12BULGE, B12D(IMAX-1),
     $                    RWORK(IV2TSN+IMAX-1-1),
     $                    RWORK(IV2TCS+IMAX-1-1), R )
         ELSE IF( RESTART12 .AND. .NOT. RESTART22 ) THEN
            CALL AB_AB_SLARTGP( B22BULGE, B22D(IMAX-1),
     $                    RWORK(IV2TSN+IMAX-1-1),
     $                    RWORK(IV2TCS+IMAX-1-1), R )
         ELSE IF( NU .LT. MU ) THEN
