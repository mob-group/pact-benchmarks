*> \brief <b> AB_AB_AB_DGESVDX computes the singular value decomposition (SVD) for GE matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download AB_AB_AB_DGESVDX + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_AB_AB_DGESVDX.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_AB_AB_DGESVDX.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_AB_AB_DGESVDX.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*     SUBROUTINE AB_AB_AB_DGESVDX( JOBU, JOBVT, RANGE, M, N, A, LDA, VL, VU,
...
*> \verbatim
*>
*>  AB_AB_AB_DGESVDX computes the singular value decomposition (SVD) of a real
...
*>
*>  AB_AB_AB_DGESVDX uses an eigenvalue problem for obtaining the SVD, which
...
*
*  =====================================================================
      SUBROUTINE AB_AB_AB_DGESVDX( JOBU, JOBVT, RANGE, M, N, A, LDA, VL,
     $ VU,
     $                    IL, IU, NS, S, U, LDU, VT, LDVT, WORK,
     $                    LWORK, IWORK, INFO )
*
...
*     .. External Subroutines ..
      EXTERNAL           AB_DBDSVDX, AB_DGEBRD, AB_AB_DGELQF, AB_AB_DGEQ
     $RF, AB_DLACPY,
...
*
      IF( INFO.EQ.0 ) THEN
         ...
         IF( MINMN.GT.0 ) THEN
            IF( M.GE.N ) THEN
               MNTHR = AB_ILAENV( 6, 'AB_AB_DGESVD', JOBU // JOBVT, M, N
     $, 0, 0 )
               IF( M.GE.MNTHR ) THEN
*
*                 Path 1 (M much larger than N)
*
                  MAXWRK = N +
     $                     N*AB_ILAENV( 1, 'AB_AB_DGEQRF', ' ', M, N, -1
     $, -1 )
...
*
                  MAXWRK = M +
     $                     M*AB_ILAENV( 1, 'AB_AB_DGELQF', ' ', M, N, -1
     $, -1 )
...
*
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_AB_DGESVDX', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
            ITAU = 1
            ITEMP = ITAU + N
            CALL AB_AB_DGEQRF( M, N, A, LDA, WORK( ITAU ), WORK( ITEMP )
     $,
     $                   LWORK-ITEMP+1, INFO )
*
...
*
            ITAU = 1
            ITEMP = ITAU + M
            CALL AB_AB_DGELQF( M, N, A, LDA, WORK( ITAU ), WORK( ITEMP )
     $,
     $                   LWORK-ITEMP+1, INFO )
