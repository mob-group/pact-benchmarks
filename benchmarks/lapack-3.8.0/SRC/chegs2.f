...
*     .. External Subroutines ..
      EXTERNAL           AB_CAXPY, AB_AB_CHER2, AB_CLACGV, AB_CAB_SSCAL,
     $ AB_CTRMV, AB_CTRSV,
     $                   AB_XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
               ...
               A( K, K ) = AKK
               IF( K.LT.N ) THEN
                  CALL AB_CAB_SSCAL( N-K, ONE / BKK, A( K+1, K ), 1 )
                  CT = -HALF*AKK
                  CALL AB_CAXPY( N-K, CT, B( K+1, K ), 1, A( K+1, K ), 1
     $ )
                  CALL AB_AB_CHER2( UPLO, N-K, -CONE, A( K+1, K ), 1,
...
*
     ...
     $                     LDB, A( 1, K ), 1 )
               ...
               CALL AB_AB_CHER2( UPLO, K-1, CONE, A( 1, K ), 1, B( 1, K 
     $), 1,
     $                     A, LDA )
               CALL AB_CAXPY( K-1, CT, B( 1, K ), 1, A( 1, K ), 1 )
               CALL AB_CAB_SSCAL( K-1, BKK, A( 1, K ), 1 )
               A( K, K ) = AKK*BKK**2
   30       CONTINUE
         ELSE
*
...
*
     ...
     $                     B, LDB, A( K, 1 ), LDA )
               ...
               CALL AB_AB_CHER2( UPLO, K-1, CONE, A( K, 1 ), LDA, B( K, 
     $1 ),
     $                     LDB, A, LDA )
               CALL AB_CAXPY( K-1, CT, B( K, 1 ), LDB, A( K, 1 ), LDA )
               CALL AB_CLACGV( K-1, B( K, 1 ), LDB )
               CALL AB_CAB_SSCAL( K-1, BKK, A( K, 1 ), LDA )
               CALL AB_CLACGV( K-1, A( K, 1 ), LDA )
               A( K, K ) = AKK*BKK**2
   40       CONTINUE
         END IF
