...
*     .. External Subroutines ..
      EXTERNAL           AB_AB_DLARFG, AB_AB_DLARFX, AB_AB_DLARFY
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MOD
*     .. External Functions ..
...
*
   ...
   10         CONTINUE
              CTMP = ( A( OFDPOS, ST ) )
              CALL AB_AB_DLARFG( LM, CTMP, V( VPOS+1 ), 1, 
     $                                       TAU( TAUPOS ) )
              A( OFDPOS, ST ) = CTMP
*
              LM = ED - ST + 1
              CALL AB_AB_DLARFY( UPLO, LM, V( VPOS ), 1,
     $                     ( TAU( TAUPOS ) ),
...
*
          IF( TTYPE.EQ.2 ) THEN
              ...
              LM = J2-J1+1
              IF( LM.GT.0) THEN
                  CALL AB_AB_DLARFX( 'Left', LN, LM, V( VPOS ),
     $                         ( TAU( TAUPOS ) ),
...
*
   ...
   30             CONTINUE
                  CTMP = ( A( DPOS-NB, J1 ) )
                  CALL AB_AB_DLARFG( LM, CTMP, V( VPOS+1 ), 1, TAU( TAUP
     $OS ) )
                  A( DPOS-NB, J1 ) = CTMP
*                 
                  CALL AB_AB_DLARFX( 'Right', LN-1, LM, V( VPOS ),
     $                         TAU( TAUPOS ),
...
*
   ...
   20         CONTINUE
              CALL AB_AB_DLARFG( LM, A( OFDPOS, ST-1 ), V( VPOS+1 ), 1, 
     $                                       TAU( TAUPOS ) )
*
              LM = ED - ST + 1
*
...
*
              IF( LM.GT.0) THEN
                  CALL AB_AB_DLARFX( 'Right', LM, LN, V( VPOS ), 
...
*
                  ...
                  DO 40 I = 1, LM-1
                      ...
                      A( DPOS+NB+I, ST ) = ZERO
   40             CONTINUE
                  CALL AB_AB_DLARFG( LM, A( DPOS+NB, ST ), V( VPOS+1 ), 
     $1, 
     $                                        TAU( TAUPOS ) )
*
                  CALL AB_AB_DLARFX( 'Left', LM, LN-1, V( VPOS ), 
     $                         ( TAU( TAUPOS ) ),
