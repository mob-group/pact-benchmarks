*> \brief \b AB_AB_ZLASYF_ROOK computes a partial factorization of a complex symmetric matrix using the bounded Bunch-Kaufman ("rook") diagonal pivoting method.
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download AB_AB_ZLASYF_ROOK + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_AB_ZLASYF_ROOK.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_AB_ZLASYF_ROOK.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_AB_ZLASYF_ROOK.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE AB_AB_ZLASYF_ROOK( UPLO, N, NB, KB, A, LDA, IPIV, W, LDW, INFO )
*
*       .. Scalar Arguments ..
*       CHARACTER          UPLO
...
*> \verbatim
*>
*> AB_AB_ZLASYF_ROOK computes a partial factorization of a complex symmetric
...
*>
*> AB_AB_ZLASYF_ROOK is an auxiliary routine called by AB_AB_ZSYTRF_ROOK. It uses
...
*
*  =====================================================================
      SUBROUTINE AB_AB_ZLASYF_ROOK( UPLO, N, NB, KB, A, LDA, IPIV, W, LD
     $W,
     $                        INFO )
*
...
*     .. External Subroutines ..
      EXTERNAL           AB_ZCOPY, AB_ZGEMM, AB_ZGEMV, AB_ZSCAL, AB_ZSWA
     $P
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL AB_ZCOPY( K, A( 1, K ), 1, W( 1, KW ), 1 )
         IF( K.LT.N )
     $      CALL AB_ZGEMV( 'No transpose', K, N-K, -CONE, A( 1, K+1 ),
...
*
                  CALL AB_ZCOPY( IMAX, A( 1, IMAX ), 1, W( 1, KW-1 ), 1 
     $)
...
*
               CALL AB_ZCOPY( K-P, A( P+1, K ), 1, A( P, P+1 ), LDA )
               CALL AB_ZCOPY( P, A( 1, K ), 1, A( 1, P ), 1 )
*
*              Interchange rows K and P in last N-K+1 columns of A
*              and last N-K+2 columns of W
*
               CALL AB_ZSWAP( N-K+1, A( K, K ), LDA, A( P, K ), LDA )
               CALL AB_ZSWAP( N-KK+1, W( K, KKW ), LDW, W( P, KKW ), LDW
     $ )
            END IF
*
...
*
               A( KP, K ) = A( KK, K )
               CALL AB_ZCOPY( K-1-KP, A( KP+1, KK ), 1, A( KP, KP+1 ),
     $                     LDA )
               CALL AB_ZCOPY( KP, A( 1, KK ), 1, A( 1, KP ), 1 )
*
*              Interchange rows KK and KP in last N-KK+1 columns
*              of A and W
*
               CALL AB_ZSWAP( N-KK+1, A( KK, KK ), LDA, A( KP, KK ), LDA
     $ )
...
*
            IF( J.GE.2 )
     $         CALL AB_ZGEMM( 'No transpose', 'Transpose', J-1, JB,
...
*
         CALL AB_ZCOPY( N-K+1, A( K, K ), 1, W( K, K ), 1 )
         IF( K.GT.1 )
...
*
     ...
     $ 1)
                  CALL AB_ZCOPY( N-IMAX+1, A( IMAX, IMAX ), 1,
     $                        W( IMAX, K+1 ), 1 )
                  IF( K.GT.1 )
...
*              Copy non-updated column K to column P
*
               CALL AB_ZCOPY( P-K, A( K, K ), 1, A( P, K ), LDA )
               CALL AB_ZCOPY( N-P+1, A( P, K ), 1, A( P, P ), 1 )
*
*              Interchange rows K and P in first K columns of A
*              and first K+1 columns of W
*
               CALL AB_ZSWAP( K, A( K, 1 ), LDA, A( P, 1 ), LDA )
...
*
               A( KP, K ) = A( KK, K )
               CALL AB_ZCOPY( KP-K-1, A( K+1, KK ), 1, A( KP, K+1 ), LDA
     $ )
               CALL AB_ZCOPY( N-KP+1, A( KP, KK ), 1, A( KP, KP ), 1 )
*
*              Interchange rows KK and KP in first KK columns of A and W
*
               CALL AB_ZSWAP( KK, A( KK, 1 ), LDA, A( KP, 1 ), LDA )
