...
*     .. External Subroutines ..
      EXTERNAL           AB_DLASR, AB_DSCAL, AB_DSWAP, AB_AB_DLARTGP, AB
     $_AB_DLARTGS, AB_DLAS2,
     $                   AB_XERBLA
*     ..
*     .. External Functions ..
...
*
         IF( B11D(IMIN)**2+B11BULGE**2 .GT. THRESH**2 ) THEN
            CALL AB_AB_DLARTGP( B11BULGE, B11D(IMIN), WORK(IU1SN+IMIN-1)
     $,
     $                    WORK(IU1CS+IMIN-1), R )
     ...
     $                    WORK(IU1CS+IMIN-1), WORK(IU1SN+IMIN-1) )
         ...
         IF( B21D(IMIN)**2+B21BULGE**2 .GT. THRESH**2 ) THEN
            CALL AB_AB_DLARTGP( B21BULGE, B21D(IMIN), WORK(IU2SN+IMIN-1)
     $,
     $                    WORK(IU2CS+IMIN-1), R )
...
*
            IF( .NOT. RESTART11 .AND. .NOT. RESTART21 ) THEN
               CALL AB_AB_DLARTGP( X2, X1, WORK(IV1TSN+I-1), WORK(IV1TCS
     $+I-1),
     $                       R )
            ELSE IF( .NOT. RESTART11 .AND. RESTART21 ) THEN
               CALL AB_AB_DLARTGP( B11BULGE, B11E(I-1), WORK(IV1TSN+I-1)
     $,
     $                       WORK(IV1TCS+I-1), R )
            ELSE IF( RESTART11 .AND. .NOT. RESTART21 ) THEN
               CALL AB_AB_DLARTGP( B21BULGE, B21E(I-1), WORK(IV1TSN+I-1)
     $,
     $                       WORK(IV1TCS+I-1), R )
            ELSE IF( MU .LE. NU ) THEN
               CALL AB_AB_DLARTGS( B11D(I), B11E(I), MU, WORK(IV1TCS+I-1
     $),
     $                       WORK(IV1TSN+I-1) )
            ELSE
               CALL AB_AB_DLARTGS( B21D(I), B21E(I), NU, WORK(IV1TCS+I-1
     $),
     $                       WORK(IV1TSN+I-1) )
            END IF
            ...
            IF( .NOT. RESTART12 .AND. .NOT. RESTART22 ) THEN
               CALL AB_AB_DLARTGP( Y2, Y1, WORK(IV2TSN+I-1-1),
     $                       WORK(IV2TCS+I-1-1), R )
            ELSE IF( .NOT. RESTART12 .AND. RESTART22 ) THEN
               CALL AB_AB_DLARTGP( B12BULGE, B12D(I-1), WORK(IV2TSN+I-1-
     $1),
     $                       WORK(IV2TCS+I-1-1), R )
            ELSE IF( RESTART12 .AND. .NOT. RESTART22 ) THEN
               CALL AB_AB_DLARTGP( B22BULGE, B22D(I-1), WORK(IV2TSN+I-1-
     $1),
     $                       WORK(IV2TCS+I-1-1), R )
            ELSE IF( NU .LT. MU ) THEN
               CALL AB_AB_DLARTGS( B12E(I-1), B12D(I), NU, WORK(IV2TCS+I
     $-1-1),
     $                       WORK(IV2TSN+I-1-1) )
            ELSE
               CALL AB_AB_DLARTGS( B22E(I-1), B22D(I), MU, WORK(IV2TCS+I
     $-1-1),
     $                       WORK(IV2TSN+I-1-1) )
            END IF
*
...
*
            IF( .NOT. RESTART11 .AND. .NOT. RESTART12 ) THEN
               CALL AB_AB_DLARTGP( X2, X1, WORK(IU1SN+I-1), WORK(IU1CS+I
     $-1),
     $                       R )
            ELSE IF( .NOT. RESTART11 .AND. RESTART12 ) THEN
               CALL AB_AB_DLARTGP( B11BULGE, B11D(I), WORK(IU1SN+I-1),
     $                       WORK(IU1CS+I-1), R )
            ELSE IF( RESTART11 .AND. .NOT. RESTART12 ) THEN
               CALL AB_AB_DLARTGP( B12BULGE, B12E(I-1), WORK(IU1SN+I-1),
     $                       WORK(IU1CS+I-1), R )
            ELSE IF( MU .LE. NU ) THEN
               CALL AB_AB_DLARTGS( B11E(I), B11D(I+1), MU, WORK(IU1CS+I-
     $1),
     $                       WORK(IU1SN+I-1) )
            ELSE
               CALL AB_AB_DLARTGS( B12D(I), B12E(I), NU, WORK(IU1CS+I-1)
     $,
     $                       WORK(IU1SN+I-1) )
            END IF
            IF( .NOT. RESTART21 .AND. .NOT. RESTART22 ) THEN
               CALL AB_AB_DLARTGP( Y2, Y1, WORK(IU2SN+I-1), WORK(IU2CS+I
     $-1),
     $                       R )
            ELSE IF( .NOT. RESTART21 .AND. RESTART22 ) THEN
               CALL AB_AB_DLARTGP( B21BULGE, B21D(I), WORK(IU2SN+I-1),
     $                       WORK(IU2CS+I-1), R )
            ELSE IF( RESTART21 .AND. .NOT. RESTART22 ) THEN
               CALL AB_AB_DLARTGP( B22BULGE, B22E(I-1), WORK(IU2SN+I-1),
     $                       WORK(IU2CS+I-1), R )
            ELSE IF( NU .LT. MU ) THEN
               CALL AB_AB_DLARTGS( B21E(I), B21E(I+1), NU, WORK(IU2CS+I-
     $1),
     $                       WORK(IU2SN+I-1) )
            ELSE
               CALL AB_AB_DLARTGS( B22D(I), B22E(I), MU, WORK(IU2CS+I-1)
     $,
     $                       WORK(IU2SN+I-1) )
            END IF
...
*
         IF( .NOT. RESTART12 .AND. .NOT. RESTART22 ) THEN
            CALL AB_AB_DLARTGP( Y2, Y1, WORK(IV2TSN+IMAX-1-1),
     $                    WORK(IV2TCS+IMAX-1-1), R )
         ELSE IF( .NOT. RESTART12 .AND. RESTART22 ) THEN
            CALL AB_AB_DLARTGP( B12BULGE, B12D(IMAX-1), WORK(IV2TSN+IMAX
     $-1-1),
     $                    WORK(IV2TCS+IMAX-1-1), R )
         ELSE IF( RESTART12 .AND. .NOT. RESTART22 ) THEN
            CALL AB_AB_DLARTGP( B22BULGE, B22D(IMAX-1), WORK(IV2TSN+IMAX
     $-1-1),
     $                    WORK(IV2TCS+IMAX-1-1), R )
         ELSE IF( NU .LT. MU ) THEN
