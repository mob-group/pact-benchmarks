...
*     .. External Subroutines ..
      EXTERNAL           AB_XERBLA, AB_ZAXPY, ZAB_DSCAL, AB_ZHPMV, AB_AB
     $_ZHPR2, AB_ZTPMV,
     $                   AB_ZTPSV
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                     AP( J1 ), 1 )
               CALL ZAB_DSCAL( J-1, ONE / BJJ, AP( J1 ), 1 )
...
*
               ...
               AP( KK ) = AKK
               IF( K.LT.N ) THEN
                  CALL ZAB_DSCAL( N-K, ONE / BKK, AP( KK+1 ), 1 )
                  CT = -HALF*AKK
                  CALL AB_ZAXPY( N-K, CT, BP( KK+1 ), 1, AP( KK+1 ), 1 )
                  CALL AB_AB_ZHPR2( UPLO, N-K, -CONE, AP( KK+1 ), 1,
     $                        BP( KK+1 ), 1, AP( K1K1 ) )
                  CALL AB_ZAXPY( N-K, CT, BP( KK+1 ), 1, AP( KK+1 ), 1 )
...
*
     ...
     $                     AP( K1 ), 1 )
               ...
               CALL AB_AB_ZHPR2( UPLO, K-1, CONE, AP( K1 ), 1, BP( K1 ),
     $ 1,
     $                     AP )
               CALL AB_ZAXPY( K-1, CT, BP( K1 ), 1, AP( K1 ), 1 )
               CALL ZAB_DSCAL( K-1, BKK, AP( K1 ), 1 )
               AP( KK ) = AKK*BKK**2
   30       CONTINUE
         ELSE
*
...
*
     ...
     $                    BP( JJ+1 ), 1 )
               CALL ZAB_DSCAL( N-J, BJJ, AP( JJ+1 ), 1 )
