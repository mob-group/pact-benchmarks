*> \brief \b AB_AB_ZHETF2_RK computes the factorization of a complex Hermitian indefinite matrix using the bounded Bunch-Kaufman (rook) diagonal pivoting method (BLAS2 unblocked algorithm).
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download AB_AB_ZHETF2_RK + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_AB_ZHETF2_RK.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_AB_ZHETF2_RK.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_AB_ZHETF2_RK.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE AB_AB_ZHETF2_RK( UPLO, N, A, LDA, E, IPIV, INFO )
*
*       .. Scalar Arguments ..
*       CHARACTER          UPLO
*       INTEGER            INFO, LDA, N
...
*>
*> \verbatim
*> AB_AB_ZHETF2_RK computes the factorization of a complex Hermitian matrix A
...
*
      ...
      EXTERNAL           AB_LSAME, AB_IZAMAX, AB_DLAMCH, AB_DLAPY2
*     ..
*     .. External Subroutines ..
      EXTERNAL           AB_XERBLA, ZAB_DSCAL, AB_ZHER, AB_ZSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_ZHETF2_RK', -INFO )
         RETURN
      END IF
*
...
*
                  IF( IMAX.NE.K ) THEN
                     JMAX = IMAX + AB_IZAMAX( K-IMAX, A( IMAX, IMAX+1 ),
     $                                     LDA )
...
*
                  IF( IMAX.GT.1 ) THEN
                     ITEMP = AB_IZAMAX( IMAX-1, A( 1, IMAX ), 1 )
...
*              (1) Swap columnar parts
               IF( P.GT.1 )
     $            CALL AB_ZSWAP( P-1, A( 1, K ), 1, A( 1, P ), 1 )
...
*
               IF( K.LT.N )
     $            CALL AB_ZSWAP( N-K, A( K, K+1 ), LDA, A( P, K+1 ), LDA
     $ )
*
            END IF
*
...
*
                     D11 = ONE / DBLE( A( K, K ) )
                     CALL AB_ZHER( UPLO, K-1, -D11, A( 1, K ), 1, A, LDA
     $ )
*
*                    Store U(k) in column k
*
                     CALL ZAB_DSCAL( K-1, D11, A( 1, K ), 1 )
                  ELSE
*
...
*
                  IF( IMAX.LT.N ) THEN
                     ITEMP = IMAX + AB_IZAMAX( N-IMAX, A( IMAX+1, IMAX )
     $,
     $                                     1 )
                     DTEMP = CABS1( A( ITEMP, IMAX ) )
                     IF( DTEMP.GT.ROWMAX ) THEN
...
*              (1) Swap columnar parts
               IF( P.LT.N )
     $            CALL AB_ZSWAP( N-P, A( P+1, K ), 1, A( P+1, P ), 1 )
*              (2) Swap and conjugate middle parts
               DO 44 J = K + 1, P - 1
                  T = DCONJG( A( J, K ) )
                  A( J, K ) = DCONJG( A( P, J ) )
...
*
                     D11 = ONE / DBLE( A( K, K ) )
                     CALL AB_ZHER( UPLO, N-K, -D11, A( K+1, K ), 1,
     $                          A( K+1, K+1 ), LDA )
*
*                    Store L(k) in column k
*
                     CALL ZAB_DSCAL( N-K, D11, A( K+1, K ), 1 )
                  ELSE
*
*                    Store L(k) in column k
*
