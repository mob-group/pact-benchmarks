...
*>              is greater than BIG, use AB_AB_CGESVJ.
...
*>               block size for AB_CGEQP3 and AB_AB_CGEQRF.
*>               In general, optimal LWORK is computed as
*>               LWORK >= max(N+LWORK(AB_CGEQP3),N+LWORK(AB_AB_CGEQRF), LWORK(AB_AB_CGESVJ)).        
...
*>               where NB is the optimal block size for AB_CGEQP3, AB_AB_CGEQRF, AB_CGELQ,
*>               AB_CUNMLQ. In general, the optimal length LWORK is computed as
*>               LWORK >= max(N+LWORK(AB_CGEQP3), N+LWORK(AB_AB_CGESVJ),
*>                       N+LWORK(AB_AB_CGELQF), 2*N+LWORK(AB_AB_CGEQRF), N+LWORK(AB_CUNMLQ)).
...
*>               where NB is the optimal block size for AB_CGEQP3, AB_AB_CGEQRF, AB_CGELQ,
*>               AB_CUNMLQ. In general, the optimal length LWORK is computed as
*>               LWORK >= max(N+LWORK(AB_CGEQP3), LWORK(AB_CPOCON), N+LWORK(AB_AB_CGESVJ),
*>                       N+LWORK(AB_AB_CGELQF), 2*N+LWORK(AB_AB_CGEQRF), N+LWORK(AB_CUNMLQ)).   
...
*>               where NB is the optimal block size for AB_CGEQP3, AB_AB_CGEQRF, AB_CUNMQR.
*>               In general, the optimal length LWORK is computed as
*>               LWORK >= max(N+LWORK(AB_CGEQP3), 2*N+LWORK(AB_AB_CGEQRF), N+LWORK(AB_CUNMQR)). 
...
*>                        2*N+LWORK(AB_AB_CGEQRF), N+LWORK(AB_CUNMQR)).
*>
...
*>            of AB_CGEQP3, AB_AB_CGEQRF, AB_AB_CGELQF, AB_CUNMQR, AB_CUNMLQ.
*> 
*>          If the call to AB_CGEJSV is a workspace query (indicated by LWORK=-1 or
...
*>  AB_AB_CGEQRF, and AB_AB_CGELQF as preprocessors and preconditioners. Optionally, an
...
*>  the choice of some tuning parameters in the iterative part (AB_AB_CGESVJ) is
...
*
     ...
     $        LWSVDJ, LWSVDJV, LRWQP3, LRWCON, LRWSVDJ, IWOFF
      INTEGER LWRK_AB_AB_CGELQF, LWRK_AB_CGEQP3,  LWRK_AB_CGEQP3N, LWRK_
     $AB_AB_CGEQRF,  
     $        LWRK_AB_AB_CGESVJ, LWRK_AB_AB_CGESVJV, LWRK_AB_AB_CGESVJU,
     $ LWRK_AB_CUNMLQ, 
     $        LWRK_AB_CUNMQR, LWRK_AB_CUNMQRM    
*     ..
*     .. Local Arrays
      COMPLEX CDUMMY(1)
      REAL    RDUMMY(1)
...
*     .. External Functions ..
      ...
      LOGICAL   AB_LSAME
      EXTERNAL  AB_ISAMAX, AB_ICAMAX, AB_LSAME, AB_SLAMCH, AB_SCNRM2
*     ..
*     .. External Subroutines ..
      EXTERNAL  AB_SLASSQ, AB_CCOPY,  AB_AB_CGELQF, AB_CGEQP3, AB_AB_CGE
     $QRF, AB_CLACPY, AB_CLAPMR,
     $          AB_CLASCL, AB_SLASCL, AB_CLASET, AB_CLASSQ, AB_CLASWP, A
     $B_CUNGQR, AB_CUNMLQ,
     $          AB_CUNMQR, AB_CPOCON, AB_SSCAL,  AB_CAB_SSCAL, AB_CSWAP,
     $  AB_CTRSM,  AB_CLACGV,
     $          AB_XERBLA
*
      EXTERNAL  AB_AB_CGESVJ
*     ..
*
...
*         AB_AB_CGEQRF of an N x N matrix, AB_AB_CGELQF of an N x N matrix,
...
*        .. minimal workspace length for AB_CPOCON of an N x N matrix
          LWCON = 2 * N 
*        .. minimal workspace length for AB_AB_CGESVJ of an N x N matrix,
*         without and with explicit accumulation of Jacobi rotations
          LWSVDJ  = MAX( 2 * N, 1 )         
          LWSVDJV = MAX( 2 * N, 1 )
*         .. minimal REAL workspace length for AB_CGEQP3, AB_CPOCON, AB_AB_CGESVJ
          LRWQP3  = N 
          LRWCON  = N 
          LRWSVDJ = N 
          IF ( LQUERY ) THEN 
     ...
     $             RDUMMY, IERR )
              LWRK_AB_CGEQP3 = CDUMMY(1)
              CALL AB_AB_CGEQRF( N, N, A, LDA, CDUMMY, CDUMMY,-1, IERR )
              LWRK_AB_AB_CGEQRF = CDUMMY(1)
              CALL AB_AB_CGELQF( N, N, A, LDA, CDUMMY, CDUMMY,-1, IERR )
              LWRK_AB_AB_CGELQF = CDUMMY(1)             
          END IF
          MINWRK  = 2
          OPTWRK  = 2
          MINIWRK = N 
...
*             only the singular values are requested
              ...
              END IF
              IF ( LQUERY ) THEN 
                  CALL AB_AB_CGESVJ( 'L', 'N', 'N', N, N, A, LDA, SVA, N
     $, V, 
     $                 LDV, CDUMMY, -1, RDUMMY, -1, IERR )
                  LWRK_AB_AB_CGESVJ = CDUMMY(1)
                  IF ( ERREST ) THEN 
                      OPTWRK = MAX( N+LWRK_AB_CGEQP3, N**2+LWCON, 
     $                              N+LWRK_AB_AB_CGEQRF, LWRK_AB_AB_CGES
     $VJ )
                  ELSE
                      OPTWRK = MAX( N+LWRK_AB_CGEQP3, N+LWRK_AB_AB_CGEQR
     $F, 
     $                              LWRK_AB_AB_CGESVJ )
                  END IF
              END IF
...
*            singular values and the right singular vectors are requested
     ...
     $                         N+LWSVDJ, N+LWUNMLQ )
             END IF
             IF ( LQUERY ) THEN
                 CALL AB_AB_CGESVJ( 'L', 'U', 'N', N,N, U, LDU, SVA, N, 
     $A,
     $                LDA, CDUMMY, -1, RDUMMY, -1, IERR )
                 LWRK_AB_AB_CGESVJ = CDUMMY(1)
                 CALL AB_CUNMLQ( 'L', 'C', N, N, N, A, LDA, CDUMMY,
     $                V, LDV, CDUMMY, -1, IERR )
                 LWRK_AB_CUNMLQ = CDUMMY(1)                
                 IF ( ERREST ) THEN 
                 OPTWRK = MAX( N+LWRK_AB_CGEQP3, LWCON, LWRK_AB_AB_CGESV
     $J, 
     $                         N+LWRK_AB_AB_CGELQF, 2*N+LWRK_AB_AB_CGEQR
     $F,
     $                         N+LWRK_AB_AB_CGESVJ,  N+LWRK_AB_CUNMLQ )
                 ELSE
                 OPTWRK = MAX( N+LWRK_AB_CGEQP3, LWRK_AB_AB_CGESVJ,N+LWR
     $K_AB_AB_CGELQF,
     $                         2*N+LWRK_AB_AB_CGEQRF, N+LWRK_AB_AB_CGESV
     $J, 
     $                         N+LWRK_AB_CUNMLQ )
                 END IF
             END IF
...
*            singular values and the left singular vectors are requested
     ...
     $               LDU, CDUMMY, -1, IERR )
                 LWRK_AB_CUNMQRM = CDUMMY(1)
                 IF ( ERREST ) THEN
                 OPTWRK = N + MAX( LWRK_AB_CGEQP3, LWCON, N+LWRK_AB_AB_C
     $GEQRF,
     $                             LWRK_AB_AB_CGESVJ, LWRK_AB_CUNMQRM )
                 ELSE
                 OPTWRK = N + MAX( LWRK_AB_CGEQP3, N+LWRK_AB_AB_CGEQRF,
     $                             LWRK_AB_AB_CGESVJ, LWRK_AB_CUNMQRM )
                 END IF
             END IF
             IF ( L2TRAN .OR. ROWPIV ) THEN 
...
*            full SVD is requested
     ...
     $                LDU, CDUMMY, -1, IERR )
                 LWRK_AB_CUNMQR = CDUMMY(1)
                 IF ( .NOT. JRACC ) THEN
                     CALL AB_CGEQP3( N,N, A, LDA, IWORK, CDUMMY,CDUMMY, 
     $-1,
     $                    RDUMMY, IERR )
                     LWRK_AB_CGEQP3N = CDUMMY(1)
                     CALL AB_AB_CGESVJ( 'L', 'U', 'N', N, N, U, LDU, SVA
     $,
     $                    N, V, LDV, CDUMMY, -1, RDUMMY, -1, IERR )
                     LWRK_AB_AB_CGESVJ = CDUMMY(1)
                     CALL AB_AB_CGESVJ( 'U', 'U', 'N', N, N, U, LDU, SVA
     $,
     $                    N, V, LDV, CDUMMY, -1, RDUMMY, -1, IERR )
                     LWRK_AB_AB_CGESVJU = CDUMMY(1)
                     CALL AB_AB_CGESVJ( 'L', 'U', 'V', N, N, U, LDU, SVA
     $,
     $                    N, V, LDV, CDUMMY, -1, RDUMMY, -1, IERR )
                     LWRK_AB_AB_CGESVJV = CDUMMY(1)
                     CALL AB_CUNMLQ( 'L', 'C', N, N, N, A, LDA, CDUMMY,
     $                    V, LDV, CDUMMY, -1, IERR )
                     LWRK_AB_CUNMLQ = CDUMMY(1)
                     IF ( ERREST ) THEN 
                       OPTWRK = MAX( N+LWRK_AB_CGEQP3, N+LWCON, 
     $                          2*N+N**2+LWCON, 2*N+LWRK_AB_AB_CGEQRF, 
     $                          2*N+LWRK_AB_CGEQP3N, 
     $                          2*N+N**2+N+LWRK_AB_AB_CGELQF,  
     $                          2*N+N**2+N+N**2+LWCON,
     $                          2*N+N**2+N+LWRK_AB_AB_CGESVJ, 
     $                          2*N+N**2+N+LWRK_AB_AB_CGESVJV,          
     $     
     $                          2*N+N**2+N+LWRK_AB_CUNMQR,
     $                          2*N+N**2+N+LWRK_AB_CUNMLQ, 
     $                          N+N**2+LWRK_AB_AB_CGESVJU,              
     $    
     $                          N+LWRK_AB_CUNMQRM )
                     ELSE
                       OPTWRK = MAX( N+LWRK_AB_CGEQP3,  
     $                          2*N+N**2+LWCON, 2*N+LWRK_AB_AB_CGEQRF, 
     $                          2*N+LWRK_AB_CGEQP3N, 
     $                          2*N+N**2+N+LWRK_AB_AB_CGELQF,  
     $                          2*N+N**2+N+N**2+LWCON,
     ...
     $                    LDU, CDUMMY, -1, IERR )
                     ...
                     IF ( ERREST ) THEN 
                        OPTWRK = MAX( N+LWRK_AB_CGEQP3, N+LWCON,   
     $                           2*N+LWRK_AB_AB_CGEQRF, 2*N+N**2,  
     $                           2*N+N**2+LWRK_AB_AB_CGESVJV,  
     $                           2*N+N**2+N+LWRK_AB_CUNMQR,N+LWRK_AB_CUN
     $MQRM )
                     ELSE
                        OPTWRK = MAX( N+LWRK_AB_CGEQP3, 2*N+LWRK_AB_AB_C
     $GEQRF,  
     $                           2*N+N**2, 2*N+N**2+LWRK_AB_AB_CGESVJV, 
     $                           2*N+N**2+N+LWRK_AB_CUNMQR, 
     $                           N+LWRK_AB_CUNMQRM )   
                     END IF                  
                 END IF               
...
*           computing all M left singular vectors of the M x 1 matrix
            IF ( N1 .NE. N  ) THEN
              CALL AB_AB_CGEQRF( M, N, U,LDU, CWORK, CWORK(N+1),LWORK-N,
     $IERR )
...
*     from SFMIN to BIG, then AB_AB_CGESVJ will compute them. So, in that case,
*     one should use AB_AB_CGESVJ instead of AB_CGEJSV.
      BIG1   = SQRT( BIG )
      TEMP1  = SQRT( BIG / REAL(N) )
*     >> for future updates: allow bigger range, i.e. the largest column
*     will be allowed up to BIG/N and AB_AB_CGESVJ will do the rest. However, for
...
*        as a precaution measure, the full SVD is computed using AB_AB_CGESVJ
...
*              .. V is available as workspace
               ...
               DO 3053 p = 1, N
                  TEMP1 = SVA(IWORK(p))
                  CALL AB_CAB_SSCAL( p, ONE/TEMP1, V(1,p), 1 )
 3053          CONTINUE
               IF ( LSVEC )THEN
...
*              .. U is available as workspace
               ...
               DO 3054 p = 1, N
                  TEMP1 = SVA(IWORK(p))
                  CALL AB_CAB_SSCAL( p, ONE/TEMP1, U(1,p), 1 )
 3054          CONTINUE
...
*              not needed for SIGMA only computation
               DO 3052 p = 1, N
                  TEMP1 = SVA(IWORK(p))
*[]               CALL AB_CAB_SSCAL( p, ONE/TEMP1, CWORK(N+(p-1)*N+1), 1 )
                  CALL AB_CAB_SSCAL( p, ONE/TEMP1, CWORK((p-1)*N+1), 1 )
 3052          CONTINUE
...
*
            CALL AB_AB_CGEQRF( N,NR, A,LDA, CWORK, CWORK(N+1),LWORK-N, I
     $ERR )
*
...
*
            CALL AB_AB_CGESVJ( 'L', 'N', 'N', NR, NR, A, LDA, SVA,
...
*
            CALL AB_AB_CGESVJ( 'L','U','N', N, NR, V, LDV, SVA, NR, A, L
     $DA,
...
*
            ...
            CALL AB_AB_CGELQF( NR,N, A, LDA, CWORK, CWORK(N+1), LWORK-N,
     $ IERR)
            CALL AB_CLACPY( 'L', NR, NR, A, LDA, V, LDV )
            CALL AB_CLASET( 'U', NR-1,NR-1, CZERO, CZERO, V(1,2), LDV )
            CALL AB_AB_CGEQRF( NR, NR, V, LDV, CWORK(N+1), CWORK(2*N+1),
     $                   LWORK-2*N, IERR )
            DO 8998 p = 1, NR
 ...
 8998       CONTINUE
            CALL AB_CLASET('U', NR-1, NR-1, CZERO, CZERO, V(1,2), LDV)
*
            CALL AB_AB_CGESVJ( 'L', 'U','N', NR, NR, V,LDV, SVA, NR, U,
...
*
         CALL AB_AB_CGESVJ( 'U','N','V', N, N, A, LDA, SVA, N, V, LDV,
     $               CWORK, LWORK, RWORK, LRWORK, INFO )
          SCALEM  = RWORK(1)
          NUMRANK = NINT(RWORK(2))
          CALL AB_CLAPMR( .FALSE., N, N, V, LDV, IWORK )
...
*
         CALL AB_AB_CGEQRF( N, NR, U, LDU, CWORK(N+1), CWORK(2*N+1),
     $              LWORK-2*N, IERR )
*
         DO 1967 p = 1, NR - 1
 ...
 1967    CONTINUE
         CALL AB_CLASET( 'U', NR-1, NR-1, CZERO, CZERO, U(1,2), LDU )
*
         CALL AB_AB_CGESVJ( 'L', 'U', 'N', NR,NR, U, LDU, SVA, NR, A,
...
*
         DO 1974 p = 1, N1
            ...
            CALL AB_CAB_SSCAL( M, XSC, U(1,p), 1 )
 1974    CONTINUE
*
         IF ( TRANSP ) THEN
...
*
            ...
            DO 3950 p = 1, NR
               ...
               CALL AB_CAB_SSCAL(NR-p+1,ONE/TEMP1,CWORK(2*N+(p-1)*NR+p),
     $1)
 3950       CONTINUE
...
*              R1^* = Q2 * R2
               CALL AB_AB_CGEQRF( N, NR, V, LDV, CWORK(N+1), CWORK(2*N+1
     $),
     $              LWORK-2*N, IERR )
*
               IF ( L2PERT ) THEN
...
*              Now, compute R2 = L3 * Q3, the LQ factorization.
               CALL AB_AB_CGELQF( NR, NR, V, LDV, CWORK(2*N+N*NR+1),
...
*              .. and estimate the condition number
               ...
               DO 4950 p = 1, NR
                  ...
                  CALL AB_CAB_SSCAL( p, ONE/TEMP1, CWORK(2*N+N*NR+NR+p),
     $ NR )
 4950          CONTINUE
...
*
            IF ( CONDR1 .LT. COND_OK ) THEN
*
               CALL AB_AB_CGESVJ( 'L','U','N',NR,NR,V,LDV,SVA,NR,U, LDU,
     ...
     $              LRWORK, INFO )
               ...
               DO 3970 p = 1, NR
                  ...
                  CALL AB_CAB_SSCAL( NR, SVA(p),    V(1,p), 1 )
 3970          CONTINUE
...
*                 used in AB_AB_CGESVJ, premultiplied with the orthogonal matrix
...
*                 used in AB_AB_CGESVJ. The Q-factor from the AB_SECOND QR
...
*              R2=L3*Q3), pre-multiplied with the transposed Q3.
               CALL AB_AB_CGESVJ( 'L', 'U', 'N', NR, NR, V, LDV, SVA, NR
     $, U,
     ...
     $              RWORK, LRWORK, INFO )
               ...
               DO 3870 p = 1, NR
                  CALL AB_CCOPY( NR, V(1,p), 1, U(1,p), 1 )
                  CALL AB_CAB_SSCAL( NR, SVA(p),    U(1,p), 1 )
 3870          CONTINUE
...
*              Compute the full SVD of L3 using AB_AB_CGESVJ with explicit
*              accumulation of Jacobi rotations.
               CALL AB_AB_CGESVJ( 'L', 'U', 'V', NR, NR, V, LDV, SVA, NR
     $, U,
     $              LDU, CWORK(2*N+N*NR+NR+1), LWORK-2*N-N*NR-NR,
     $                         RWORK, LRWORK, INFO )
               SCALEM  = RWORK(1)
...
*
  ...
  973          CONTINUE
     ...
     $           CALL AB_CAB_SSCAL( N, XSC, V(1,q), 1 )
 1972       CONTINUE
...
*     $                                        ABS(CWORK(N+(p-1)*N+q)) )
 ...
 5970          CONTINUE
            ...
            END IF
*
            CALL AB_AB_CGESVJ( 'U', 'U', 'N', N, N, CWORK(N+1), N, SVA,
...
*
            ...
            DO 6970 p = 1, N
               ...
               CALL AB_CAB_SSCAL( N, SVA(p), CWORK(N+(p-1)*N+1), 1 )
 6970       CONTINUE
*
 ...
 6972       CONTINUE
            ...
            DO 6971 p = 1, N
               ...
               IF ( (XSC .LT. (ONE-TEMP1)) .OR. (XSC .GT. (ONE+TEMP1)) )
     $            CALL AB_CAB_SSCAL( N, XSC, V(1,p), 1 )
 6971       CONTINUE
*
...
*                  U(p,q) = - TEMP1 * ( U(q,p) / ABS(U(q,p)) )
 ...
 9970       CONTINUE
         ...
         CALL AB_AB_CGESVJ( 'L', 'U', 'V', NR, NR, U, LDU, SVA,
