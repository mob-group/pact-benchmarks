*> \brief <b> AB_AB_DGGEV3 computes the eigenvalues and, optionally, the left and/or right eigenvectors for GE matrices (blocked algorithm)</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download AB_AB_DGGEV3 + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_AB_DGGEV3.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_AB_DGGEV3.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_AB_DGGEV3.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE AB_AB_DGGEV3( JOBVL, JOBVR, N, A, LDA, B, LDB, ALPHAR,
...
*> \verbatim
*>
*> AB_AB_DGGEV3 computes for a pair of N-by-N real nonsymmetric matrices (A,B)
...
*     .. External Subroutines ..
      EXTERNAL           AB_AB_DGEQRF, AB_DGGBAK, AB_DGGBAL, AB_DGGHD3, 
     $AB_DHGEQZ, AB_DLABAD,
...
*
      IF( INFO.EQ.0 ) THEN
         CALL AB_AB_DGEQRF( N, N, B, LDB, WORK, WORK, -1, IERR )
         LWKOPT = MAX(1, 8*N, 3*N+INT( WORK( 1 ) ) )
         CALL AB_DORMQR( 'L', 'T', N, N, N, B, LDB, WORK, A, LDA, WORK, 
     $-1,
     $                IERR )
         LWKOPT = MAX( LWKOPT, 3*N+INT( WORK ( 1 ) ) )
         IF( ILVL ) THEN
            CALL AB_DORGQR( N, N, N, VL, LDVL, WORK, WORK, -1, IERR )
            LWKOPT = MAX( LWKOPT, 3*N+INT( WORK ( 1 ) ) )
         END IF
         IF( ILV ) THEN
            CALL AB_DGGHD3( JOBVL, JOBVR, N, 1, N, A, LDA, B, LDB, VL,
     $                   LDVL, VR, LDVR, WORK, -1, IERR )
            LWKOPT = MAX( LWKOPT, 3*N+INT( WORK ( 1 ) ) )
            CALL AB_DHGEQZ( 'S', JOBVL, JOBVR, N, 1, N, A, LDA, B, LDB,
     $                   ALPHAR, ALPHAI, BETA, VL, LDVL, VR, LDVR,
     $                   WORK, -1, IERR )
            LWKOPT = MAX( LWKOPT, 2*N+INT( WORK ( 1 ) ) )
         ELSE
            CALL AB_DGGHD3( 'N', 'N', N, 1, N, A, LDA, B, LDB, VL, LDVL,
     $                   VR, LDVR, WORK, -1, IERR )
            LWKOPT = MAX( LWKOPT, 3*N+INT( WORK ( 1 ) ) )
            CALL AB_DHGEQZ( 'E', JOBVL, JOBVR, N, 1, N, A, LDA, B, LDB,
     $                   ALPHAR, ALPHAI, BETA, VL, LDVL, VR, LDVR,
     $                   WORK, -1, IERR )
            LWKOPT = MAX( LWKOPT, 2*N+INT( WORK ( 1 ) ) )
         END IF
...
*
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_DGGEV3 ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
         CALL AB_DGGHD3( JOBVL, JOBVR, N, ILO, IHI, A, LDA, B, LDB, VL,
     $                LDVL, VR, LDVR, WORK( IWRK ), LWORK+1-IWRK, IERR )
      ELSE
         CALL AB_DGGHD3( 'N', 'N', IROWS, 1, IROWS, A( ILO, ILO ), LDA,
