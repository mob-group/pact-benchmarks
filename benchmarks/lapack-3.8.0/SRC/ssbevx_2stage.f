*> \brief <b> AB_AB_AB_SSBEVX_2STAGE computes the eigenvalues and, optionally, the left and/or right eigenvectors for OTHER matrices</b>
*
*  @generated from AB_AB_AB_DSBEVX_2STAGE.f, fortran d -> s, Sat Nov  5 23:58:06 2016
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download AB_AB_AB_SSBEVX_2STAGE + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_AB_AB_SSBEVX_2STAGE.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_AB_AB_SSBEVX_2STAGE.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_AB_AB_SSBEVX_2STAGE.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE AB_AB_AB_SSBEVX_2STAGE( JOBZ, RANGE, UPLO, N, KD, AB, LDAB, Q,
...
*> \verbatim
*>
*> AB_AB_AB_SSBEVX_2STAGE computes selected eigenvalues and, optionally, eigenvectors
...
*
*  =====================================================================
      SUBROUTINE AB_AB_AB_SSBEVX_2STAGE( JOBZ, RANGE, UPLO, N, KD, AB, L
     $DAB, Q,
     $                          LDQ, VL, VU, IL, IU, ABSTOL, M, W, Z,
     $                          LDZ, WORK, LWORK, IWORK, IFAIL, INFO )
*
...
*     .. External Functions ..
      ...
      REAL               AB_SLAMCH, AB_SLANSB
      EXTERNAL           AB_LSAME, AB_SLAMCH, AB_SLANSB, AB_AB_ILAENV2ST
     $AGE
*     ..
*     .. External Subroutines ..
      EXTERNAL           AB_SCOPY, AB_SGEMV, AB_SLACPY, AB_SLASCL, AB_SS
     $CAL,
     $                   AB_SSTEBZ, AB_SSTEIN, AB_SSTEQR, AB_SSTERF, AB_
     $SSWAP, AB_XERBLA,
     $                   AB_SSYTRD_SB2ST
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.EQ.0 ) THEN
         IF( N.LE.1 ) THEN
            ...
            WORK( 1 ) = LWMIN
         ELSE
            IB    = AB_AB_ILAENV2STAGE( 2, 'AB_SSYTRD_SB2ST', JOBZ,
     $                            N, KD, -1, -1 )
            LHTRD = AB_AB_ILAENV2STAGE( 3, 'AB_SSYTRD_SB2ST', JOBZ,
     $                            N, KD, IB, -1 )
            LWTRD = AB_AB_ILAENV2STAGE( 4, 'AB_SSYTRD_SB2ST', JOBZ,
     $                            N, KD, IB, -1 )
...
*
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_AB_SSBEVX_2STAGE ', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL AB_SSYTRD_SB2ST( "N", JOBZ, UPLO, N, KD, AB, LDAB, WORK( INDD
     $ ),
     $                    WORK( INDE ), WORK( INDHOUS ), LHTRD, 
     $                    WORK( INDWRK ), LLWORK, IINFO )
*
*     If all eigenvalues are desired and ABSTOL is less than or equal
*     to zero, then call AB_SSTERF or AB_SSTEQR.  If this fails for some
...
*
      ...
      IF ((ALLEIG .OR. TEST) .AND. (ABSTOL.LE.ZERO)) THEN
         ...
         ELSE
            CALL AB_SLACPY( 'A', N, N, Q, LDQ, Z, LDZ )
            CALL AB_SCOPY( N-1, WORK( INDE ), 1, WORK( INDEE ), 1 )
...
*
         DO 20 J = 1, M
            CALL AB_SCOPY( N, Z( 1, J ), 1, WORK( 1 ), 1 )
            CALL AB_SGEMV( 'N', N, N, ONE, Q, LDQ, WORK, 1, ZERO,
     $                  Z( 1, J ), 1 )
   20    CONTINUE
      END IF
*
