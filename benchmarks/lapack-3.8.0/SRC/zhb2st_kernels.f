...
*     .. External Subroutines ..
      EXTERNAL           AB_AB_ZLARFG, AB_AB_ZLARFX, AB_AB_ZLARFY
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   10         CONTINUE
              ...
              CALL AB_AB_ZLARFG( LM, CTMP, V( VPOS+1 ), 1, 
     $                                       TAU( TAUPOS ) )
              A( OFDPOS, ST ) = CTMP
*
              LM = ED - ST + 1
              CALL AB_AB_ZLARFY( UPLO, LM, V( VPOS ), 1,
     $                     DCONJG( TAU( TAUPOS ) ),
...
*
          IF( TTYPE.EQ.2 ) THEN
              ...
              LM = J2-J1+1
              IF( LM.GT.0) THEN
                  CALL AB_AB_ZLARFX( 'Left', LN, LM, V( VPOS ),
     $                         DCONJG( TAU( TAUPOS ) ),
...
*
   ...
   30             CONTINUE
                  ...
                  CALL AB_AB_ZLARFG( LM, CTMP, V( VPOS+1 ), 1, TAU( TAUP
     $OS ) )
                  A( DPOS-NB, J1 ) = CTMP
*                 
                  CALL AB_AB_ZLARFX( 'Right', LN-1, LM, V( VPOS ),
     $                         TAU( TAUPOS ),
...
*
   ...
   20         CONTINUE
              CALL AB_AB_ZLARFG( LM, A( OFDPOS, ST-1 ), V( VPOS+1 ), 1, 
     $                                       TAU( TAUPOS ) )
*
              LM = ED - ST + 1
*
...
*
              IF( LM.GT.0) THEN
                  CALL AB_AB_ZLARFX( 'Right', LM, LN, V( VPOS ), 
...
*
                  ...
                  DO 40 I = 1, LM-1
                      ...
                      A( DPOS+NB+I, ST ) = ZERO
   40             CONTINUE
                  CALL AB_AB_ZLARFG( LM, A( DPOS+NB, ST ), V( VPOS+1 ), 
     $1, 
     $                                        TAU( TAUPOS ) )
*
                  CALL AB_AB_ZLARFX( 'Left', LM, LN-1, V( VPOS ), 
     $                         DCONJG( TAU( TAUPOS ) ),
