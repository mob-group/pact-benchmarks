*> \brief <b> AB_AB_ZGELSS solves overdetermined or underdetermined systems for GE matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download AB_AB_ZGELSS + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_AB_ZGELSS.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_AB_ZGELSS.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_AB_ZGELSS.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE AB_AB_ZGELSS( M, N, NRHS, A, LDA, B, LDB, S, RCOND, RANK,
...
*> \verbatim
*>
*> AB_AB_ZGELSS computes the minimum norm solution to a complex linear
...
*
*  =====================================================================
      SUBROUTINE AB_AB_ZGELSS( M, N, NRHS, A, LDA, B, LDB, S, RCOND, RAN
     $K,
     $                   WORK, LWORK, RWORK, INFO )
*
...
*     .. Local Scalars ..
     ...
     $                   MAXWRK, MINMN, MINWRK, MM, MNTHR
      INTEGER            LWORK_AB_AB_ZGEQRF, LWORK_AB_ZUNMQR, LWORK_AB_Z
     $GEBRD,
     $                   LWORK_AB_ZUNMBR, LWORK_AB_ZUNGBR, LWORK_AB_ZUNM
     $LQ,
     $                   LWORK_AB_AB_ZGELQF
...
*     .. External Subroutines ..
     ...
     $ZBDSQR, AB_ZCOPY,
     $                   ZAB_DRSCL, AB_ZGEBRD, AB_AB_ZGELQF, AB_ZGEMM, A
     $B_ZGEMV, AB_AB_ZGEQRF,
...
*     .. External Functions ..
      ...
      EXTERNAL           AB_ILAENV, AB_DLAMCH, AB_ZLANGE
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
      IF( INFO.EQ.0 ) THEN
         ...
         IF( MINMN.GT.0 ) THEN
            MM = M
            MNTHR = AB_ILAENV( 6, 'AB_AB_ZGELSS', ' ', M, N, NRHS, -1 )
...
*              Compute space needed for AB_AB_ZGEQRF
               CALL AB_AB_ZGEQRF( M, N, A, LDA, DUM(1), DUM(1), -1, INFO
     $ )
               LWORK_AB_AB_ZGEQRF=DUM(1)
*              Compute space needed for AB_ZUNMQR
               CALL AB_ZUNMQR( 'L', 'C', M, NRHS, N, A, LDA, DUM(1), B,
     $                   LDB, DUM(1), -1, INFO )
               LWORK_AB_ZUNMQR=DUM(1)
               MM = N
               MAXWRK = MAX( MAXWRK, N + N*AB_ILAENV( 1, 'AB_AB_ZGEQRF',
     $ ' ', M,
     $                       N, -1, -1 ) )
...
*                 Compute space needed for AB_AB_ZGELQF
                  CALL AB_AB_ZGELQF( M, N, A, LDA, DUM(1), DUM(1),
     $                -1, INFO )
                  LWORK_AB_AB_ZGELQF=DUM(1)
*                 Compute space needed for AB_ZGEBRD
...
*                 Compute space needed for AB_ZUNMLQ
     ...
     $                 B, LDB, DUM(1), -1, INFO )
                  LWORK_AB_ZUNMLQ=DUM(1)
*                 Compute total workspace needed
                  MAXWRK = M + LWORK_AB_AB_ZGELQF
                  MAXWRK = MAX( MAXWRK, 3*M + M*M + LWORK_AB_ZGEBRD )
                  MAXWRK = MAX( MAXWRK, 3*M + M*M + LWORK_AB_ZUNMBR )
                  MAXWRK = MAX( MAXWRK, 3*M + M*M + LWORK_AB_ZUNGBR )
                  IF( NRHS.GT.1 ) THEN
                     MAXWRK = MAX( MAXWRK, M*M + M + M*NRHS )
                  ELSE
                     MAXWRK = MAX( MAXWRK, M*M + 2*M )
                  END IF
                  MAXWRK = MAX( MAXWRK, M + LWORK_AB_ZUNMLQ )
               ELSE
*
...
*                 Compute space needed for AB_ZUNGBR
     ...
     $                   DUM(1), -1, INFO )
                  ...
                  MAXWRK = MAX( MAXWRK, 2*M + LWORK_AB_ZUNMBR )
                  MAXWRK = MAX( MAXWRK, 2*M + LWORK_AB_ZUNGBR )
                  MAXWRK = MAX( MAXWRK, N*NRHS )
               END IF
            END IF
...
*
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_ZGELSS', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*           (RWorkspace: none)
*
            CALL AB_AB_ZGEQRF( M, N, A, LDA, WORK( ITAU ), WORK( IWORK )
     $,
     $                   LWORK-IWORK+1, INFO )
*
...
*
     ...
     $      THR = MAX( EPS*S( 1 ), SFMIN )
         ...
         DO 10 I = 1, N
            IF( S( I ).GT.THR ) THEN
               CALL ZAB_DRSCL( NRHS, S( I ), B( I, 1 ), LDB )
               RANK = RANK + 1
            ELSE
...
*        (RWorkspace: none)
*
         CALL AB_AB_ZGELQF( M, N, A, LDA, WORK( ITAU ), WORK( IWORK ),
     $                LWORK-IWORK+1, INFO )
         IL = IWORK
*
