...
*     .. External Subroutines ..
      EXTERNAL           AB_DGEQRF, AB_DLAQP2, AB_DLAQPS, AB_DORMQR, AB_
     $DSWAP, AB_XERBLA
*     ..
*     .. External Functions ..
      INTEGER            AB_ILAENV
...
*
      IF( INFO.EQ.0 ) THEN
         ...
         ELSE
            IWS = 3*N + 1
            NB = AB_ILAENV( INB, 'AB_DGEQRF', ' ', M, N, -1, -1 )
            LWKOPT = 2*N + ( N + 1 )*NB
         END IF
         WORK( 1 ) = LWKOPT
*
...
*
      IF( NFXD.GT.0 ) THEN
         NA = MIN( M, NFXD )
*CC      CALL AB_DGEQR2( M, NA, A, LDA, TAU, WORK, INFO )
         CALL AB_DGEQRF( M, NA, A, LDA, TAU, WORK, LWORK, INFO )
...
*
         NB = AB_ILAENV( INB, 'AB_DGEQRF', ' ', SM, SN, -1, -1 )
         NBMIN = 2
         NX = 0
*
...
*
            NX = MAX( 0, AB_ILAENV( IXOVER, 'AB_DGEQRF', ' ', SM, SN, -1
     $,
     $           -1 ) )
*
*
...
*
                  NB = ( LWORK-2*SN ) / ( SN+1 )
                  NBMIN = MAX( 2, AB_ILAENV( INBMIN, 'AB_DGEQRF', ' ', S
     $M, SN,
     $                    -1, -1 ) )
*
*
