*> \brief \b AB_ZLALS0 applies back multiplying factors in solving the least squares problem using divide and conquer SVD approach. Used by AB_SGELSd.
*
*  =========== DOCUMENTATION ===========
*
...
*     .. External Subroutines ..
      EXTERNAL           AB_DGEMV, AB_XERBLA, AB_ZCOPY, AB_ZDROT, AB_ZDS
     $CAL, AB_ZLACPY,
     $                   AB_ZLASCL
*     ..
*     .. External Functions ..
...
*
         DO 10 I = 1, GIVPTR
            CALL AB_ZDROT( NRHS, B( GIVCOL( I, 2 ), 1 ), LDB,
...
*
         IF( K.EQ.1 ) THEN
            ...
            IF( Z( 1 ).LT.ZERO ) THEN
               CALL AB_ZDSCAL( NRHS, NEGONE, B, LDB )
            END IF
         ELSE
            DO 100 J = 1, K
               DIFLJ = DIFL( J )
...
*
         IF( SQRE.EQ.1 ) THEN
            ...
            CALL AB_ZDROT( NRHS, BX( 1, 1 ), LDBX, BX( M, 1 ), LDBX, C, 
     $S )
         END IF
         IF( K.LT.MAX( M, N ) )
