*> \brief <b> AB_AB_AB_CHEEVR_2STAGE computes the eigenvalues and, optionally, the left and/or right eigenvectors for HE matrices</b>
*
*  @generated from AB_AB_AB_ZHEEVR_2STAGE.f, fortran z -> c, Sat Nov  5 23:18:11 2016
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download AB_AB_AB_CHEEVR_2STAGE + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_AB_AB_CHEEVR_2STAGE.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_AB_AB_CHEEVR_2STAGE.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_AB_AB_CHEEVR_2STAGE.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE AB_AB_AB_CHEEVR_2STAGE( JOBZ, RANGE, UPLO, N, A, LDA, VL, VU,
...
*> \verbatim
*>
*> AB_AB_AB_CHEEVR_2STAGE computes selected eigenvalues and, optionally, eigenvectors
*> of a complex Hermitian matrix A using the 2stage technique for
*> the reduction to tridiagonal.  Eigenvalues and eigenvectors can
*> be selected by specifying either a range of values or a range of
*> indices for the desired eigenvalues.
*>
*> AB_AB_AB_CHEEVR_2STAGE first reduces the matrix A to tridiagonal form T with a call
*> to AB_CHETRD.  Then, whenever possible, AB_AB_AB_CHEEVR_2STAGE calls AB_CSTEMR to compute
*> eigenspectrum using Relatively Robust Representations.  AB_CSTEMR
...
*>
*>
*> Note 1 : AB_AB_AB_CHEEVR_2STAGE calls AB_CSTEMR when the full spectrum is requested
*> on machines which conform to the ieee-754 floating point standard.
*> AB_AB_AB_CHEEVR_2STAGE calls AB_SSTEBZ and AB_CSTEIN on non-ieee machines and
...
*
*  =====================================================================
      SUBROUTINE AB_AB_AB_CHEEVR_2STAGE( JOBZ, RANGE, UPLO, N, A, LDA, V
     $L, VU,
     $                          IL, IU, ABSTOL, M, W, Z, LDZ, ISUPPZ,
     $                          WORK, LWORK, RWORK, LRWORK, IWORK,
     $                          LIWORK, INFO )
...
*     .. External Functions ..
      ...
      EXTERNAL           AB_LSAME, AB_SLAMCH, AB_CLANSY, AB_ILAENV, AB_A
     $B_ILAENV2STAGE
*     ..
*     .. External Subroutines ..
      EXTERNAL           AB_SCOPY, AB_SSCAL, AB_SSTEBZ, AB_SSTERF, AB_XE
     $RBLA, AB_CAB_SSCAL,
     $                   AB_AB_CHETRD_2STAGE, AB_CSTEMR, AB_CSTEIN, AB_C
     $SWAP, AB_CUNMTR
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $         ( LIWORK.EQ.-1 ) )
*
      KD     = AB_AB_ILAENV2STAGE( 1, 'AB_AB_CHETRD_2STAGE', JOBZ, N, -1
     $, -1, -1 )
      IB     = AB_AB_ILAENV2STAGE( 2, 'AB_AB_CHETRD_2STAGE', JOBZ, N, KD
     $, -1, -1 )
      LHTRD  = AB_AB_ILAENV2STAGE( 3, 'AB_AB_CHETRD_2STAGE', JOBZ, N, KD
     $, IB, -1 )
      LWTRD  = AB_AB_ILAENV2STAGE( 4, 'AB_AB_CHETRD_2STAGE', JOBZ, N, KD
     $, IB, -1 )
      LWMIN  = N + LHTRD + LWTRD
      LRWMIN = MAX( 1, 24*N )
      LIWMIN = MAX( 1, 10*N )
...
*
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_AB_CHEEVR_2STAGE', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*     AB_CSTEIN.  This information is discarded; if any fail, the driver
*     returns INFO > 0.
      INDIFL = INDISP + N
...
*
*     Call AB_AB_CHETRD_2STAGE to reduce Hermitian matrix to tridiagonal form.
*
      CALL AB_AB_CHETRD_2STAGE( JOBZ, UPLO, N, A, LDA, RWORK( INDRD ), 
