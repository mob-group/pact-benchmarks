C> \brief \b AB_AB_ZGEQRF VARIANT: left-looking Level 3 BLAS of the algo
     $rithm.
*
*  =========== DOCUMENTATION ===========
*
...
C>\verbatim
C>
C> AB_AB_ZGEQRF computes a QR factorization of a real M-by-N matrix A:
C> A = Q * R.
C>
...
C>                   NX = MAX( 0, AB_ILAENV( 3, 'AB_AB_ZGEQRF', ' ', M, 
     $N, -1, -1 ) )
C> \endverbatim
C> \verbatim
C>          3) The part for AB_AB_DLARFB is of size max((N-M)*K, (N-M)*N
     $B, K*NB, NB*NB)
C> \endverbatim
C> \verbatim
...
*     .. External Subroutines ..
      EXTERNAL           AB_AB_ZGEQR2, AB_AB_ZLARFB, AB_AB_ZLARFT, AB_XE
     $RBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
*     optimal workspace = space for AB_AB_DLARFB + space for normal T's + space for the last T
*
      ...
      IF ( NT.GT.NB ) THEN
          ...
          LBWORK = K-NT
*
*         Optimal workspace for AB_AB_DLARFB = MAX(1,N)*NT
*
...
*
               CALL AB_AB_ZLARFB( 'Left', 'Transpose', 'Forward',
...
*           A(I:M,I:I+IB-1)
*
            CALL AB_AB_ZGEQR2( M-I+1, IB, A( I, I ), LDA, TAU( I ),
...
30           CONTINUE
             ...
             CALL AB_AB_ZGEQR2( M-I+1, K-I+1, A( I, I ), LDA, TAU( I ),
     $                   WORK(LBWORK*NB+NT*NT+1),IINFO )
...
*        Use unblocked code to factor the last or only block.
*
         CALL AB_AB_ZGEQR2( M-I+1, N-I+1, A( I, I ), LDA, TAU( I ),
     $               WORK,IINFO )
...
*
          IF ( NT .LE. NB ) THEN
               CALL AB_AB_ZLARFT( 'Forward', 'Columnwise', M-I+1, K-I+1,
