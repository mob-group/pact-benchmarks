C> \brief \b AB_DPOTRF VARIANT: right looking block version of the algor
...
C>\verbatim
C>
C> AB_DPOTRF computes the Cholesky factorization of a real symmetric
...
*     .. External Subroutines ..
      EXTERNAL           AB_DGEMM, AB_DPOTF2, AB_AB_DSYRK, AB_DTRSM, AB_
     $XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*        Use unblocked code.
*
         CALL AB_DPOTF2( UPLO, N, A, LDA, INFO )
      ELSE
*
*        Use blocked code.
*
...
*
               ...
               CALL AB_DPOTF2( 'Upper', JB, A( J, J ), LDA, INFO )
...
*
     ...
     $                        A( J, J+JB ), LDA )
                  CALL AB_AB_DSYRK( 'Upper', 'Transpose', N-J-JB+1, JB, 
     $-ONE,
     $                        A( J, J+JB ), LDA,
     $                        ONE, A( J+JB, J+JB ), LDA )
               END IF
...
*
               ...
               CALL AB_DPOTF2( 'Lower', JB, A( J, J ), LDA, INFO )
...
*                Updating the trailing submatrix.
*
                 CALL AB_DTRSM( 'Right', 'Lower', 'Transpose', 'Non-unit
     $',
     $                       N-J-JB+1, JB, ONE, A( J, J ), LDA,
     $                       A( J+JB, J ), LDA )

                 CALL AB_AB_DSYRK( 'Lower', 'No Transpose', N-J-JB+1, JB
     $,
     $                       -ONE, A( J+JB, J ), LDA,
     $                       ONE, A( J+JB, J+JB ), LDA )
               END IF
