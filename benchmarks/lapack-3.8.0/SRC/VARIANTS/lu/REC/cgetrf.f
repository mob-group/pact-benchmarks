C> \brief \b AB_CGETRF VARIANT: iterative version of Sivan Toledo's recu
     $rsive LU algorithm
*
*  =========== DOCUMENTATION ===========
*
...
*        Apply pending permutations to L
         ...
         DO WHILE ( NTOPIV .LT. KAHEAD )
            CALL AB_CLASWP( NTOPIV, A( 1, JPIVSTART ), LDA, IPIVSTART, J
     $,
     $           IPIV, 1 )
...
*        Permute U block to match L
         CALL AB_CLASWP( KCOLS, A( 1,J+1 ), LDA, KSTART, J, IPIV, 1 )

*        Factor the current column
         PIVMAG = ABS( A( J, J ) )
         IF( PIVMAG.NE.ZERO .AND. .NOT.AB_SISNAN( PIVMAG ) ) THEN
...
*        Solve for U block.
         CALL AB_CTRSM( 'Left', 'Lower', 'No transpose', 'Unit', KAHEAD,
     $        KCOLS, ONE, A( KSTART, KSTART ), LDA,
     $        A( KSTART, J+1 ), LDA )
*        Schur complement.
         CALL AB_CGEMM( 'No transpose', 'No transpose', M-J,
...
*     Handle pivot permutations on the way out of the recursion
      ...
      DO WHILE ( J .GT. 0 )
         NTOPIV = IAND( J, -J )
         CALL AB_CLASWP( NTOPIV, A( 1, J-NTOPIV+1 ), LDA, J+1, NSTEP,
     $        IPIV, 1 )
         J = J - NTOPIV
      END DO
...
*     If short and wide, handle the rest of the columns.
      IF ( M .LT. N ) THEN
         CALL AB_CLASWP( N-M, A( 1, M+KCOLS+1 ), LDA, 1, M, IPIV, 1 )
         CALL AB_CTRSM( 'Left', 'Lower', 'No transpose', 'Unit', M,
