...
*  ===========
*
*       SUBROUTINE AB_CLATSQR( M, N, MB, NB, A, LDA, T, LDT, WORK,
*                           LWORK, INFO)
*
*       .. Scalar Arguments ..
...
*     .. EXTERNAL SUBROUTINES ..
      EXTERNAL    AB_AB_CGEQRT, AB_CTPQRT, AB_XERBLA
*     .. INTRINSIC FUNCTIONS ..
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
        CALL AB_XERBLA( 'AB_CLATSQR', -INFO )
        RETURN
      ELSE IF (LQUERY) THEN
       RETURN
      END IF
...
*
       IF ((MB.LE.N).OR.(MB.GE.M)) THEN
         CALL AB_AB_CGEQRT( M, N, NB, A, LDA, T, LDT, WORK, INFO)
         RETURN
       END IF
       KK = MOD((M-N),(MB-N))
       II=M-KK+1
...
*
       CALL AB_AB_CGEQRT( MB, N, NB, A(1,1), LDA, T, LDT, WORK, INFO )
       CTR = 1
*
...
*
         CALL AB_CTPQRT( MB-N, N, 0, NB, A(1,1), LDA, A( I, 1 ), LDA,
     $                 T(1,CTR * N + 1),
     $                  LDT, WORK, INFO )
         CTR = CTR + 1
       END DO
...
*
       IF (II.LE.M) THEN
         CALL AB_CTPQRT( KK, N, 0, NB, A(1,1), LDA, A( II, 1 ), LDA,
     $                 T(1, CTR * N + 1), LDT,
     $                  WORK, INFO )
       END IF
*
