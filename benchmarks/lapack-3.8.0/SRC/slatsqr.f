...
*  ===========
*
*       SUBROUTINE AB_SLATSQR( M, N, MB, NB, A, LDA, T, LDT, WORK,
*                           LWORK, INFO)
*
*       .. Scalar Arguments ..
...
*     .. EXTERNAL SUBROUTINES ..
      EXTERNAL           AB_AB_SGEQRT, AB_STPQRT, AB_XERBLA
*     .. INTRINSIC FUNCTIONS ..
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
        CALL AB_XERBLA( 'AB_SLATSQR', -INFO )
        RETURN
      ELSE IF (LQUERY) THEN
       RETURN
      END IF
...
*
       IF ((MB.LE.N).OR.(MB.GE.M)) THEN
         CALL AB_AB_SGEQRT( M, N, NB, A, LDA, T, LDT, WORK, INFO)
         RETURN
       END IF
       KK = MOD((M-N),(MB-N))
       II=M-KK+1
...
*
       CALL AB_AB_SGEQRT( MB, N, NB, A(1,1), LDA, T, LDT, WORK, INFO )
*
       CTR = 1
...
*
         CALL AB_STPQRT( MB-N, N, 0, NB, A(1,1), LDA, A( I, 1 ), LDA,
     $                 T(1, CTR * N + 1),
     $                  LDT, WORK, INFO )
         CTR = CTR + 1
       END DO
...
*
       IF (II.LE.M) THEN
         CALL AB_STPQRT( KK, N, 0, NB, A(1,1), LDA, A( II, 1 ), LDA,
     $                 T(1, CTR * N + 1), LDT,
     $                  WORK, INFO )
       END IF
*
