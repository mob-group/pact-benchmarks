...
*> This subroutine is called by AB_AB_AB_SGERFSX to perform iterative refinement.
...
*     .. External Subroutines ..
      EXTERNAL           AB_SAXPY, AB_SCOPY, AB_SGETRS, AB_SGEMV, BLAS_A
     $B_SGEMV_X,
     $                   BLAS_AB_SGEMV2_X, AB_SLA_GEAMV, AB_SLA_WWADDW, 
     $AB_SLAMCH,
...
*
     ...
     $              1.0, RES, 1 )
            ELSE IF ( Y_PREC_STATE .EQ. EXTRA_RESIDUAL ) THEN
               CALL BLAS_AB_SGEMV_X( TRANS_TYPE, N, N, -1.0, A, LDA,
     $              Y( 1, J ), 1, 1.0, RES, 1, PREC_TYPE )
            ELSE
               CALL BLAS_AB_SGEMV2_X( TRANS_TYPE, N, N, -1.0, A, LDA,
