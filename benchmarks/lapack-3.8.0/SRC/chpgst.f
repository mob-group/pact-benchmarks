...
*     .. External Subroutines ..
      EXTERNAL           AB_CAXPY, AB_CHPMV, AB_AB_CHPR2, AB_CAB_SSCAL, 
     $AB_CTPMV, AB_CTPSV,
     $                   AB_XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                     AP( J1 ), 1 )
               CALL AB_CAB_SSCAL( J-1, ONE / BJJ, AP( J1 ), 1 )
...
*
               ...
               AP( KK ) = AKK
               IF( K.LT.N ) THEN
                  CALL AB_CAB_SSCAL( N-K, ONE / BKK, AP( KK+1 ), 1 )
                  CT = -HALF*AKK
                  CALL AB_CAXPY( N-K, CT, BP( KK+1 ), 1, AP( KK+1 ), 1 )
                  CALL AB_AB_CHPR2( UPLO, N-K, -CONE, AP( KK+1 ), 1,
     $                        BP( KK+1 ), 1, AP( K1K1 ) )
                  CALL AB_CAXPY( N-K, CT, BP( KK+1 ), 1, AP( KK+1 ), 1 )
...
*
     ...
     $                     AP( K1 ), 1 )
               ...
               CALL AB_AB_CHPR2( UPLO, K-1, CONE, AP( K1 ), 1, BP( K1 ),
     $ 1,
     $                     AP )
               CALL AB_CAXPY( K-1, CT, BP( K1 ), 1, AP( K1 ), 1 )
               CALL AB_CAB_SSCAL( K-1, BKK, AP( K1 ), 1 )
               AP( KK ) = AKK*BKK**2
   30       CONTINUE
         ELSE
*
...
*
     ...
     $                    BP( JJ+1 ), 1 )
               CALL AB_CAB_SSCAL( N-J, BJJ, AP( JJ+1 ), 1 )
