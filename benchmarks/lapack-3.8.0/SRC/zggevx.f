*> \brief <b> AB_AB_ZGGEVX computes the eigenvalues and, optionally, the left and/or right eigenvectors for GE matrices</b>
*
*  =========== DOCUMENTATION ===========
*
...
*
*> \htmlonly
*> Download AB_AB_ZGGEVX + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_AB_ZGGEVX.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_AB_ZGGEVX.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_AB_ZGGEVX.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE AB_AB_ZGGEVX( BALANC, JOBVL, JOBVR, SENSE, N, A, LDA, B, LDB,
...
*> \verbatim
*>
*> AB_AB_ZGGEVX computes for a pair of N-by-N complex nonsymmetric matrices
...
*>          or both, then B contains the AB_SECOND part of the complex
...
*>                =N+2: error return from AB_ZTGEVC.
*> \endverbatim
*
*  Authors:
*  ========
...
*>
*  =====================================================================
      SUBROUTINE AB_AB_ZGGEVX( BALANC, JOBVL, JOBVR, SENSE, N, A, LDA, B
     $, LDB,
     $                   ALPHA, BETA, VL, LDVL, VR, LDVR, ILO, IHI,
     $                   LSCALE, RSCALE, ABNRM, BBNRM, RCONDE, RCONDV,
     $                   WORK, LWORK, RWORK, IWORK, BWORK, INFO )
...
*     .. External Subroutines ..
      EXTERNAL           AB_DLABAD, AB_DLASCL, AB_XERBLA, AB_AB_ZGEQRF, 
     $AB_ZGGBAK, AB_ZGGBAL,
     $                   AB_ZGGHRD, AB_ZHGEQZ, AB_ZLACPY, AB_ZLASCL, AB_
     $ZLASET, AB_ZTGEVC,
     $                   AB_ZTGSNA, AB_ZUNGQR, AB_ZUNMQR
*     ..
*     .. External Functions ..
      LOGICAL            AB_LSAME
      INTEGER            AB_ILAENV
...
*
      IF( INFO.EQ.0 ) THEN
         ...
         ELSE
            ...
            MAXWRK = MINWRK
            MAXWRK = MAX( MAXWRK,
     $                    N + N*AB_ILAENV( 1, 'AB_AB_ZGEQRF', ' ', N, 1,
     $ N, 0 ) )
            MAXWRK = MAX( MAXWRK,
     $                    N + N*AB_ILAENV( 1, 'AB_ZUNMQR', ' ', N, 1, N,
     $ 0 ) )
            IF( ILVL ) THEN
               MAXWRK = MAX( MAXWRK, N +
     $                       N*AB_ILAENV( 1, 'AB_ZUNGQR', ' ', N, 1, N, 
     $0 ) )
            END IF
         END IF
         WORK( 1 ) = MAXWRK
...
*
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_ZGGEVX', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
*
      CALL AB_ZGGBAL( BALANC, N, A, LDA, B, LDB, ILO, IHI, LSCALE, RSCAL
     $E,
     $             RWORK, IERR )
*
*     Compute ABNRM and BBNRM
*
      ABNRM = AB_ZLANGE( '1', N, N, A, LDA, RWORK( 1 ) )
      IF( ILASCL ) THEN
         RWORK( 1 ) = ABNRM
         CALL AB_DLASCL( 'G', 0, 0, ANRMTO, ANRM, 1, 1, RWORK( 1 ), 1,
     $                IERR )
         ABNRM = RWORK( 1 )
      END IF
*
      BBNRM = AB_ZLANGE( '1', N, N, B, LDB, RWORK( 1 ) )
      IF( ILBSCL ) THEN
         RWORK( 1 ) = BBNRM
         CALL AB_DLASCL( 'G', 0, 0, BNRMTO, BNRM, 1, 1, RWORK( 1 ), 1,
     $                IERR )
         BBNRM = RWORK( 1 )
      END IF
*
...
*     AB_ZTGEVC: (Complex Workspace: need 2*N )
*             (Real Workspace:    need 2*N )
*     AB_ZTGSNA: (Complex Workspace: need 2*N*N if SENSE='V' or 'B')
...
*
      IF( ILV .OR. .NOT.WANTSN ) THEN
         IF( ILV ) THEN
            ...
            END IF
*
            CALL AB_ZTGEVC( CHTEMP, 'B', LDUMMA, N, A, LDA, B, LDB, VL,
...
*
               IF( WANTSE .OR. WANTSB ) THEN
                  CALL AB_ZTGEVC( 'B', 'S', BWORK, N, A, LDA, B, LDB,
     ...
     $                         WORK( IWRK1 ), RWORK, IERR )
               ...
               END IF
*
               CALL AB_ZTGSNA( SENSE, 'S', BWORK, N, A, LDA, B, LDB,
...
*
      IF( ILVL ) THEN
         CALL AB_ZGGBAK( BALANC, 'L', N, ILO, IHI, LSCALE, RSCALE, N, VL
     $,
     $                LDVL, IERR )
*
         DO 50 JC = 1, N
...
*
      IF( ILVR ) THEN
         CALL AB_ZGGBAK( BALANC, 'R', N, ILO, IHI, LSCALE, RSCALE, N, VR
     $,
     $                LDVR, IERR )
         DO 80 JC = 1, N
            TEMP = ZERO
