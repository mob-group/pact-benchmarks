...
*
*> \htmlonly
*> Download AB_AB_AB_ZHETRI2X + dependencies
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.tgz?format=tgz&filename=/lapack/lapack_routine/AB_AB_AB_ZHETRI2X.f">
*> [TGZ]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.zip?format=zip&filename=/lapack/lapack_routine/AB_AB_AB_ZHETRI2X.f">
*> [ZIP]</a>
*> <a href="http://www.netlib.org/cgi-bin/netlibfiles.txt?format=txt&filename=/lapack/lapack_routine/AB_AB_AB_ZHETRI2X.f">
*> [TXT]</a>
*> \endhtmlonly
*
*  Definition:
*  ===========
*
*       SUBROUTINE AB_AB_AB_ZHETRI2X( UPLO, N, A, LDA, IPIV, WORK, NB, INFO )
*
*       .. Scalar Arguments ..
*       CHARACTER          UPLO
...
*> \verbatim
*>
*> AB_AB_AB_ZHETRI2X computes the inverse of a COMPLEX*16 Hermitian indefinite matrix
...
*
*  =====================================================================
      SUBROUTINE AB_AB_AB_ZHETRI2X( UPLO, N, A, LDA, IPIV, WORK, NB, INF
     $O )
*
...
*     .. External Subroutines ..
      EXTERNAL           AB_AB_ZSYCONV, AB_XERBLA, AB_ZTRTRI
      EXTERNAL           AB_ZGEMM, AB_ZTRMM, AB_ZHESWAPR
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
*     ..
...
*
      IF( INFO.NE.0 ) THEN
         CALL AB_XERBLA( 'AB_AB_AB_ZHETRI2X', -INFO )
         RETURN
      END IF
      IF( N.EQ.0 )
     $   RETURN
...
*
         CALL AB_ZGEMM('C','N',NNB,NNB,CUT,CONE,A(1,CUT+1),LDA,
...
*        U01 =  U00**H*invD0*U01
*
         CALL AB_ZTRMM('L',UPLO,'C','U',CUT, NNB,
...
*
            ...
            DO WHILE ( I .LE. N )
               IF( IPIV(I) .GT. 0 ) THEN
                  IP=IPIV(I)
                 IF (I .LT. IP) CALL AB_ZHESWAPR( UPLO, N, A, LDA, I ,IP
     $ )
                 IF (I .GT. IP) CALL AB_ZHESWAPR( UPLO, N, A, LDA, IP ,I
     $ )
               ELSE
                 IP=-IPIV(I)
                 I=I+1
                 IF ( (I-1) .LT. IP)
     $                  CALL AB_ZHESWAPR( UPLO, N, A, LDA, I-1 ,IP )
                 IF ( (I-1) .GT. IP)
     $                  CALL AB_ZHESWAPR( UPLO, N, A, LDA, IP ,I-1 )
              ENDIF
               I=I+1
            END DO
      ELSE
...
*       L11**H*invD1*L11->L11
*
        CALL AB_ZTRMM('L',UPLO,'C','U',NNB, NNB,
     $             CONE,A(CUT+1,CUT+1),LDA,WORK(U11+1,1),N+NB+1)
*
         DO I=1,NNB
            DO J=1,I
...
*
         CALL AB_ZGEMM('C','N',NNB,NNB,N-NNB-CUT,CONE,A(CUT+NNB+1,CUT+1)
...
*        L01 =  L22**H*invD2*L21
*
         CALL AB_ZTRMM('L',UPLO,'C','U', N-NNB-CUT, NNB,
...
*
     ...
     $ )
               ELSE
                 IP=-IPIV(I)
                 IF ( I .LT. IP) CALL AB_ZHESWAPR( UPLO, N, A, LDA, I ,I
     $P )
                 IF ( I .GT. IP) CALL AB_ZHESWAPR( UPLO, N, A, LDA, IP ,
     $I )
                 I=I-1
               ENDIF
               I=I-1
