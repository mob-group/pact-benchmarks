...
*     .. EXTERNAL SUBROUTINES ..
      EXTERNAL           AB_AB_SGELQT, AB_AB_SGEQRT, AB_STPLQT, AB_STPQR
     $T, AB_XERBLA
*     .. INTRINSIC FUNCTIONS ..
...
*
       IF((M.GE.N).OR.(NB.LE.M).OR.(NB.GE.N)) THEN
        CALL AB_AB_SGELQT( M, N, MB, A, LDA, T, LDT, WORK, INFO)
        RETURN
       END IF
*
       KK = MOD((N-M),(NB-M))
...
*
       CALL AB_AB_SGELQT( M, NB, MB, A(1,1), LDA, T, LDT, WORK, INFO)
       CTR = 1
*
