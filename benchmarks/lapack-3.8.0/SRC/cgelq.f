...
*  ===========
*
*       SUBROUTINE AB_CGELQ( M, N, A, LDA, T, TSIZE, WORK, LWORK,
*                         INFO )
*
*       .. Scalar Arguments ..
...
*>
*> \verbatim
*> AB_CGELQ computes a LQ factorization of an M-by-N matrix A.
*> \endverbatim
*
*  Arguments:
*  ==========
...
*>                           AB_CLASWLQ or AB_AB_CGELQT
*>
*>  Depending on the matrix dimensions M and N, and row and column
*>  block sizes MB and NB returned by AB_ILAENV, AB_CGELQ will use either
*>  AB_CLASWLQ (if the matrix is short-and-wide) or AB_AB_CGELQT to compute
...
*     .. External Subroutines ..
      EXTERNAL           AB_AB_CGELQT, AB_CLASWLQ, AB_XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( MIN( M, N ).GT.0 ) THEN
        MB = AB_ILAENV( 1, 'AB_CGELQ ', ' ', M, N, 1, -1 )
        NB = AB_ILAENV( 1, 'AB_CGELQ ', ' ', M, N, 2, -1 )
      ELSE
        MB = 1
        NB = N
      END IF
...
*
      IF( ( N.LE.M ) .OR. ( NB.LE.M ) .OR. ( NB.GE.N ) ) THEN
        CALL AB_AB_CGELQT( M, N, MB, A, LDA, T( 6 ), MB, WORK, INFO )
      ELSE
        CALL AB_CLASWLQ( M, N, MB, NB, A, LDA, T( 6 ), MB, WORK,
     $                LWORK, INFO )
      END IF
*
      WORK( 1 ) = MAX( 1, MB*M )
