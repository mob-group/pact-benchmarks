...
*> This subroutine is called by AB_AB_AB_CSYRFSX to perform iterative refinement.
...
*     .. External Subroutines ..
      EXTERNAL           AB_CAXPY, AB_CCOPY, AB_CSYTRS, AB_CSYMV, BLAS_A
     $B_CSYMV_X,
     $                   BLAS_AB_CSYMV2_X, AB_CLA_SYAMV, AB_CLA_WWADDW,
     $                   AB_CLA_LIN_BERR
      REAL               AB_SLAMCH
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $              CMPLX(1.0), RES, 1 )
            ELSE IF ( Y_PREC_STATE .EQ. EXTRA_RESIDUAL ) THEN
               CALL BLAS_AB_CSYMV_X( UPLO2, N, CMPLX(-1.0), A, LDA,
     $              Y( 1, J ), 1, CMPLX(1.0), RES, 1, PREC_TYPE )
            ELSE
               CALL BLAS_AB_CSYMV2_X(UPLO2, N, CMPLX(-1.0), A, LDA,
