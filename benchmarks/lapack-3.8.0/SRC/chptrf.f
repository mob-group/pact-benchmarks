...
*     .. External Subroutines ..
      EXTERNAL           AB_CHPR, AB_CAB_SSCAL, AB_CSWAP, AB_XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
               CALL AB_CAB_SSCAL( K-1, R1, AP( KC ), 1 )
            ELSE
*
...
*
                  KP = K
               ELSE IF( ABS( REAL( AP( KPC ) ) ).GE.ALPHA*ROWMAX ) TH
     $EN
*
*                 interchange rows and columns K and IMAX, use 1-by-1
*                 pivot block
...
*
                  CALL AB_CAB_SSCAL( N-K, R1, AP( KC+1 ), 1 )
               END IF
            ELSE
*
*              2-by-2 pivot block D(k): columns K and K+1 now hold
