...
*     .. External Subroutines ..
      EXTERNAL           AB_XERBLA, AB_ZGEQRF, AB_ZLAQP2, AB_ZLAQPS, AB_
     $ZSWAP, AB_ZUNMQR
*     ..
*     .. External Functions ..
      INTEGER            AB_ILAENV
...
*
      IF( INFO.EQ.0 ) THEN
         ...
         ELSE
            IWS = N + 1
            NB = AB_ILAENV( INB, 'AB_ZGEQRF', ' ', M, N, -1, -1 )
            LWKOPT = ( N + 1 )*NB
         END IF
...
*
      IF( NFXD.GT.0 ) THEN
         NA = MIN( M, NFXD )
*CC      CALL AB_ZGEQR2( M, NA, A, LDA, TAU, WORK, INFO )
         CALL AB_ZGEQRF( M, NA, A, LDA, TAU, WORK, LWORK, INFO )
...
*
         NB = AB_ILAENV( INB, 'AB_ZGEQRF', ' ', SM, SN, -1, -1 )
         NBMIN = 2
         NX = 0
*
...
*
            NX = MAX( 0, AB_ILAENV( IXOVER, 'AB_ZGEQRF', ' ', SM, SN, -1
     $,
     $           -1 ) )
*
*
...
*
                  NB = LWORK / ( SN+1 )
                  NBMIN = MAX( 2, AB_ILAENV( INBMIN, 'AB_ZGEQRF', ' ', S
     $M, SN,
     $                    -1, -1 ) )
*
*
