...
*     .. External Subroutines ..
      EXTERNAL           AB_AB_DGEQR2, AB_DLARF, AB_AB_DLARFG, AB_DORM2R
     $, AB_DSWAP, AB_XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( ITEMP.GT.0 ) THEN
         MA = MIN( ITEMP, M )
         CALL AB_AB_DGEQR2( M, MA, A, LDA, TAU, WORK, INFO )
         IF( MA.LT.N ) THEN
...
*
            IF( I.LT.M ) THEN
               CALL AB_AB_DLARFG( M-I+1, A( I, I ), A( I+1, I ), 1, TAU(
     $ I ) )
            ELSE
               CALL AB_AB_DLARFG( 1, A( M, M ), A( M, M ), 1, TAU( M ) )
            END IF
*
            IF( I.LT.N ) THEN
*
