...
*     .. External Functions ..
      COMPLEX            AB_CLADIV
      REAL               AB_SLAMCH
      EXTERNAL           AB_CLADIV, AB_SLAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           AB_CCOPY, AB_AB_CLARFG, AB_CSCAL, AB_SLABAD
*     ..
*     .. Statement Functions ..
      REAL               CABS1
*     ..
...
*           submatrix.
*
*           V(2) is always real before the call to AB_AB_CLARFG, and hence
*           after the call T2 ( = T1*V(2) ) is also real.
*
            IF( K.GT.M )
     $         CALL AB_CCOPY( 2, H( K, K-1 ), 1, V, 1 )
            CALL AB_AB_CLARFG( 2, V( 1 ), V( 2 ), 1, T1 )
            IF( K.GT.M ) THEN
               H( K, K-1 ) = V( 1 )
               H( K+1, K-1 ) = ZERO
            END IF
