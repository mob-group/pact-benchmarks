...
*     .. External Subroutines ..
      EXTERNAL           AB_CLARFG, AB_CLARFX, AB_CLARFY
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          CONJG, MOD
*     .. External Functions ..
...
*
   ...
   10         CONTINUE
              ...
              CALL AB_CLARFG( LM, CTMP, V( VPOS+1 ), 1, 
     $                                       TAU( TAUPOS ) )
              A( OFDPOS, ST ) = CTMP
*
              LM = ED - ST + 1
              CALL AB_CLARFY( UPLO, LM, V( VPOS ), 1,
     $                     CONJG( TAU( TAUPOS ) ),
...
*
          IF( TTYPE.EQ.2 ) THEN
              ...
              LM = J2-J1+1
              IF( LM.GT.0) THEN
                  CALL AB_CLARFX( 'Left', LN, LM, V( VPOS ),
     $                         CONJG( TAU( TAUPOS ) ),
...
*
   ...
   30             CONTINUE
                  ...
                  CALL AB_CLARFG( LM, CTMP, V( VPOS+1 ), 1, TAU( TAUPOS 
     $) )
                  A( DPOS-NB, J1 ) = CTMP
*                 
                  CALL AB_CLARFX( 'Right', LN-1, LM, V( VPOS ),
     $                         TAU( TAUPOS ),
...
*
   ...
   20         CONTINUE
              CALL AB_CLARFG( LM, A( OFDPOS, ST-1 ), V( VPOS+1 ), 1, 
     $                                       TAU( TAUPOS ) )
*
              LM = ED - ST + 1
*
...
*
              IF( LM.GT.0) THEN
                  CALL AB_CLARFX( 'Right', LM, LN, V( VPOS ), 
...
*
                  ...
                  DO 40 I = 1, LM-1
                      ...
                      A( DPOS+NB+I, ST ) = ZERO
   40             CONTINUE
                  CALL AB_CLARFG( LM, A( DPOS+NB, ST ), V( VPOS+1 ), 1, 
     $                                        TAU( TAUPOS ) )
*
                  CALL AB_CLARFX( 'Left', LM, LN-1, V( VPOS ), 
     $                         CONJG( TAU( TAUPOS ) ),
