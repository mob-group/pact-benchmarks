...
*     .. External Subroutines ..
      EXTERNAL           AB_CGEMM, AB_CLACPY, AB_CLAQR1, AB_CLARFG, AB_C
     $LASET, AB_CTRMM,
     $                   AB_SLABAD
*     ..
*     .. Statement Functions ..
...
*
     ...
     $                         S( 2*M ), V( 1, M ) )
                  ALPHA = V( 1, M )
                  CALL AB_CLARFG( 3, ALPHA, V( 2, M ), 1, V( 1, M ) )
               ELSE
                  BETA = H( K+1, K )
                  V( 2, M ) = H( K+2, K )
                  V( 3, M ) = H( K+3, K )
                  CALL AB_CLARFG( 3, BETA, V( 2, M ), 1, V( 1, M ) )
*
...
*
     ...
     $                            S( 2*M ), VT )
                     ALPHA = VT( 1 )
                     CALL AB_CLARFG( 3, ALPHA, VT( 2 ), 1, VT( 1 ) )
                     REFSUM = CONJG( VT( 1 ) )*
     $                        ( H( K+1, K )+CONJG( VT( 2 ) )*
     $                        H( K+2, K ) )
*
...
*
     ...
     $                         S( 2*M22 ), V( 1, M22 ) )
                  BETA = V( 1, M22 )
                  CALL AB_CLARFG( 2, BETA, V( 2, M22 ), 1, V( 1, M22 ) )
               ELSE
                  BETA = H( K+1, K )
                  V( 2, M22 ) = H( K+2, K )
                  CALL AB_CLARFG( 2, BETA, V( 2, M22 ), 1, V( 1, M22 ) )
